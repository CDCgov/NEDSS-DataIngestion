...
class KafkaProducerServiceTest {
  ...
  void testSendMessageFromController() throws KafkaProducerException {
    ...
    kafkaProducerService.sendMessageFromController(
        msg, topic, msgType, dltOccurrence, false, "false");
  ...
  @Test
  void testSendMessagePreparationTopicXML()
    ...
    kafkaProducerService.sendMessagePreparationTopic(model, topic, topicType, 1, "false");
    verify(kafkaTemplate, times(1)).send(any(ProducerRecord.class));
  }

  @Test
  void testSendMessagePreparationTopicFHIR()
      throws ConversionPrepareException, KafkaProducerException {
  ...
  void testSendElrXmlMessageFromController() throws KafkaProducerException {
    ...
    kafkaProducerService.sendElrXmlMessageFromController(
        msg, topic, msgType, dltOccurrence, "payload", "false");
    verify(kafkaTemplate, times(1)).send(any(ProducerRecord.class));
  }
  ...
  void sendMessageDlt() throws KafkaProducerException {
    ...
    kafkaProducerService.sendMessageDlt("test", "test", topic, 1, "error", topic);
    verify(kafkaTemplate, times(1)).send(any(ProducerRecord.class));
  }
  ...
  void testSendMessageAfterConvertedToXml() throws KafkaProducerException {
    ...
    kafkaProducerService.sendMessageAfterConvertedToXml(msg, topic, 1);
    verify(kafkaTemplate, times(1)).send(any(ProducerRecord.class));
  }
  ...
  void testSendMessageAfterCheckingDuplicateHL7() throws KafkaProducerException {
    ...
    kafkaProducerService.sendMessageAfterCheckingDuplicateHL7(model, topic, 1);
    verify(kafkaTemplate, times(1)).send(any(ProducerRecord.class));
  }
  ...
  void testSendMessageException() throws Exception {
    ...
    Method sendMessageMethod =
        KafkaProducerService.class.getDeclaredMethod("sendMessage", ProducerRecord.class);
    sendMessageMethod.setAccessible(true);

    KafkaProducerException exception =
        assertThrows(
    ...
    Assertions.assertEquals(
        "Failed publishing message to kafka topic: test-topic with UUID: test-value",

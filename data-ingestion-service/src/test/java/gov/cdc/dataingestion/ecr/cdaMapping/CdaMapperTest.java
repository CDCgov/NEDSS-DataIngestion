...
class CdaMapperTest {
  @Mock private ICdaLookUpService cdaLookUpService;

  @InjectMocks private CdaMapper target;
  ...
  void transformSelectedEcrToCDAXml_Test() throws EcrCdaXmlException {
    ...
    verify(cdaLookUpService, never())
        .fetchConstantLookUpByCriteriaWithColumn("QuestionIdentifier", "CUS101");
  ...
  void transformSelectedEcrToCDAXml_Test_TypePart() throws EcrCdaXmlException {
    ...
    // Using the correct mock (interface) here
    when(cdaLookUpService.fetchPhdcQuestionByCriteria("")).thenReturn(questionLookup);

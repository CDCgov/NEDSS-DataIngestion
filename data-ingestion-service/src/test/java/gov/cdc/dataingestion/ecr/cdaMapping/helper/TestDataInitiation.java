...
public class TestDataInitiation {
  private static EcrSelectedRecord initTestData(EcrSelectedRecord ecrObject)
    ...
    for (int i = 0;
        i < ecrObject.getMsgInterviews().get(0).getMsgInterviewProviders().size();
        i++) {
    ...
    for (int i = 0;
        i < ecrObject.getMsgInterviews().get(0).getMsgInterviewAnswerRepeats().size();
        i++) {
    ...
    for (int i = 0;
        i < ecrObject.getMsgTreatments().get(0).getMsgTreatmentProviders().size();
        i++) {
      ecrObject.getMsgTreatments().get(0).getMsgTreatmentProviders().get(i).initDataMap();
    }
    for (int i = 0;
        i < ecrObject.getMsgTreatments().get(0).getMsgTreatmentOrganizations().size();
        i++) {
  ...
  public static EcrSelectedRecord getTestData() {
    ...
    try (Reader reader =
        new InputStreamReader(
            JsonReader.class.getResourceAsStream("/test-data-ecr/ecr.json"), "UTF-8")) {

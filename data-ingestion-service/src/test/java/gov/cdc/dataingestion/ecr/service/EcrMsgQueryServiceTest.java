...
class EcrMsgQueryServiceTest {
  @Mock private IEcrMsgQueryRepository ecrMsgQueryRepository;
  @Mock private EcrMsgContainerResolver ecrMsgContainerResolver;
  @InjectMocks private EcrMsgQueryService target;
  ...
  void getSelectedEcrRecord_Test() throws EcrCdaXmlException {
    ...
    caseList.add(cases);
    when(ecrMsgQueryRepository.fetchMsgCaseForApplicableEcr(container.msgContainerUid()))
        .thenReturn(caseList);
    ...
    when(ecrMsgQueryRepository.fetchMsgCaseParticipantForApplicableEcr(
            container.msgContainerUid(), cases.getInvLocalId()))
    ...
    when(ecrMsgQueryRepository.fetchMsgCaseAnswerForApplicableEcr(
            container.msgContainerUid(), cases.getInvLocalId()))
        .thenReturn(caseAnsList);
    ...
    when(ecrMsgQueryRepository.fetchMsgCaseAnswerRepeatForApplicableEcr(
            container.msgContainerUid(), cases.getInvLocalId()))
        .thenReturn(caseAnsReList);

    EcrMsgXmlAnswerDto xmlAns = new EcrMsgXmlAnswerDto();
    List<EcrMsgXmlAnswerDto> xmlAnsList = new ArrayList<>();
    xmlAnsList.add(xmlAns);
    when(ecrMsgQueryRepository.fetchMsgXmlAnswerForApplicableEcr(
            container.msgContainerUid(), cases.getInvLocalId()))
        .thenReturn(xmlAnsList);
    ...
    when(ecrMsgQueryRepository.fetchMsgInterviewProviderForApplicableEcr(
            container.msgContainerUid(), inx.getIxsLocalId()))
        .thenReturn(providerInxList);
    ...
    when(ecrMsgQueryRepository.fetchMsgInterviewAnswerForApplicableEcr(
            container.msgContainerUid(), inx.getIxsLocalId()))
        .thenReturn(caseAnsInxList);
    ...
    when(ecrMsgQueryRepository.fetchMsgInterviewAnswerRepeatForApplicableEcr(
            container.msgContainerUid(), inx.getIxsLocalId()))
        .thenReturn(caseAnsReInxList);
    ...
    when(ecrMsgQueryRepository.fetchMsgTreatmentProviderForApplicableEcr(
            eq(container.msgContainerUid()), anyString()))
    ...
    when(ecrMsgQueryRepository.fetchMsgTreatmentOrganizationForApplicableEcr(
            eq(container.msgContainerUid()), anyString()))

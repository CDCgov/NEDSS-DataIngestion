...
class ElrReportsControllerTest {
  ...
  @MockitoBean private RawElrService rawELRService;
  @MockitoBean private CustomMetricsBuilder customMetricsBuilder;
  @MockitoBean private IHL7Service hl7Service;
  ...
  void testSaveHL7Message() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/elrs")
  ...
  void testHl7Validator() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/elrs/validate")
                .contentType("text/plain")
                .content(hl7Payload)
                .with(SecurityMockMvcRequestPostProcessors.jwt()))
        .andExpect(MockMvcResultMatchers.status().isOk());

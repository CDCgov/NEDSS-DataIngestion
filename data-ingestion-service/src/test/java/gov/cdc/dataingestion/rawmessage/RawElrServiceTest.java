...
class RawElrServiceTest {
  ...
  @InjectMocks private RawElrService target;
  ...
  void testUpdateRawMessageAfterRetry_XML_Success() throws KafkaProducerException {
    ...
    Mockito.doNothing()
        .when(kafkaProducerService)
        .sendElrXmlMessageFromController(
            Mockito.any(),
            Mockito.any(),
            Mockito.any(),
            Mockito.any(),
  ...
  void testUpdateRawMessageAfterRetry_KafkaProducerException() throws KafkaProducerException {
    ...
    modelDto.setType("HL7");
    Mockito.doThrow(new KafkaProducerException("Retry failed"))
        .when(kafkaProducerService)
        .sendMessageFromController(
            Mockito.any(),
            Mockito.any(),
    ...
    Assertions.assertThrows(
        KafkaProducerException.class, () -> target.updateRawMessageAfterRetry(modelDto, 1));
    Mockito.verify(iElrDeadLetterRepository)
        .updateDltOccurrenceForRawId(Mockito.any(), Mockito.eq(2), Mockito.eq("ERROR"));
  }
  ...
  void testSaveHL7_KafkaProducerException() throws KafkaProducerException {
    ...
    Mockito.doThrow(new KafkaProducerException("Failed sending message to kafka"))
        .when(kafkaProducerService)
        .sendMessageFromController(
            Mockito.any(),
            Mockito.any(),

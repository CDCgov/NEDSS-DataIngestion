...
public class OBRSplitter {
  ...
  private List<HL7ParsedMessage<OruR1>> splitElrByOBX(
      HL7ParsedMessage<OruR1> parsedMessageOrig, List<OrderObservation> obrListToSplit) {
    ...
    CommonOrder orc =
        oruR1.getPatientResult().getFirst().getOrderObservation().getFirst().getCommonOrder();
    Gson gson = new Gson();

    log.info(
        "OBR size in original Message:"
            + oruR1.getPatientResult().getFirst().getOrderObservation().size());
    ...
    for (OrderObservation orderObservation : obrListToSplit) {
      ...
      for (int j = 0; j < obxList.size(); j++) {
        ...
        OrderObservation obrCopy =
            gson.fromJson(gson.toJson(orderObservation), OrderObservation.class);
        obrCopy.getObservationRequest().setSetIdObr("1");

        // To make split OBRs unique to defeat snapshot processing, make addtional changes to the
        // specimen collection time, //NOSONAR
        ...
        // out.Messages[i].ORUR01.MSH.SendingFacility.UniversalID =
        // (out.Messages[i].ORUR01.MSH.SendingFacility.UniversalID + (i * .01) );//NOSONAR
        String newUniversalID =
            oruR1Copy.getMessageHeader().getSendingFacility().getUniversalId() + ((j + 1) * .01);
  ...
  private List<HL7ParsedMessage<OruR1>> splitElrByOBR(HL7ParsedMessage<OruR1> parsedMessageOrig) {
    ...
    if (obrList != null && obrList.size() > 1) {
      ...
      // ORC data is available only in the first OBR object and needs to be copied to the other
      // OBRs.
      CommonOrder orc =
          oruR1.getPatientResult().getFirst().getOrderObservation().getFirst().getCommonOrder();
      ...
      for (int i = 0; i < obrList.size(); i++) {
        ...
        String obr_2_1_placerOrderId =
            obr.getObservationRequest().getPlacerOrderNumber().getEntityIdentifier();
        String obr_3_1_fillerOrderId =
            obr.getObservationRequest().getFillerOrderNumber().getEntityIdentifier();
        String obrId = obr.getObservationRequest().getSetIdObr();
        log.info(
            "Parent SetIdObr:"
                + obrId
                + " placerOrderId:"
                + obr_2_1_placerOrderId
                + " fillerOrderId:"
                + obr_3_1_fillerOrderId);
        ...
        if (!obrIds.contains(obrId)) {
          ...
          for (Observation obx : obxList) {
            ...
            // OBX-3 for Parent
            String obx_3_1_id =
                obx.getObservationResult().getObservationIdentifier().getIdentifier();
            String obx_3_2_text = obx.getObservationResult().getObservationIdentifier().getText();
            String obx_3_3_nameOfCodeSys =
                obx.getObservationResult().getObservationIdentifier().getNameOfCodingSystem();
            log.info(
                "Parent OBX-3 - obx_3_1_id:"
                    + obx_3_1_id
                    + " obx_3_2_text:"
                    + obx_3_2_text
                    + " obx_3_3_nameOfCodeSys:"
                    + obx_3_3_nameOfCodeSys);
            // OBX-4 for Parent
            ...
            for (int j = i + 1; j < obrList.size(); j++) {
              ...
              if (!obrIds.contains(nextObrId)) {
                ...
                String obr_29_1_1_placerOrderNumId =
                    nextObr
                        .getObservationRequest()
                        .getParent()
                        .getPlacerAssignedIdentifier()
                        .getEntityIdentifier();
                String obr_29_2_1_fillerOrderNumId =
                    nextObr
                        .getObservationRequest()
                        .getParent()
                        ...
                        .getEntityIdentifier();
                log.info(
                    "Child:OBR-29 obr_29_2_1_fillerOrderNumId:"
                        + obr_29_2_1_fillerOrderNumId
                        + " obr_29_1_1_placerOrderNumId:"
                        + obr_29_1_1_placerOrderNumId
                        + " parent:obr_2_1_placerOrderId"
                        + obr_2_1_placerOrderId
                        + " obr_3_1_fillerOrderId:"
                        + obr_3_1_fillerOrderId);
                // OBR-26 Parent Result link from the child obr
                String obr_26_1_1_parentRsltId =
                    nextObr
                        .getObservationRequest()
                        .getParentResult()
                        .getParentObservationIdentifier()
                        .getIdentifier();
                String obr_26_1_2_parentRsltTxt =
                    nextObr
                        .getObservationRequest()
                        .getParentResult()
                        .getParentObservationIdentifier()
                        .getText();
                String obr_26_1_3_parentRsltCodeSys =
                    nextObr
                        .getObservationRequest()
                        .getParentResult()
                        .getParentObservationIdentifier()
                ...
                log.info(
                    "Child obr_26_1_1_parentRsltId:"
                        + obr_26_1_1_parentRsltId
                        + " obr_26_1_2_parentRsltTxt:"
                        + obr_26_1_2_parentRsltTxt
                        + " obr_26_1_3_parentRsltCodeSys:"
                        + obr_26_1_3_parentRsltCodeSys);

                String obr_26_2_parentRsltSubId =
                    nextObr
                        .getObservationRequest()
                        .getParentResult()
                ...
                // Child OBR-29.2.1(filler order number) = Parent OBR-3.1(filler order
                // number);//NOSONAR
                // Child OBR-29.1.1(placer order number) = Parent OBR-2.1(Placer order
                // number);//NOSONAR
                // the filler order may be blank but then it is blank in both OBR.3.1 and
                // OBR.29.2.1//NOSONAR
                // Child OBR-26.1 = OBX-3 from the parent OBX;//NOSONAR
                // Child OBR-26.2 = OBX-4 from parent OBX;//NOSONAR
                if (StringUtils.equals(obr_29_1_1_placerOrderNumId, obr_2_1_placerOrderId)
                    && StringUtils.equals(obr_29_2_1_fillerOrderNumId, obr_3_1_fillerOrderId)
                    && StringUtils.equals(obr_26_1_1_parentRsltId, obx_3_1_id)
                    && StringUtils.equals(obr_26_1_2_parentRsltTxt, obx_3_2_text)
                    && StringUtils.equals(obr_26_1_3_parentRsltCodeSys, obx_3_3_nameOfCodeSys)
                    && StringUtils.equals(obr_26_2_parentRsltSubId, obx_4_subId)) {
                  log.info(
                      "Matching parent-child found. parent obr:"
                          + obrId
                          + "child Obr:"
                          + nextObrId);
                  OrderObservation nextObrCopy =
                      gson.fromJson(gson.toJson(nextObr), OrderObservation.class);
  ...
  private void changeSctToSnmForCodingSystem(Observation obx) {
    ...
    if (valueType != null && valueType.equals("CWE")) {
      obx.getObservationResult()
          .getObservationValue()
          .replaceAll(str -> str.replace("^SCT^", "^SNM^"));
    }
  }

  private static String getCustomMessageControlId(String messageControlId, int i) {
    return StringUtils.left(messageControlId, 8)
        + i
        + (i + 1)
        + (i + 2)
        + StringUtils.right(messageControlId, 9);
  }

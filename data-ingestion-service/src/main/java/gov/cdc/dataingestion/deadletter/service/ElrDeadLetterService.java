...
public class ElrDeadLetterService {
  ...
  private static final String DEAD_LETTER_NULL_EXCEPTION =
      "The Record does not exist in elr_dlt. Please try with a different ID";
  private static final String START_END_DATE_RANGE_MSG =
  ...
  public List<ElrDeadLetterDto> getAllErrorDltRecord() {
    Optional<List<ElrDeadLetterModel>> deadLetterELRModels =
        dltRepository.findAllDltRecordByDltStatus(
  ...
  public List<ElrDeadLetterDto> getDltErrorsByDate(String startDate, String endDate)
    ...
    try {
      ...
      Optional<List<ElrDeadLetterModel>> deadLetterELRModels =
          dltRepository.findAllDltRecordsByDate(startDateWithTime, endDateWithTime);
      if (deadLetterELRModels.isPresent()) {
        results = convertModelToDtoList(deadLetterELRModels.get());
      }
    } catch (Exception e) {
    ...
    if (!isValidUUID(id)) {
      if (id.isEmpty()) {
        ...
        throw new DeadLetterTopicException(
            id + " is an Invalid Unique Id, please provide the correct id.");
      }
    }
  ...
  public ElrDeadLetterDto updateAndReprocessingMessage(String id, String body)
    ...
    if (!existingRecord.getDltStatus().equalsIgnoreCase(EnumElrDltStatus.ERROR.name())) {
      throw new DeadLetterTopicException(
          "Selected record is in REINJECTED state. Please either wait for the ERROR state to occur or select a different record.");
    }
    ...
    if (existingRecord.getErrorMessageSource().equalsIgnoreCase(rawTopic)) {
      ...
      kafkaProducerService.sendMessageFromDltController(
          rawModel.getId(), rawTopic, rawModel.getType(), existingRecord.getDltOccurrence());
      ...
      kafkaProducerService.sendMessageFromDltController(
          validateModel.getId(),
          validatedTopic,
          validateModel.getMessageType(),
          existingRecord.getDltOccurrence());
      ...
      kafkaProducerService.sendMessageFromDltController(
          existingRecord.getErrorMessageId(),
          topic,
          EnumMessageType.HL7.name(),
          existingRecord.getDltOccurrence());
    } else {

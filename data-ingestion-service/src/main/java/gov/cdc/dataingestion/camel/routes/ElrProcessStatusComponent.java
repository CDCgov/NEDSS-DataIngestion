...
public class ElrProcessStatusComponent {
  ...
  public String process(String body) {
    ...
    if (body != null && !body.trim().isEmpty() && body.split(":").length == 2) {
      // body =
      // "HL7file-sftpstatus1.txt:64BC53EC-E263-438E-AC2D-0E399E67510B,38CABB77-AF83-4DFE-AA45-07BFEF932051";//NOSONAR
      ...
      for (String elrId : elrIdArr) {
        ...
        for (MessageStatus messageStatus : msgStatusList) {
          String status = "";
          if (messageStatus.getNbsInfo().getNbsInterfaceStatus() != null
              && messageStatus.getNbsInfo().getNbsInterfaceStatus().equals(SUCCESS)) {
            status =
                "Status: Success "
                    + DOUBLR_LINE_BREAK
                    + NBS_INTERFACE_ID
                    + ": "
                    + messageStatus.getNbsInfo().getNbsInterfaceId()
                    + DOUBLR_LINE_BREAK
                    + ELR_ID
                    + ": "
                    + elrId
                    + EMPTY_LINE;
          } else if (messageStatus.getNbsInfo().getNbsInterfaceStatus() != null
              && messageStatus.getNbsInfo().getNbsInterfaceStatus().equals(FAILURE)) {
            StringBuilder activityLogSb = new StringBuilder();
            activityLogSb.append("Status: Failure ");
            activityLogSb
                .append(DOUBLR_LINE_BREAK)
                .append(NBS_INTERFACE_ID)
                .append(": ")
                .append(messageStatus.getNbsInfo().getNbsInterfaceId());
            activityLogSb.append(DOUBLR_LINE_BREAK).append(ELR_ID).append(": ").append(elrId);
            List<EdxActivityDetailLog> edxActivityLogList =
                messageStatus.getEdxLogStatus().getEdxActivityDetailLogList();
            EdxActivityLog edxActivityLogParent =
                messageStatus.getEdxLogStatus().getEdxActivityLog();
            for (EdxActivityDetailLog edxActivityLogStatus : edxActivityLogList) {
              ...
              String activityLog =
                  "\n\nRecordType: "
                      + edxActivityLogStatus.getRecordType()
                      + " \nLog Type: "
                      + edxActivityLogStatus.getLogType()
                      + " \nLog Comment: "
                      + logComment
                      + " \nRecord Status Time: "
                      + edxActivityLogParent.getRecordStatusTime();
            ...
            status = activityLogSb.toString();
              ...
              && messageStatus
                  .getNbsInfo()
                  .getNbsInterfacePipeLineStatus()
                  .equals(MSG_STATUS_FAILED)) {
            status =
                STATUS_VALIDATION_ERROR
                    + " \n"
                    + ERROR_MESSAGE
                    + ": "
                    + messageStatus.getValidatedInfo().getDltInfo().getDltShortTrace()
                    + " \n"
                    + CREATED_ON
                    + ": "
                    + messageStatus.getValidatedInfo().getDltInfo().getDltCreatedOn()
                    + " \n"
                    + ELR_ID
                    + ": "
                    + elrId
            ...
            status =
                STATUS_VALIDATION_ERROR
                    ...
                    + ERROR_MESSAGE
                    + ": "
                    + messageStatus.getRawInfo().getDltInfo().getDltShortTrace()
                    + " \n"
                    + CREATED_ON
                    + ": "
                    + messageStatus.getRawInfo().getDltInfo().getDltCreatedOn()
                    + " \n"
                    + ELR_ID
                    + ": "
                    + elrId

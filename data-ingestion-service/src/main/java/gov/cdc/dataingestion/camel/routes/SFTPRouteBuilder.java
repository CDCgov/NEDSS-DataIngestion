...
public class SFTPRouteBuilder extends RouteBuilder {
  ...
  public void configure() throws Exception {
    ...
    String fileTypeValidationCondition =
        "${file:name.ext.single} in ${variable.validFileExtns} && ${bodyAs(String).trim.length} != '0'";
    ...
    for (String path : ftpPaths) {
      ...
      String sftpUnProcessedUri =
          sftpUnProcessed.setPath(sftpPath + "/unprocessed").build().toString();
      ...
      // Download the file from sftp server.If the file is zip, it will be moved into
      // files/tempZipFileDir directory.
      // If it's a text file, it will be moved to the folder files/tempTextFileDir, where all the
      // files are temporarily stored.
      from(sftpServer)
          .routeId("sftpRouteId_" + i)
          .log("The file from sftpRouteId: ${file:name} and file extn: ${file:name.ext.single}")
          .setVariable(VAR_VALID_FILE_EXTNS)
          .constant(validFileExtns)
          .choice()
      ...
      from(routeMoveToUnprocessed + "?delete=true")
          .routeId("moveToUnProcessRouteId_" + i)
      ...
      from(routeProcessingStatus)
          ...
          .setHeader(
              Exchange.FILE_NAME,
              simple("${date:now:yyyyMMddHHmmss}-success-${file:onlyname.noext.single}.log"))
          .to(sftpProcessedUri)
          .when(
              simple(
                  "${bodyAs(String).startsWith('Status: Failure')} == 'true' || ${bodyAs(String).startsWith('Status: Validation Error')} == 'true'"))
          .log("Completed - the status is available in the processed folder")
          .setBody(simple("${body}"))
          .setHeader(
              Exchange.FILE_NAME,
              simple("${date:now:yyyyMMddHHmmss}-failure-${file:onlyname.noext.single}.log"))
          .to(sftpProcessedUri)
          .otherwise()
  ...
  /**
   * Make a list of file extensions with lowercase and uppercase.And remove the '.' from the file
   * extn.
   *

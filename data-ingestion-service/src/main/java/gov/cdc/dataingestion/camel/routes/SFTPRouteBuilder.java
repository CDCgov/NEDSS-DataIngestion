            ...
            from(routeProcessingStatus)
                    ...
                    .choice()
                        .when(simple("${bodyAs(String).startsWith('Status: Success')} == 'true'"))
                            .log("Complete - status is available in the processed folder")
                            .setBody(simple("${body}"))
                            .setHeader(Exchange.FILE_NAME, simple("${date:now:yyyyMMddHHmmss}-success-${file:onlyname.noext}.log"))
                            .to(sftpProcessedUri)
                        .when(simple("${bodyAs(String).startsWith('Status: Failure')} == 'true' || ${bodyAs(String).startsWith('Status: Validation Error')} == 'true'"))
                            .log("Complete - status is available in the processed folder")
                            .setBody(simple("${body}"))
                            .setHeader(Exchange.FILE_NAME, simple("${date:now:yyyyMMddHHmmss}-failure-${file:onlyname.noext}.log"))
                            .to(sftpProcessedUri)
                        .otherwise()
                            .log("Checking the status until it is available..seda:updateStatus----${body}")
                            .to(routeProcessingStatus)
                    .endChoice()
                    .end();
        }

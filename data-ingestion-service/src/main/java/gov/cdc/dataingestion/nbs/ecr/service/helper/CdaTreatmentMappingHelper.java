...
public class CdaTreatmentMappingHelper implements ICdaTreatmentMappingHelper {
  ...
  public CdaTreatmentMapper mapToTreatmentTop(
    ...
    if (input.getMsgTreatments() != null && !input.getMsgTreatments().isEmpty()) {
      for (int i = 0; i < input.getMsgTreatments().size(); i++) {
        ...
        clinicalDocument
            ...
            .getEntryArray(cTreatment)
            .getSubstanceAdministration()
            .setMoodCode(XDocumentSubstanceMood.EVN);
        clinicalDocument
            .getComponent()
            .getStructuredBody()
            .getComponentArray(c)
        ...
        var o2 =
            clinicalDocument
                ...
                .getSection()
                .getText();
        CdaTreatmentAdministrationMapper mappedVal =
            mapToTreatment(input.getMsgTreatments().get(0), o1, o2);
        clinicalDocument
            .getComponent()
            .getStructuredBody()
            .getComponentArray(c)
            .getSection()
            .getEntryArray(cTreatment)
            .setSubstanceAdministration(mappedVal.getAdministration());
        clinicalDocument
            .getComponent()
            .getStructuredBody()
            .getComponentArray(c)
            .getSection()
            .setText(mappedVal.getText());
  ...
  private void mapToTreatmentTopHasNoCounter(POCDMT000040ClinicalDocument1 clinicalDocument, int c)
      throws EcrCdaXmlException {
    clinicalDocument
        .getComponent()
        .getStructuredBody()
    ...
    clinicalDocument
        ...
        .getSection()
        .getCode()
        .setDisplayName("Treatment Information");
    ...
    clinicalDocument
        ...
        .getSection()
        .getTitle()
        .set(cdaMapHelper.mapToPCData("TREATMENT INFORMATION"));
  ...
  private TreatmentDocument mapToTreatmentTopDocCheck(
      POCDMT000040ClinicalDocument1 clinicalDocument) {
  ...
  private CdaTreatmentAdministrationMapper mapToTreatment(
      EcrSelectedTreatment input, POCDMT000040SubstanceAdministration output, StrucDocText list)
      throws EcrCdaXmlException {
    String treatmentUid = "";
    ...
    for (Map.Entry<String, Object> entry : input.getMsgTreatment().getDataMap().entrySet()) {
      ...
      var treatmentField = mapToTreatmentFieldCheck(input, output, name, value, param);
    ...
    list = mapToTreatmentCustomTreat(customTreatment, list);

    mapToTreatmentTreatName(output, treatmentName, customTreatment, treatmentNameQuestion);
    mapToTreatmentTreatDt(output, trtTreatmentDt, trtDurationAmt, trtDurationUnitCd);
    mapToTreatmentTreatFrequency(output, trtFrequencyAmtCd);

    int performerCounter = 0;

    if (!input.getMsgTreatmentOrganizations().isEmpty()
        || !input.getMsgTreatmentProviders().isEmpty()) {
      var model = mapToTreatmentProviderAndParticipant(output, input, performerCounter);
  ...
  private TreatmentProviderAndParticipant mapToTreatmentProviderAndParticipant(
      POCDMT000040SubstanceAdministration output, EcrSelectedTreatment input, int performerCounter)
      throws EcrCdaXmlException {
    for (int i = 0; i < input.getMsgTreatmentOrganizations().size(); i++) {
      ...
      output
          .getParticipantArray(c)
          .getParticipantRole()
          .getIdArray(0)
          .setAssigningAuthorityName("LR_ORG");
      performerCounter++;
    }
  ...
  private void mapToTreatmentTreatFrequency(
      POCDMT000040SubstanceAdministration output, String trtFrequencyAmtCd) {
  ...
  private void mapToTreatmentTreatDt(
    ...
    if (!trtTreatmentDt.isEmpty()) {
      ...
      cursor.insertAttributeWithValue(
          VALUE_NAME, cdaMapHelper.mapToTsType(trtTreatmentDt).getValue());

      if (trtDurationAmt != null
          && !trtDurationAmt.isEmpty()
          && trtDurationUnitCd != null
          && !trtDurationUnitCd.isEmpty()) {
        cursor.toEndDoc();
  ...
  private void mapToTreatmentTreatName(
      POCDMT000040SubstanceAdministration output,
      String treatmentName,
      String customTreatment,
      String treatmentNameQuestion)
    ...
    if (!treatmentName.isEmpty()) {
      if (output.getConsumable() == null) {
        output
            ...
            .addNewManufacturedLabeledDrug()
            .addNewCode();
      }
      var ce = this.cdaMapHelper.mapToCEAnswerType(treatmentName, treatmentNameQuestion);
      ...
      output
          ...
          .getManufacturedLabeledDrug()
          .getName()
          .set(cdaMapHelper.mapToCData(customTreatment));
    }
  }

  private StrucDocText mapToTreatmentCustomTreat(String customTreatment, StrucDocText list)
      throws EcrCdaXmlException {
    if (!customTreatment.isEmpty()) {
      ...
      list.getListArray(c)
          .getCaption()
          .set(cdaMapHelper.mapToCData("CDA Treatment Information Section"));
    }

    return list;
  }

  private void mapToTreatmentFieldCheckDoseAmt(
  ...
  private void mapToTreatmentFieldCheckLocalId(
      EcrSelectedTreatment input, POCDMT000040SubstanceAdministration output) {
    int c = 0;
  ...
  private TreatmentField mapToTreatmentFieldCheckHelper1(
      String name, String value, EcrSelectedTreatment input, TreatmentField param) {
    ...
    if (name.equals("trtCustomTreatmentTxt")
        && value != null
        && input.getMsgTreatment().getTrtCustomTreatmentTxt() != null
        && !input.getMsgTreatment().getTrtCustomTreatmentTxt().isEmpty()) {
      customTreatment = input.getMsgTreatment().getTrtCustomTreatmentTxt();
    }
    if (name.equals("trtDurationAmt")
        && value != null
        && input.getMsgTreatment().getTrtDurationAmt() != null) {
      trtDurationAmt = input.getMsgTreatment().getTrtDurationAmt().toString();
    }
    if (name.equals("trtDurationUnitCd")
        && value != null
        && input.getMsgTreatment().getTrtDurationUnitCd() != null
        && !input.getMsgTreatment().getTrtDurationUnitCd().isEmpty()) {
  ...
  private TreatmentField mapToTreatmentFieldCheck(
    ...
    if (name.equals("trtTreatmentDt")
        && value != null
        && input.getMsgTreatment().getTrtTreatmentDt() != null) {
      trtTreatmentDt = input.getMsgTreatment().getTrtTreatmentDt().toString();
    }
    if (name.equals("trtFrequencyAmtCd")
        && value != null
        && input.getMsgTreatment().getTrtFrequencyAmtCd() != null
        && !input.getMsgTreatment().getTrtFrequencyAmtCd().isEmpty()) {
      trtFrequencyAmtCd = input.getMsgTreatment().getTrtFrequencyAmtCd();
    }
    if (name.equals("trtDosageUnitCd")
        && value != null
        && input.getMsgTreatment().getTrtDosageUnitCd() != null
        && !input.getMsgTreatment().getTrtDosageUnitCd().isEmpty()) {
      trtDosageUnitCd = input.getMsgTreatment().getTrtDosageUnitCd();
      output.getDoseQuantity().setUnit(trtDosageUnitCd);
    }
    if (name.equals("trtDosageAmt")
        && value != null
        && input.getMsgTreatment().getTrtDosageAmt() != null) {
      mapToTreatmentFieldCheckDoseAmt(input, output);
    }
    if (name.equals("trtDrugCd")
        && value != null
        && input.getMsgTreatment().getTrtDrugCd() != null
        && !input.getMsgTreatment().getTrtDrugCd().isEmpty()) {
      treatmentNameQuestion = this.cdaMapHelper.mapToQuestionId("TRT_DRUG_CD");
      treatmentName = input.getMsgTreatment().getTrtDrugCd();
    }
    if (name.equals("trtLocalId")
        && value != null
        && input.getMsgTreatment().getTrtLocalId() != null
        && !input.getMsgTreatment().getTrtLocalId().isEmpty()) {
      mapToTreatmentFieldCheckLocalId(input, output);
      treatmentUid = input.getMsgTreatment().getTrtLocalId();
    }

    mapToTreatmentFieldCheckHelper1(name, value, input, param);

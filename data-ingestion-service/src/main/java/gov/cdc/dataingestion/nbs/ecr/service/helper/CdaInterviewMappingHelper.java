...
public class CdaInterviewMappingHelper implements ICdaInterviewMappingHelper {
      ...
      for (int i = 0; i < input.getMsgInterviews().size(); i++) {
        ...
        if (interviewCounter < 1) {
          ...
          clinicalDocument
              ...
              .getSection()
              .getCode()
              .setCodeSystem(CLINICAL_CODE_SYSTEM);
          clinicalDocument
              .getComponent()
              .getStructuredBody()
              .getComponentArray(c)
              .getSection()
              .getCode()
              .setCodeSystemName(CLINICAL_CODE_SYSTEM_NAME);
          clinicalDocument
              .getComponent()
              .getStructuredBody()
              .getComponentArray(c)
          ...
          clinicalDocument
              ...
              .getSection()
              .getTitle()
              .set(cdaMapHelper.mapToPCData("INTERVIEW SECTION"));
        }

        POCDMT000040Component3 ot =
            clinicalDocument.getComponent().getStructuredBody().getComponentArray(c);
  ...
  private InterviewTopField mapToInterviewTopFieldCheck(
      POCDMT000040ClinicalDocument1 clinicalDocument) {
    var struct = this.cdaMapHelper.mapToStructureBodyCheck(clinicalDocument);

    InterviewTopField model = new InterviewTopField();
    model.setClinicalDocument(struct.getClinicalDocument());
    model.setC(struct.getC());
    return model;
  }

  private POCDMT000040Component3 mapToInterview(EcrSelectedInterview in, POCDMT000040Component3 out)
      throws EcrCdaXmlException {
    ...
    out.getSection()
        .getEntryArray(sectionEntryCounter)
        .getEncounter()
        .setMoodCode(XDocumentEncounterMood.EVN);
    ...
    if (in.getMsgInterview().getDataMap() == null
        || in.getMsgInterview().getDataMap().size() == 0) {
    ...
    for (Map.Entry<String, Object> entry : in.getMsgInterview().getDataMap().entrySet()) {
      ...
      var interviewFieldP1 =
          mapToInterviewFieldCheckP1(in, out, name, sectionEntryCounter, entryCounter);
    ...
    if (!in.getMsgInterviewProviders().isEmpty()
        || !in.getMsgInterviewAnswers().isEmpty()
        || !in.getMsgInterviewAnswerRepeats().isEmpty()) {
      mapToInterviewProvider(in, out, sectionCounter, providerRoleCounter, sectionEntryCounter);

      var interviewAns = mapToInterviewAnswer(in, out, sectionCounter, entryCounter);
      ...
      for (int i = 0; i < in.getMsgInterviewAnswerRepeats().size(); i++) {
        ...
        var mapped =
            mapToInterviewMultiSelectObservation(
                in.getMsgInterviewAnswerRepeats().get(i),
                answerGroupCounter,
  ...
  private InterviewAnswer mapToInterviewAnswer(
      EcrSelectedInterview in, POCDMT000040Component3 out, int sectionCounter, int entryCounter)
      throws EcrCdaXmlException {
    for (int i = 0; i < in.getMsgInterviewAnswers().size(); i++) {
      var element = out.getSection().getEntryArray(sectionCounter).getEncounter();
      var ot = mapToInterviewObservation(in.getMsgInterviewAnswers().get(i), entryCounter, element);
  ...
  private void mapToInterviewProvider(
      EcrSelectedInterview in,
      POCDMT000040Component3 out,
      int sectionCounter,
      int providerRoleCounter,
      int sectionEntryCounter)
      throws EcrCdaXmlException {
    for (int i = 0; i < in.getMsgInterviewProviders().size(); i++) {
      if (out.getSection().getEntryArray(sectionCounter).getEncounter().getParticipantArray().length
          == 0) {
        out.getSection()
            .getEntryArray(sectionCounter)
            .getEncounter()
            .addNewParticipant()
      ...
      var element =
          out.getSection()
              .getEntryArray(sectionCounter)
              .getEncounter()
              .getParticipantArray(providerRoleCounter);

      POCDMT000040Participant2 ot =
          this.cdaMapHelper.mapToPSN(in.getMsgInterviewProviders().get(i), element);

      out.getSection()
          .getEntryArray(sectionCounter)
          .getEncounter()
          .setParticipantArray(providerRoleCounter, ot);
      var element2 =
          out.getSection()
              .getEntryArray(sectionEntryCounter)
              .getEncounter()
  ...
  private InterviewField mapToInterviewFieldCheckP1(
      EcrSelectedInterview in,
      POCDMT000040Component3 out,
      String name,
      int sectionEntryCounter,
    ...
    if (validateInterviewGeneric(name, in)) {
    ...
    } else if (name.equals("ixsLocalId")
        && in.getMsgInterview().getIxsLocalId() != null
        && !in.getMsgInterview().getIxsLocalId().isEmpty()) {
      mapToInterviewFieldCheckP1IxsLocalId(out, in, sectionEntryCounter);
    } else if (name.equals("ixsStatusCd")
        && in.getMsgInterview().getIxsStatusCd() != null
        && !in.getMsgInterview().getIxsStatusCd().isEmpty()) {
    ...
    } else if (name.equals("ixsIntervieweeRoleCd")
        && in.getMsgInterview().getIxsIntervieweeRoleCd() != null
        && !in.getMsgInterview().getIxsIntervieweeRoleCd().isEmpty()) {
      String questionCode = this.cdaMapHelper.mapToQuestionId("IXS_INTERVIEWEE_ROLE_CD");

      var model = mapInterviewDuplicateLoc(out, sectionEntryCounter);
      int c = model.getC();
      out = model.getOut();
      var obs = model.getObs();

      this.cdaMapHelper.mapToObservation(
          questionCode, in.getMsgInterview().getIxsIntervieweeRoleCd(), obs);
      out.getSection()
          .getEntryArray(sectionEntryCounter)
          .getEncounter()
          .getEntryRelationshipArray(c)
          .setObservation(obs);
      ...
      var obs = model.getObs();
      this.cdaMapHelper.mapToObservation(
          questionCode, in.getMsgInterview().getIxsInterviewTypeCd(), obs);
      out.getSection()
          .getEntryArray(sectionEntryCounter)
          .getEncounter()
          .getEntryRelationshipArray(c)
      ...
      this.cdaMapHelper.mapToObservation(
          questionCode, in.getMsgInterview().getIxsInterviewLocCd(), obs);
      out.getSection()
          .getEntryArray(sectionEntryCounter)
          .getEncounter()
          .getEntryRelationshipArray(c)
  ...
  private InterviewEntryRelation mapInterviewDuplicateLoc(
      POCDMT000040Component3 out, int sectionEntryCounter) {
    ...
    InterviewEntryRelation model = new InterviewEntryRelation();
    model.setC(c);
    model.setOut(out);
    model.setObs(obs);
    return model;
  ...
  private boolean validateInterviewType(String name, EcrSelectedInterview in) {
    return name.equals("ixsInterviewTypeCd")
        && in.getMsgInterview().getIxsInterviewTypeCd() != null
        && !in.getMsgInterview().getIxsInterviewTypeCd().isEmpty();
  }

  private boolean validateInterviewLoc(String name, EcrSelectedInterview in) {
    return name.equals("ixsInterviewLocCd")
        && in.getMsgInterview().getIxsInterviewLocCd() != null
        && !in.getMsgInterview().getIxsInterviewLocCd().isEmpty();
  }

  private InterviewRole mapToInterviewFieldCheckP1Loc(
      POCDMT000040Component3 out, int sectionEntryCounter) {
    int c = 0;
    if (out.getSection()
            .getEntryArray(sectionEntryCounter)
            .getEncounter()
            .getEntryRelationshipArray()
            .length
        == 0) {
      out.getSection()
          .getEntryArray(sectionEntryCounter)
  ...
  private void mapToInterviewFieldCheckP1InterviewDt(
      POCDMT000040Component3 out, EcrSelectedInterview in, int sectionEntryCounter)
      throws EcrCdaXmlException {
    var ts = cdaMapHelper.mapToTsType(in.getMsgInterview().getIxsInterviewDt().toString());
    if (out.getSection().getEntryArray(sectionEntryCounter).getEncounter().getEffectiveTime()
        == null) {
  ...
  private void mapToInterviewFieldCheckP1StatusCd(
      POCDMT000040Component3 out, EcrSelectedInterview in, int sectionEntryCounter) {
    if (out.getSection().getEntryArray(sectionEntryCounter).getEncounter().getStatusCode()
        == null) {
    ...
    out.getSection()
        ...
        .getEncounter()
        .getStatusCode()
        .setCode(in.getMsgInterview().getIxsStatusCd());
  }

  private void mapToInterviewFieldCheckP1IxsLocalId(
      POCDMT000040Component3 out, EcrSelectedInterview in, int sectionEntryCounter) {
    var model = checkEntryEncounter(out, sectionEntryCounter);
    int c = model.getC();
    out = model.getOut();
    out.getSection()
        .getEntryArray(sectionEntryCounter)
        .getEncounter()
        .getIdArray(c)
        .setExtension(in.getMsgInterview().getIxsLocalId());
  }

  private void mapToInterviewFieldCheckP1GenericCheck(
      POCDMT000040Component3 out, EcrSelectedInterview in, int sectionEntryCounter) {
    var model = checkEntryEncounter(out, sectionEntryCounter);
    int c = model.getC();
    out = model.getOut();
    out.getSection()
        ...
        .getEncounter()
        .getIdArray(c)
        .setExtension(in.getMsgInterview().getMsgContainerUid().toString());
  }

  private InterviewEntryEncounter checkEntryEncounter(
      POCDMT000040Component3 out, int sectionEntryCounter) {
    int c = 0;
    if (out.getSection().getEntryArray(sectionEntryCounter).getEncounter().getIdArray().length
        == 0) {
    ...
    InterviewEntryEncounter model = new InterviewEntryEncounter();
    model.setC(c);
    model.setOut(out);
    return model;
  }

  private InterviewAnswerMultiMapper mapToInterviewMultiSelectObservation(
    ...
    for (Map.Entry<String, Object> entry : in.getDataMap().entrySet()) {
      ...
      var interviewMultiObsP1 = mapToInterviewMultiSelectObservationFieldP1(in, out, name, param);
      ...
      var param2 =
          new InterviewMultiObs(
              dataType, sectionCounter, componentCounter, seqNbr, questionIdentifier);
      var interviewMultiObsP2 =
          mapToInterviewMultiSelectObservationFieldP2(in, out, name, value, param2);
      ...
      var param3 =
          new InterviewMultiObs(questionIdentifier, questionId, sectionCounter, componentCounter);
      var interviewMultiObsP3 =
          mapToInterviewMultiSelectObservationFieldP3(out, name, value, param3);
  ...
  private InterviewMultiObs mapToInterviewMultiSelectObservationFieldP3(
      POCDMT000040Encounter out, String name, String value, InterviewMultiObs param) {
    ...
    switch (name) {
      case COL_QUES_IDENTIFIER -> {
        ...
        if (out.getEntryRelationshipArray(sectionCounter)
                .getOrganizer()
                .getComponentArray(componentCounter)
                .getObservation()
                .getCode()
            == null) {
          out.getEntryRelationshipArray(sectionCounter)
              .getOrganizer()
              .getComponentArray(componentCounter)
              .getObservation()
              .addNewCode();
        ...
        out.getEntryRelationshipArray(sectionCounter)
            ...
            .getComponentArray(componentCounter)
            .getObservation()
            .setMoodCode(XActMoodDocumentObservation.EVN);
        out.getEntryRelationshipArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
            .getObservation()
  ...
  private InterviewMultiObs mapToInterviewMultiSelectObservationFieldP2(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Encounter out,
      String name,
      String value,
    ...
    if (dataType.equalsIgnoreCase(DATA_TYPE_CODE) || dataType.equalsIgnoreCase("CODED_COUNTY")) {
      mapToInterviewMultiSelectObservationFieldP2CodedCounty(
          name, in, out, sectionCounter, componentCounter, seqNbr);
    } else if ((dataType.equalsIgnoreCase("TEXT") || dataType.equalsIgnoreCase(DATA_TYPE_NUMERIC))
        && name.equals(COL_ANS_TXT)) {
      mapToInterviewMultiSelectObservationFieldP2Text(
          out, value, questionIdentifier, componentCounter, sectionCounter);
    } else if (dataType.equalsIgnoreCase("DATE")) {
      mapToInterviewMultiSelectObservationFieldP2Date(
          name, in, out, value, sectionCounter, componentCounter);
    }
  ...
  private void mapToInterviewMultiSelectObservationFieldP2Date(
      String name,
      EcrMsgCaseAnswerDto in,
      POCDMT000040Encounter out,
      String value,
  ...
  private void mapToInterviewMultiSelectObservationFieldP2Text(
      POCDMT000040Encounter out,
      String value,
      String questionIdentifier,
      int componentCounter,
    ...
    if (questionIdentifier.equalsIgnoreCase("NBS243")
      ...
      var element =
          out.getEntryRelationshipArray(sectionCounter)
              ...
              .getComponentArray(componentCounter)
              .getObservation();
      var ot = cdaMapHelper.mapToObservationPlace(value, element);
      out.getEntryRelationshipArray(sectionCounter)
          .getOrganizer()
          .getComponentArray(componentCounter)
          .setObservation((POCDMT000040Observation) ot);
  ...
  private void mapToInterviewMultiSelectObservationFieldP2CodedCounty(
      String name,
      EcrMsgCaseAnswerDto in,
      POCDMT000040Encounter out,
      int sectionCounter,
    ...
    } else if (name.equals(COL_ANS_CODE_SYSTEM_DESC_TXT)
        && !in.getAnsCodeSystemDescTxt().isEmpty()) {
    ...
    out.getEntryRelationshipArray(sectionCounter)
        ...
        .getComponentArray(componentCounter)
        .getObservation()
        .getValueArray(seqNbr)
        .set(ce);
  }

  private boolean validateMultiSelectObservationFieldP2AnsCodeDesc(
      String name, EcrMsgCaseAnswerDto in) {
    return name.equals(COL_ANS_TO_CODE_SYSTEM_DESC_TXT)
        && !in.getAnsToCodeSystemDescTxt().isEmpty();
  }

  private boolean validateMultiSelectObservationFieldP2AnsDisplayNm(
      String name, EcrMsgCaseAnswerDto in) {
    return name.equals(COL_ANS_TO_DISPLAY_NM) && !in.getAnsToDisplayNm().isEmpty();
  }

  private InterviewMultiObs mapToInterviewMultiSelectObservationFieldP1(
      EcrMsgCaseAnswerDto in, POCDMT000040Encounter out, String name, InterviewMultiObs param) {
    ...
    if (name.equals(COL_QUES_GROUP_SEQ_NBR) && !in.getQuestionGroupSeqNbr().isEmpty()) {
      ...
      if ((answerGroupSeqNbr == answerGroupCounter)
        ...
        componentCounter =
            out.getEntryRelationshipArray(sectionCounter).getOrganizer().getComponentArray().length;
      } else {
        ...
        out.getEntryRelationshipArray(sectionCounter)
            .getOrganizer()
            .getCode()
            .setCode(String.valueOf(questionGroupSeqNbr));
        out.getEntryRelationshipArray(sectionCounter)
            .getOrganizer()
            .getCode()
            .setCode("1234567RPT");
        ...
        out.getEntryRelationshipArray(sectionCounter)
            .getOrganizer()
            .getCode()
            .setDisplayName("Generic Repeating Questions Section");

        out.getEntryRelationshipArray(sectionCounter)
            .setTypeCode(XActRelationshipEntryRelationship.COMP);
        out.getEntryRelationshipArray(sectionCounter)
            .getOrganizer()
            .setClassCode(XActClassDocumentEntryOrganizer.CLUSTER);
  ...
  private void mapToInterviewMultiSelectObservationFieldP1DocCheckOrg(
      int sectionCounter, POCDMT000040Encounter out) {
    if (out.getEntryRelationshipArray(sectionCounter).getOrganizer() == null) {
  ...
  private InterviewAnswerMapper mapToInterviewObservation(
      EcrMsgCaseAnswerDto in, int counter, POCDMT000040Encounter out) throws EcrCdaXmlException {
    ...
    for (Map.Entry<String, Object> entry : in.getDataMap().entrySet()) {
      ...
      InterviewObs param =
          new InterviewObs(questionGroupSeqNbr, answerGroupSeqNbr, dataType, sequenceNbr);
      var invObsP1 = mapToInterviewObservationFieldP1(in, name, param);
      ...
      var invObsP2 = mapToInterviewObservationFieldP2(in, out, name, value, param2);
      out = invObsP2.getOut();
      ...
      var invObsP3 = mapToInterviewObservationFieldP3(in, out, name, value, param3);
      out = invObsP3.getOut();
  ...
  private InterviewObs mapToInterviewObservationFieldP3(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Encounter out,
      String name,
      String value,
    ...
    if (name.equals(COL_QUES_IDENTIFIER) && !in.getQuestionIdentifier().isEmpty()) {
      if (in.getQuestionIdentifier().equals(value)) {
        ...
        out.getEntryRelationshipArray(counter)
            .getObservation()
            .setMoodCode(XActMoodDocumentObservation.EVN);
        out.getEntryRelationshipArray(counter)
            .getObservation()
            .getCode()
            .setCode(in.getQuestionIdentifier());
      }
    } else if (name.equals(COL_QUES_CODE_SYSTEM_CD) && !in.getQuesCodeSystemCd().isEmpty()) {
      out.getEntryRelationshipArray(counter)
          .getObservation()
          .getCode()
          .setCodeSystem(in.getQuesCodeSystemCd());
    } else if (name.equals(COL_QUES_CODE_SYSTEM_DESC_TXT)
        && !in.getQuesCodeSystemDescTxt().isEmpty()) {
      out.getEntryRelationshipArray(counter)
          .getObservation()
          .getCode()
          .setCodeSystemName(in.getQuesCodeSystemDescTxt());
    } else if (name.equals(COL_QUES_DISPLAY_TXT) && !in.getQuesDisplayTxt().isEmpty()) {
      out.getEntryRelationshipArray(counter)
          .getObservation()
          .getCode()
          .setDisplayName(in.getQuesDisplayTxt());
    }
  ...
  private InterviewObs mapToInterviewObservationFieldP2(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Encounter out,
      String name,
      String value,
    ...
    if (dataType.equalsIgnoreCase(DATA_TYPE_CODE) || dataType.equalsIgnoreCase(COUNTY)) {
      mapToInterviewObservationFieldP2County(name, out, in, counter, sequenceNbr);

    } else if (dataType.equals("TEXT") || dataType.equals(DATA_TYPE_NUMERIC)) {
      if (name.equals(COL_ANS_TXT) && !in.getAnswerTxt().isEmpty()) {
      ...
      mapToInterviewObservationFieldP2Date(name, out, in, counter, value);
    }
  ...
  private void mapToInterviewObservationFieldP2Date(
      String name, POCDMT000040Encounter out, EcrMsgCaseAnswerDto in, int counter, String value)
      throws EcrCdaXmlException {
    if (name.equals(COL_ANS_TXT) && !in.getAnswerTxt().isEmpty()) {
      ...
      cursor.setAttributeText(
          new QName("name"), value); // This is an assumption based on the original code
  ...
  private void mapToInterviewObservationFieldP2County(
      String name,
      POCDMT000040Encounter out,
      EcrMsgCaseAnswerDto in,
      int counter,
    ...
    if (name.equals(COL_ANS_TXT) && !in.getAnswerTxt().isEmpty()) {
      ...
      ce.setCodeSystem(in.getAnsToCodeSystemCd());
    } else if (name.equals(COL_ANS_TO_CODE_SYSTEM_DESC_TXT)
        && !in.getAnsToCodeSystemDescTxt().isEmpty()) {
      ce.setCodeSystemName(in.getAnsToCodeSystemDescTxt());
  ...
  private boolean validateMapToInterviewObservationFieldP2CountyAnsDisplayNm(
      String name, EcrMsgCaseAnswerDto in) {
    return name.equals(COL_ANS_TO_DISPLAY_NM) && !in.getAnsToDisplayNm().isEmpty();
  }

  private InterviewObs mapToInterviewObservationFieldP1(
      EcrMsgCaseAnswerDto in, String name, InterviewObs param) {
    int questionGroupSeqNbr = param.getQuestionGroupSeqNbr();
    int answerGroupSeqNbr = param.getAnswerGroupSeqNbr();
    String dataType = param.getDataType();
    int sequenceNbr = param.getSequenceNbr();

...
public class CdaOrgMappingHelper implements ICdaOrgMappingHelper {
  ...
  public CdaOrganizationMapper mapToOrganizationTop(
    ...
    if (input.getMsgOrganizations() != null && !input.getMsgOrganizations().isEmpty()) {
      for (int i = 0; i < input.getMsgOrganizations().size(); i++) {
        ...
        mapToOrganizationTopFieldMap(clinicalDocument, performerComponentCounter, input, i);
      }
    }
  ...
  private void mapToOrganizationTopFieldMap(
    ...
    var model =
        this.cdaMapHelper.mapOrgPlaceDocCommonField(clinicalDocument, performerComponentCounter);
    ...
    POCDMT000040Participant2 output =
        this.cdaMapHelper.mapToORG(input.getMsgOrganizations().get(i), out);

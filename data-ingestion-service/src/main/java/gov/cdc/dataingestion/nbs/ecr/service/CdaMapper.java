...
public class CdaMapper implements ICdaMapper {
  ...
  public String tranformSelectedEcrToCDAXml(EcrSelectedRecord input) throws EcrCdaXmlException {
    ...
    POCDMT000040ClinicalDocument1 clinicalDocument =
        POCDMT000040ClinicalDocument1.Factory.newInstance();

    var containerModel = mapParentContainer(clinicalDocument, input, inv168);
    ...
    var pat =
        this.patientMappingHelper.mapToPatient(
            input, clinicalDocument, patientComponentCounter, inv168);
    clinicalDocument = pat.getClinicalDocument();
    inv168 = pat.getInv168();

    var ecrCase =
        caseMappingHelper.mapToCaseTop(
            input,
            clinicalDocument,
            componentCounter,
            clinicalCounter,
    ...
    var ecrXmlAnswer =
        xmlAnswerMappingHelper.mapToXmlAnswerTop(input, clinicalDocument, componentCounter);
    ...
    var ecrProvider =
        this.providerMappingHelper.mapToProviderTop(
            input,
            clinicalDocument.getComponent().getStructuredBody().getComponentArray(c).getSection(),
            inv168,
    ...
    clinicalDocument
        ...
        .getStructuredBody()
        .getComponentArray(c)
        .setSection(ecrProvider.getClinicalSection());
    ...
    var ecrOrganization =
        this.orgMappingHelper.mapToOrganizationTop(
            input,
            clinicalDocument.getComponent().getStructuredBody().getComponentArray(c).getSection(),
            performerComponentCounter,
            componentCounter,
    ...
    POCDMT000040Section interestedPartyComp =
        clinicalDocument.getComponent().getStructuredBody().getComponentArray(c).getSection();
    var ecrPlace =
        this.placeMappingHelper.mapToPlaceTop(
            input,
            performerComponentCounter,
            componentCounter,
            performerSectionCounter,
    ...
    clinicalDocument
        ...
        .getStructuredBody()
        .getComponentArray(c)
        .setSection(ecrPlace.getSection());
    componentCounter = ecrPlace.getComponentCounter();

    var ecrInterview =
        this.interviewMappingHelper.mapToInterviewTop(
            input, clinicalDocument, interviewCounter, componentCounter);
    clinicalDocument = ecrInterview.getClinicalDocument();
    componentCounter = ecrInterview.getComponentCounter();

    var ecrTreatment =
        this.treatmentMappingHelper.mapToTreatmentTop(
            input, clinicalDocument, treatmentCounter, componentCounter, treatmentSectionCounter);
  ...
  private CdaContainerComp mapParentContainer(
      POCDMT000040ClinicalDocument1 clinicalDocument, EcrSelectedRecord input, String inv168)
    ...
    if (input.getMsgContainer().invLocalId() != null
        && !input.getMsgContainer().invLocalId().isEmpty()) {
      clinicalDocument.setId(II.Factory.newInstance());
      clinicalDocument.getId().setRoot(ROOT_ID);
      clinicalDocument.getId().setExtension(input.getMsgContainer().invLocalId());
      clinicalDocument.getId().setAssigningAuthorityName("LR");
      inv168 = input.getMsgContainer().invLocalId();
    }

    if (input.getMsgContainer().ongoingCase() != null
        && !input.getMsgContainer().ongoingCase().isEmpty()) {
      clinicalDocument.setSetId(II.Factory.newInstance());
      clinicalDocument.getSetId().setExtension("ONGOING_CASE");
      clinicalDocument
          .getSetId()
          .setDisplayable(input.getMsgContainer().ongoingCase().equalsIgnoreCase("yes"));
    }

    if (input.getMsgContainer().versionCtrlNbr() != null) {
      versionCtrNbr = input.getMsgContainer().versionCtrlNbr();
    }
    ...
    clinicalDocument
        .getTitle()
        .set(
            cdaMapHelper.mapToPCData("Public Health Case Report - Data from Legacy System to CDA"));

    clinicalDocument.setEffectiveTime(TS.Factory.newInstance());
    clinicalDocument
        .getEffectiveTime()
        .setValue(this.cdaMapHelper.getCurrentUtcDateTimeInCdaFormat());
  ...
  private void mapCustodian(POCDMT000040ClinicalDocument1 clinicalDocument)
      throws EcrCdaXmlException {
    int k = 0;
    ...
    clinicalDocument
        .getCustodian()
        .getAssignedCustodian()
        .setRepresentedCustodianOrganization(element);
    ...
    if (AUTHOR_ORG.getStreetAddress1() != null && !AUTHOR_ORG.getStreetAddress1().isEmpty()) {
      clinicalDocument
          ...
          .getRepresentedCustodianOrganization()
          .getAddr()
          .getStreetAddressLineArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getStreetAddress1()));
    }
    k = k + 1;
    ...
    if (AUTHOR_ORG.getStreetAddress2() != null && !AUTHOR_ORG.getStreetAddress2().isEmpty()) {
      clinicalDocument
          ...
          .getAddr()
          .getStreetAddressLineArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getStreetAddress2()));
    }
    k = 0;
    ...
    if (AUTHOR_ORG.getCity() != null && !AUTHOR_ORG.getCity().isEmpty()) {
      clinicalDocument
          ...
          .getAddr()
          .getCityArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getCity()));
    }
    ...
    if (AUTHOR_ORG.getState() != null && !AUTHOR_ORG.getState().isEmpty()) {
      clinicalDocument
          ...
          .getAddr()
          .getStateArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getState()));
    }
    ...
    if (AUTHOR_ORG.getZip() != null && !AUTHOR_ORG.getZip().isEmpty()) {
      clinicalDocument
          ...
          .getRepresentedCustodianOrganization()
          .getAddr()
          .getPostalCodeArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getZip()));
    }
    ...
    if (AUTHOR_ORG.getCountry() != null && !AUTHOR_ORG.getCountry().isEmpty()) {
      clinicalDocument
          ...
          .getAddr()
          .getCountryArray(k)
          .set(cdaMapHelper.mapToPCData(AUTHOR_ORG.getCountry()));
    }
  ...
  private void mapAuthor(POCDMT000040ClinicalDocument1 clinicalDocument) throws EcrCdaXmlException {
    ...
    clinicalDocument
        ...
        .getAssignedAuthor()
        .getIdArray(0)
        .setRoot("2.16.840.1.113883.19.5");
  ...
  private String convertXmlToString(ClinicalDocumentDocument1 clinicalDocument) {
    ...
    xmlOutput =
        xmlOutput.replaceAll("<STRING[^>]*>([^<]+)</STRING>", "$1"); // NOSONAR // remove string tag
    xmlOutput =
        xmlOutput.replaceAll(
            "\\[CDATA\\](.*?)\\[CDATA\\]", "<![CDATA[$1]]>"); // NOSONAR // replace CDATA
    // with
    // real CDATA
    xmlOutput =
        xmlOutput.replaceAll(
            "<CDATA[^>]*>(.*?)</CDATA>", "<![CDATA[$1]]>"); // NOSONAR // replace CDATA with
    // real CDATA
    xmlOutput = xmlOutput.replaceAll("<(\\w+)></\\1>", ""); // NOSONAR // remove empty <tag></tag>
    xmlOutput =
        xmlOutput.replaceAll("<STUD xmlns=\"\">STUD</STUD>", ""); // NOSONAR // remove STUD tag
    xmlOutput =
        xmlOutput.replaceAll("<stud xmlns=\"\">stud</stud>", ""); // NOSONAR // remove STUD tag
    ...
    xmlOutput =
        xmlOutput.replaceAll(
            "sdtcxmlnamespaceholder=\"" + XML_NAME_SPACE_HOLDER + "\"",
            "xmlns:sdtcxmlnamespaceholder=\"" + XML_NAME_SPACE_HOLDER + "\""); // NOSONAR
    xmlOutput =
        xmlOutput.replaceAll(
            "sdt=\"urn:hl7-org:sdtc\"", "xmlns:sdt=\"urn:hl7-org:sdtc\""); // NOSONAR
    xmlOutput =
        xmlOutput.replaceAll(
    ...
    xmlOutput =
        xmlOutput.replaceAll(
            "schemaLocation=\"" + XML_NAME_SPACE_HOLDER + " CDA_SDTC.xsd\"",
            "xsi:schemaLocation=\"" + XML_NAME_SPACE_HOLDER + " CDA_SDTC.xsd\""); // NOSONAR

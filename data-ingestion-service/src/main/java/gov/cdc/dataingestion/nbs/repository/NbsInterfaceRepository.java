...
public interface NbsInterfaceRepository extends JpaRepository<NbsInterfaceModel, Integer> {
  @Query(
      value =
          "SELECT * FROM NBS_interface WHERE nbs_interface_uid = :nbsInterfaceUid AND doc_type_cd = :docTypeCd",
      nativeQuery = true)
  Optional<NbsInterfaceModel> getNbsInterfaceByIdAndDocType(
      @Param("nbsInterfaceUid") Integer id, @Param("docTypeCd") String docType);
  ...
  @Query(
      value =
          "SELECT TOP 100 nbs_interface_uid, payload, imp_exp_ind_cd, record_status_cd, record_status_time, add_time, system_nm, \n"
              ...
              + "original_payload_RR, original_doc_type_cd_RR FROM NBS_interface",
      nativeQuery = true)

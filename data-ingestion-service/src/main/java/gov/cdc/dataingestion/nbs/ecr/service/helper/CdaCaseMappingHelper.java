...
public class CdaCaseMappingHelper implements ICdaCaseMappingHelper {
  ...
  // region CASE
  public CdaCaseMapper mapToCaseTop(
    ...
    if (!input.getMsgCases().isEmpty()) {
      ...
      for (int i = 0; i < input.getMsgCases().size(); i++) {
        if (componentCounter < 0) {
          ...
          CdaCaseComponent caseComponent =
              checkCaseStructComponentWithSectionAndIndex(clinicalDocument, c);
          ...
          clinicalDocument
              ...
              .getSection()
              .getCode()
              .setDisplayName("Clinical Information");
          clinicalDocument
              .getComponent()
              .getStructuredBody()
              .getComponentArray(c)
              .getSection()
              .getTitle()
              .set(cdaMapHelper.mapToPCData("CLINICAL INFORMATION"));
        ...
        clinicalCounter =
            clinicalDocument
                .getComponent()
                .getStructuredBody()
                .getComponentArray(componentCaseCounter)
                .getSection()
                .getEntryArray()
                .length;
        ...
        componentCounter =
            clinicalDocument.getComponent().getStructuredBody().getComponentArray().length - 1;
      }
    }
  ...
  private POCDMT000040StructuredBody mapToCase(
      int entryCounter, EcrSelectedCase caseDto, POCDMT000040StructuredBody output)
      throws EcrCdaXmlException {
    ...
    if (!caseDto.getMsgCaseParticipants().isEmpty()
        || !caseDto.getMsgCaseAnswers().isEmpty()
        || !caseDto.getMsgCaseAnswerRepeats().isEmpty()) {
      ...
      var caseParRepeatModel =
          mapCaseParticipantRepeat(
              caseDto,
              output,
              repeatComponentCounter,
  ...
  private CdaCaseField mapCaseFieldCheck(
      EcrSelectedCase caseDto,
    ...
    for (Map.Entry<String, Object> entry : caseDto.getMsgCase().getDataMap().entrySet()) {
      ...
      if (checkInvalidField(name, caseDto)) {
        ...
        if (name.equalsIgnoreCase("invConditionCd")) {
          repeats =
              (int) caseDto.getMsgCase().getInvConditionCd().chars().filter(x -> x == '^').count();
        }

        if (repeats > 1) {
          var repeatModel =
              mapCaseFieldCheckHasRepeat(output, componentCaseCounter, value, questionId);

          output = repeatModel.getOutput();
          repeats = repeatModel.getRepeats();
        } else {
          mapCaseFieldCheckHasNoRepeat(output, componentCaseCounter, value, questionId);
        }
        counter++;
      }
    }
  ...
  private CdaCaseFieldSectionCommon mapCaseFieldSectionCommon(
      POCDMT000040StructuredBody output, int componentCaseCounter) {
    int c = 0;
  ...
  private void mapCaseFieldCheckHasNoRepeat(
      POCDMT000040StructuredBody output, int componentCaseCounter, String value, String questionId)
      throws EcrCdaXmlException {

    var model = mapCaseFieldSectionCommon(output, componentCaseCounter);
    int c = model.getC();
    var element = model.getElement();
    output = model.getOutput();
    POCDMT000040Observation obs = this.cdaMapHelper.mapToObservation(questionId, value, element);
    output
        .getComponentArray(componentCaseCounter)
        .getSection()
        .getEntryArray(c)
  ...
  private CdaCaseFieldRepeat mapCaseFieldCheckHasRepeat(
      POCDMT000040StructuredBody output, int componentCaseCounter, String value, String questionId)
      throws EcrCdaXmlException {
    var modelSection = mapCaseFieldSectionCommon(output, componentCaseCounter);
    int c = modelSection.getC();
    var element = modelSection.getElement();
    output = modelSection.getOutput();

    var obs = mapTripletToObservation(value, questionId, element);
    output
        .getComponentArray(componentCaseCounter)
        .getSection()
        .getEntryArray(c)
  ...
  private CdaCaseParticipantRepeat mapCaseParticipantRepeat(
      EcrSelectedCase caseDto,
      POCDMT000040StructuredBody output,
    ...
    if (caseDto.getMsgCaseAnswerRepeats() != null) {
      for (int i = 0; i < caseDto.getMsgCaseAnswerRepeats().size(); i++) {
        ...
        var ot =
            mapToMultiSelect(
                caseDto.getMsgCaseAnswerRepeats().get(i),
                answerGroupCounter,
                questionGroupCounter,
                sectionCounter,
                out);
  ...
  private CdaCaseAnswer mapCaseAnswer(
      EcrSelectedCase caseDto,
      POCDMT000040StructuredBody output,
      int componentCaseCounter,
      int counter)
    ...
    if (caseDto.getMsgCaseAnswers() != null) {
      for (int i = 0; i < caseDto.getMsgCaseAnswers().size(); i++) {
        ...
        var res =
            mapToMessageAnswer(caseDto.getMsgCaseAnswers().get(i), oldQuestionId, counter, out);
  ...
  private CdaCaseParticipant mapCaseParticipant(
      EcrSelectedCase caseDto,
      POCDMT000040StructuredBody output,
      int componentCaseCounter,
      int counter)
    ...
    if (caseDto.getMsgCaseParticipants() != null) {
      for (int i = 0; i < caseDto.getMsgCaseParticipants().size(); i++) {
        int c = 0;
        if (output.getComponentArray(componentCaseCounter).getSection().getEntryArray().length
            == 0) {
          output.getComponentArray(componentCaseCounter).getSection().addNewEntry();
        } else {
          c = output.getComponentArray(componentCaseCounter).getSection().getEntryArray().length;
        ...
        POCDMT000040Observation out =
            mapToObsFromParticipant(caseDto.getMsgCaseParticipants().get(i), element);
        output
            .getComponentArray(componentCaseCounter)
            .getSection()
            .getEntryArray(c)
  ...
  private POCDMT000040Observation mapTripletToObservation(
      String invConditionCd, String questionId, POCDMT000040Observation output)
      throws EcrCdaXmlException {
  ...
  private POCDMT000040Observation mapToObsFromParticipant(
      EcrMsgCaseParticipantDto in, POCDMT000040Observation out) throws EcrCdaXmlException {
    String localId = "";
    String questionCode = "";
    ...
    if (in.getPartTypeCd() != null
        && !in.getPartTypeCd().isEmpty()
        && in.getPartTypeCd().contains("OrgAsReporterOfPHC")) {
      orgAsReporterOfPHCInd = in.getPartTypeCd();
    }

    if (in.getPartTypeCd() != null
        && !in.getPartTypeCd().isEmpty()
        && in.getPartTypeCd().contains("PerAsReporterOfPHC")) {
  ...
  private MessageAnswer mapToMessageAnswer(
      EcrMsgCaseAnswerDto in, String questionSeq, int counter, POCDMT000040Component3 out)
      throws EcrCdaXmlException {
    String dataType = "";
    int sequenceNbr = 0;
    ...
    for (Map.Entry<String, Object> entry : in.getDataMap().entrySet()) {
      ...
      if (in.getDataMap().containsKey(COL_SEQ_NBR) && in.getDataMap().get(COL_SEQ_NBR) != null) {
        param.setSequenceNbr(sequenceNbr);
      }
      param.setCounter(counter);

      var caseMsgAnsModel = mapToMessageAnswerFieldCheck(name, value, in, out, param, ce);
      ...
      if (name.equalsIgnoreCase(COL_QUES_IDENTIFIER)) {
        var caseAnsQuesIdentifier =
            setMessageAnswerQuestionIdentifier(in, out, questionSeq, counter, sequenceNbr);
  ...
  private CdaCaseMsgAnswer mapToMessageAnswerFieldCheck(
      String name,
      String value,
    ...
    if (name.equals(COL_DATA_TYPE) && in.getDataType() != null && !in.getDataType().isEmpty()) {
      ...
      param.setSequenceNbr(
          out.getSection()
              .getEntryArray(param.getCounter())
              .getObservation()
              .getValueArray()
              .length);
    } else if (param.getDataType().equalsIgnoreCase(DATA_TYPE_CODE)
        || param.getDataType().equalsIgnoreCase(COUNTY)) {
      setMessageAnswerArrayValue(name, in, param.getSequenceNbr(), param.getCounter(), out, ce);
    } else if (param.getDataType().equalsIgnoreCase("TEXT")
        || param.getDataType().equalsIgnoreCase(DATA_TYPE_NUMERIC)) {
  ...
  private CdaCaseAnsQuesIdentifier setMessageAnswerQuestionIdentifier(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Component3 out,
      String questionSeq,
      int counter,
  ...
  private void setMessageAnswerDate(
      String name, EcrMsgCaseAnswerDto in, POCDMT000040Component3 out, int counter, String value)
      throws EcrCdaXmlException {
  ...
  private void setMessageAnswerAnsText(
      String name, EcrMsgCaseAnswerDto in, POCDMT000040Component3 out, int counter) {
    if (name.equals(COL_ANS_TXT) && !in.getAnswerTxt().isEmpty()) {
  ...
  private void setMessageAnswerArrayValue(
      String name,
      EcrMsgCaseAnswerDto in,
      int sequenceNbr,
      int counter,
  ...
  private void setMessageAnswerArrayValueAnsDisplayNm(
      CE ce, EcrMsgCaseAnswerDto in, int counter, POCDMT000040Component3 out) {
  ...
  private MultiSelect mapToMultiSelect(
      EcrMsgCaseAnswerDto in,
      int answerGroupCounter,
      int questionGroupCounter,
      int sectionCounter,
    ...
    for (Map.Entry<String, Object> entry : in.getDataMap().entrySet()) {
      ...
      if (dataType.equalsIgnoreCase(DATA_TYPE_CODE) || dataType.equalsIgnoreCase("CODED_COUNTY")) {
        mapMultiSelectCodedCounty(name, in, out, sectionCounter, componentCounter);
      } else if ((dataType.equalsIgnoreCase("TEXT") || dataType.equalsIgnoreCase(DATA_TYPE_NUMERIC))
          && name.equals(COL_ANS_TXT)) {
        mapMultiSelectDataNumericType(
            in, out, questionIdentifier, sectionCounter, componentCounter);
      ...
      var multiSelectField = mapToMultiSelectFields(name, value, in, out, param);
  ...
  private CdaCaseMultiSelectFields mapToMultiSelectFields(
    ...
    switch (name) {
      case COL_QUES_IDENTIFIER -> {
        mapMultiSelectQuesIdentifier(
            in, out, param.getSectionCounter(), param.getComponentCounter());
        param.setQuestionIdentifier(value);
      }
      case COL_QUES_CODE_SYSTEM_CD ->
          mapMultiSelectCodeSystemQues(
              in, out, param.getSectionCounter(), param.getComponentCounter());
      case COL_QUES_CODE_SYSTEM_DESC_TXT ->
          mapMultiSelectCodeSystemDescQues(
              in, out, param.getSectionCounter(), param.getComponentCounter());
      case COL_QUES_DISPLAY_TXT ->
          mapMultiSelectDisplayQues(
              in, out, param.getSectionCounter(), param.getComponentCounter());
  ...
  private void mapMultiSelectDisplayQues(
      EcrMsgCaseAnswerDto in, POCDMT000040Section out, int sectionCounter, int componentCounter) {
    if (out.getEntryArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
            .getObservation()
        == null) {
      out.getEntryArray(sectionCounter)
          .getOrganizer()
          .getComponentArray(componentCounter)
          .addNewObservation()
          .addNewCode();
  ...
  private void mapMultiSelectCodeSystemDescQues(
      EcrMsgCaseAnswerDto in, POCDMT000040Section out, int sectionCounter, int componentCounter) {
    if (out.getEntryArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
  ...
  private void mapMultiSelectCodeSystemQues(
      EcrMsgCaseAnswerDto in, POCDMT000040Section out, int sectionCounter, int componentCounter) {
    if (out.getEntryArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
  ...
  private void mapMultiSelectQuesIdentifier(
      EcrMsgCaseAnswerDto in, POCDMT000040Section out, int sectionCounter, int componentCounter) {
    if (out.getEntryArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
  ...
  private void mapMultiSelectDate(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Section out,
      String name,
      String value,
    ...
    if (name.equals(COL_ANS_TXT)) {
      ...
      if (size == 0) {
        ...
        componentCounter =
            out.getEntryArray(sectionCounter).getOrganizer().getComponentArray().length;
        out.getEntryArray(sectionCounter).getOrganizer().addNewComponent();
      }
  ...
  private void mapMultiSelectDataNumericType(
      EcrMsgCaseAnswerDto in,
      POCDMT000040Section out,
      String questionIdentifier,
      int sectionCounter,
    ...
    if (questionIdentifier.equalsIgnoreCase("NBS243")
      ...
      var element =
          out.getEntryArray(sectionCounter)
              ...
              .getComponentArray(componentCounter)
              .getObservation();
      var ot = cdaMapHelper.mapToObservationPlace(in.getAnswerTxt(), element);
      out.getEntryArray(sectionCounter)
          .getOrganizer()
          .getComponentArray(componentCounter)
  ...
  private void checkEntryOrgCompObservationValue(
      int sectionCounter, int componentCounter, POCDMT000040Section out) {
    if (out.getEntryArray(sectionCounter)
            .getOrganizer()
            .getComponentArray(componentCounter)
            .getObservation()
  ...
  private CdaCaseMultiGroupSeqNumber mapMultiSelectAnsGroupSeqNumber(
      CdaCaseMultiGroupSeqNumber param, EcrMsgCaseAnswerDto in) {
    ...
    if ((param.getAnswerGroupSeqNbr() == param.getAnswerGroupCounter())
        && (param.getQuestionGroupSeqNbr() == param.getQuestionGroupCounter())) {
      param.setComponentCounter(
          param
              .getOut()
              .getEntryArray(param.getSectionCounter())
              .getOrganizer()
              .getComponentArray()
              .length);
    } else {
      if (param.getOut().getEntryArray(param.getSectionCounter()).getOrganizer().getCode()
          == null) {
        param.getOut().getEntryArray(param.getSectionCounter()).getOrganizer().addNewCode();
      }

      if (param.getOut().getEntryArray(param.getSectionCounter()).getOrganizer().getStatusCode()
          == null) {
      ...
      param
          ...
          .getOrganizer()
          .getCode()
          .setCode(String.valueOf(param.getQuestionGroupSeqNbr()));
      param
          .getOut()
          .getEntryArray(param.getSectionCounter())
          .setTypeCode(XActRelationshipEntry.COMP);
      param
          .getOut()
          .getEntryArray(param.getSectionCounter())
          .getOrganizer()
    ...
    param
        ...
        .getEntryArray(param.getSectionCounter())
        .getOrganizer()
        .getComponentArray(param.getComponentCounter())
        .getObservation()
        .setClassCode("OBS");
    param
        .getOut()
  ...
  private void checkCaseStructComponent(POCDMT000040ClinicalDocument1 clinicalDocument) {
  ...
  private CdaCaseComponent checkCaseStructComponentWithSectionAndIndex(
      POCDMT000040ClinicalDocument1 clinicalDocument, int c) {
    ...
    if (clinicalDocument.getComponent().getStructuredBody().getComponentArray(c).getSection()
        == null) {
      clinicalDocument
          .getComponent()
          .getStructuredBody()
  ...
  private boolean checkInvalidField(String name, EcrSelectedCase caseDto) {
    //        boolean patLocalIdFailedCheck = (name.equalsIgnoreCase(PAT_LOCAL_ID_CONST) &&
    // caseDto.getMsgCase().getPatLocalId() == null)
    //                || (name.equalsIgnoreCase(PAT_LOCAL_ID_CONST)  &&
    // caseDto.getMsgCase().getPatLocalId() != null &&
    // caseDto.getMsgCase().getPatLocalId().isEmpty());
    //        boolean patInvEffTimeFailedCheck = name.equalsIgnoreCase("invEffectiveTime")  &&
    // caseDto.getMsgCase().getInvEffectiveTime() == null;
    //        boolean patInvAuthorIdFailedCheck = (name.equalsIgnoreCase("invAuthorId") &&
    // caseDto.getMsgCase().getInvAuthorId() == null)
    //                || (name.equalsIgnoreCase("invAuthorId") &&
    // caseDto.getMsgCase().getInvAuthorId() != null &&
    // caseDto.getMsgCase().getInvAuthorId().isEmpty());
    //        return patLocalIdFailedCheck || patInvEffTimeFailedCheck || patInvAuthorIdFailedCheck;
    return name != null
        && (name.startsWith(PAT_LOCAL_ID_CONST)

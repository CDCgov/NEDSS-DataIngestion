...
public class RawElrService {
  ...
  private static final String KAFKA_PRODUCER_FAILED_MSG =
      "Sending event to elr_raw kafka topic failed";
  ...
  public String submissionElrXml(RawElrDto rawElrDto) throws KafkaProducerException {
    ...
    try {
      ...
      iElrDeadLetterRepository.addErrorStatusForRawId(
          created.getId(),
          topicName,
          created.getType(),
          created.getPayload(),
          KAFKA_PRODUCER_FAILED_MSG,
          dltOccurrence + 1);
    ...
    if (rawElrDto.getType().equalsIgnoreCase(HL7_ELR)) {
      if (rawElrDto.getCustomMapper() != null && !rawElrDto.getCustomMapper().trim().isEmpty()) {
        rawElrDto.setPayload(
            hl7MessageCustomMapping(rawElrDto.getPayload(), rawElrDto.getCustomMapper()));
      }
  ...
  public void updateRawMessageAfterRetry(RawElrDto rawElrDto, int dltOccurrence)
    ...
    try {
      ...
      iElrDeadLetterRepository.updateDltOccurrenceForRawId(
          rawElrDto.getId(), dltOccurrence + 1, "ERROR");
      throw new KafkaProducerException(
          "Failed publishing message again to kafka topic: "
              + topicName
              + " with UUID: "
              + rawElrDto.getId());
    }
  ...
  private List<RawElrModel> createRawElrModelsForBatch(
      List<String> hl7Messages, RawElrDto rawElrDto) {
  ...
  private String hl7MessageCustomMapping(String message, String customMapper)
      throws KafkaProducerException {
    ...
    try {
      ...
      throw new KafkaProducerException(
          "Custom mapping find and replace:Error at parsing/replacing the mapper value:"
              + e.getMessage());
    }
    return message;
  }

...
public class KafkaConsumerService {
  ...
  /**
   * Read Me: - Standard flow -> RawELR -> ValidatedELR -> Preparation For Conversion (No
   * significant logic, simply send message to appropriate topics for consuming) -> These 2 paths
   * will be consumed in no particular order -> XML Conversion (consuming from Prepare XML topic) ->
  ...
  public void handleMessageForRawElr(
      String message,
  ...
  /**
   * Receive XML converted HL7 and save it to NBS Interface Description: The ELR ingestion endpoint
   * that we have right now is modified to accept both plain text HL7 as well as HL7s converted to
   * XML already.
   */
  @RetryableTopic(
      attempts = "${kafka.consumer.max-retry}",
  ...
  public void handleMessageForElrXml(
      String message,
    ...
    timeMetricsBuilder.recordElrRawXmlEventTime(
        () -> {
          ...
          try {
            nbsInterfaceModel =
                nbsRepositoryServiceProvider.saveElrXmlMessage(
                    messageId, message, dataProcessingApplied);
          } catch (XmlConversionException e) {
            throw new RuntimeException(e); // NOSONAR
          }
          log.debug(
              "Saved Elr xml to NBS_interface table with uid: {}",
  ...
  /**
   * After Validation Process Description: After validated by Raw Consumer, data will be sent to
   * validated producer the data will be picked up by this consumer this logic will direct validated
   * data into 2 path (producers + topic) 1 - to XML conversion consumer 2 - to FHIR conversion
   * consumer
   */
  @RetryableTopic(
      attempts = "${kafka.consumer.max-retry}",
  ...
  @KafkaListener(
      topics = "${kafka.validation.topic}",
      containerFactory = "kafkaListenerContainerFactoryValidate")
  public void handleMessageForValidatedElr(
      String message,
      @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
      @Header(KafkaHeaderValue.DATA_PROCESSING_ENABLE) String dataProcessingEnable) {
  ...
  /** XML Conversion */
  @KafkaListener(
      topics = "${kafka.xml-conversion-prep.topic}",
      containerFactory = "kafkaListenerContainerFactoryXml")
  public void handleMessageForXmlConversionElr(
      String message,
      @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
  ...
  private void shareProcessingDlt(
      String dltOccurrence,
    ...
    ElrDeadLetterDto elrDeadLetterDto =
        new ElrDeadLetterDto(
            message,
            erroredSource,
            stacktrace,
            dltCount,
  ...
  private void processingDltRecord(ElrDeadLetterDto elrDeadLetterDto) {
    ...
    try {
      if (elrDeadLetterDto.getErrorMessageSource().equalsIgnoreCase(rawTopic)) {
        var message =
            this.iRawELRRepository
                .findById(elrDeadLetterDto.getErrorMessageId())
                .orElseThrow(
                    () ->
                        new DeadLetterTopicException(
                            processDltErrorMessage + elrDeadLetterDto.getErrorMessageId()));
        ...
        throw new DeadLetterTopicException(
            "Unsupported Topic; topic: " + elrDeadLetterDto.getErrorMessageSource());
      } else {
        throw new DeadLetterTopicException(
            "Unsupported Topic; topic: " + elrDeadLetterDto.getErrorMessageSource());
      }
  ...
  private void preparationForConversionHandler(String message, String dataProcessingEnable)
      throws ConversionPrepareException, KafkaProducerException {
    Optional<ValidatedELRModel> validatedElrResponse =
        this.iValidatedELRRepository.findById(message);
    if (validatedElrResponse.isPresent()) {
      kafkaProducerService.sendMessagePreparationTopic(
          validatedElrResponse.get(),
          prepXmlTopic,
          TopicPreparationType.XML,
          0,
          dataProcessingEnable);
      // kafkaProducerService.sendMessagePreparationTopic(validatedElrResponse.get(), prepFhirTopic,
      // TopicPreparationType.FHIR, 0, dataProcessingEnable);//NOSONAR
    } else {
  ...
  /**
   * make this public so we can add unit test for now. we need to implementation interface pattern
   * for NBS convert and transformation classes. it better for unit testing
   */
  @SuppressWarnings("java:S3776")
  public void xmlConversionHandlerProcessing(
      String messageId, String operation, String dataProcessingEnable)
    ...
    try {
      ...
      List<ReportStatusIdData> statusList =
          iReportStatusRepository.findByRawMessageId(validatedELRModel.get().getRawId());
      ...
      if (operation.equalsIgnoreCase(EnumKafkaOperation.INJECTION.name())) {
        Optional<ValidatedELRModel> validatedElrResponse =
            this.iValidatedELRRepository.findById(messageId);
      ...
      HL7ParsedMessage<OruR1> parsedMessageOrig =
          Hl7ToRhapsodysXmlConverter.getInstance().parsedStringToHL7(hl7Msg);
      ...
      for (HL7ParsedMessage<OruR1> hl7ParsedMessage : parsedMessageList) {
        ...
        NbsInterfaceModel nbsInterfaceModel =
            nbsRepositoryServiceProvider.saveXmlMessage(
                messageId, phdcXml, hl7ParsedMessage, dataProcessingApplied);
        ...
        if (dataProcessingApplied) {
          kafkaProducerService.sendMessageAfterConvertedToXml(
              nbsInterfaceModel.getNbsInterfaceUid().toString(), rtiTopic, 0); // NOSONAR
        } else {
          kafkaProducerService.sendMessageAfterConvertedToXml(
              nbsInterfaceModel.getNbsInterfaceUid().toString(), convertedToXmlTopic, 0);
        }
      ...
      kafkaProducerService.sendMessageDlt(
          msg, messageId, "xml_prep_dlt_manual", 0, stackTrace, prepXmlTopic);
    }
  }
  ...
  private void xmlConversionHandler(String message, String operation, String dataProcessingEnable)
      throws KafkaProducerException {
    log.debug(
        "Received message id will be retrieved from db and associated hl7 will be converted to xml");
    xmlConversionHandlerProcessing(message, operation, dataProcessingEnable);
  }

  private void validationHandler(
      String message, boolean hl7ValidationActivated, String dataProcessingEnable)
      throws DuplicateHL7FileFoundException, DiHL7Exception, KafkaProducerException {
    ...
    switch (messageType) {
      case KafkaHeaderValue.MESSAGE_TYPE_HL7:
        ...
        try {
          hl7ValidatedModel =
              iHl7v2Validator.messageValidation(
                  message, elrModel, validatedTopic, hl7ValidationActivated);
          ...
          if (errorMsg != null
              && errorMsg.contains("ValidationException")
              && errorMsg.contains("HL7 datetime string at MSH-6")) {
            errorMsg =
                errorMsg.replace("HL7 datetime string at MSH-6", "HL7 datetime string at MSH-7");
          }
        ...
        kafkaProducerService.sendMessageAfterValidatingMessage(

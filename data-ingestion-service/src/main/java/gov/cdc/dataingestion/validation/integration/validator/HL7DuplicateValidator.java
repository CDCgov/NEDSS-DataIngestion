...
public class HL7DuplicateValidator implements IHL7DuplicateValidator {
  ...
  @Override
  public void validateHL7Document(ValidatedELRModel hl7ValidatedModel)
    ...
    if (!checkForDuplicateHL7HashString(hashedString)) {
      ...
      kafkaProducerService.sendMessageAfterCheckingDuplicateHL7(
          hl7ValidatedModel, validatedElrDuplicateTopic, 0);
      throw new DuplicateHL7FileFoundException(
          "HL7 document already exists in the database. "
              + "Please check elr_raw table for the failed document. Record Id: "
              + hl7ValidatedModel.getRawId());
    }
  }

  public boolean checkForDuplicateHL7HashString(String hashedString) {
    log.debug(
        "Generated HashString is being checked for duplicate if already present in the database");
    Optional<ValidatedELRModel> validatedELRResponseFromDatabase =
        iValidatedELRRepository.findByHashedHL7String(hashedString);
    if (!validatedELRResponseFromDatabase.isEmpty()
        && hashedString.equals(validatedELRResponseFromDatabase.get().getHashedHL7String())) {
      log.error(
          "Duplicate hashed string found for the HL7 message in the database. Sending details to kafka dlt topic.");
      return true;
    }

...
public class ReportStatusService {
  ...
  public ReportStatusService(
      ...
      IEdxActivityParentLogRepository iEdxActivityParentLogRepository,
  ...
  public List<MessageStatus> getMessageStatus(String rawMessageID) {
    ...
    if (rawMessageData.isPresent()) {
      ...
      msgStatus
          .getRawInfo()
          .setRawCreatedOn(
              TimeStampHelper.convertTimestampToString(rawMessageData.get().getCreatedOn()));
      msgStatus.getRawInfo().setRawPipeLineStatus(MSG_STATUS_SUCCESS);

      if (rawMessageData.get().getType().equalsIgnoreCase(HL7_ELR)) {
        Optional<ValidatedELRModel> validatedMessageData =
            iValidatedELRRepository.findByRawId(msgStatus.getRawInfo().getRawMessageId());
        if (validatedMessageData.isPresent()) {
          msgStatus.getValidatedInfo().setValidatedMessageId(validatedMessageData.get().getId());
          msgStatus
              .getValidatedInfo()
              .setValidatedCreatedOn(
                  TimeStampHelper.convertTimestampToString(
                      validatedMessageData.get().getCreatedOn()));
      ...
      for (MessageStatus msgStatusNew : msgStatusList) {
        if (msgStatusNew.getNbsInfo().getNbsInterfaceStatus() != null) {
          EdxActivityLog edxActivityLog =
              iEdxActivityParentLogRepository.getParentEdxActivity(
                  Long.valueOf(msgStatusNew.getNbsInfo().getNbsInterfaceId()));
          List<EdxActivityDetailLog> edxActivityStatusList =
              iEdxActivityLogRepository.getEdxActivityLogDetailsBySourceId(
                  Long.valueOf(msgStatusNew.getNbsInfo().getNbsInterfaceId()));
          if (!edxActivityStatusList.isEmpty() && edxActivityLog != null) {
            ...
            for (EdxActivityDetailLog edxActivityLogModel : edxActivityStatusList) {
              ...
              if (seenComments.add(logComment)) {
                msgStatusNew
                    .getEdxLogStatus()
                    .getEdxActivityDetailLogList()
                    .add(edxActivityLogModel);
              }
            }
          }
        }
  ...
  private List<MessageStatus> setDiXmlTransformationInfo(MessageStatus msgStatus) {
    List<MessageStatus> msgStatusList = new ArrayList<>();
    List<ReportStatusIdData> elrStatusIdList =
        iReportStatusRepository.findByRawMessageId(msgStatus.getRawInfo().getRawMessageId());
    if (!elrStatusIdList.isEmpty()) {
      ...
      for (ReportStatusIdData reportStatusIdData : elrStatusIdList) {
        ...
        msgStatusNew
            .getNbsInfo()
            .setNbsCreatedOn(
                TimeStampHelper.convertTimestampToString(reportStatusIdData.getCreatedOn()));
      ...
      if (msgStatus.getValidatedInfo().getValidatedMessageId() == null) {
        ...
        setDltInfo(
            msgStatus.getValidatedInfo().getValidatedMessageId(), msgStatus, DLT_ORIGIN_VALIDATED);
      }
      msgStatusList.add(msgStatus);
    }
    return msgStatusList;
  ...
  private void setNbsInfo(MessageStatus msgStatus) {
    Optional<NbsInterfaceModel> nbsInterfaceModel =
        nbsInterfaceRepository.findByNbsInterfaceUid(msgStatus.getNbsInfo().getNbsInterfaceId());
      ...
      switch (origin) {
        case DLT_ORIGIN_RAW:
          ...
          msgStatus
              .getRawInfo()
              .getDltInfo()
              .setDltCreatedOn(TimeStampHelper.convertTimestampToString(dlt.get().getCreatedOn()));
        ...
        case DLT_ORIGIN_VALIDATED:
          ...
          msgStatus
              .getValidatedInfo()
              .getDltInfo()
              .setDltShortTrace(dlt.get().getErrorStackTraceShort());

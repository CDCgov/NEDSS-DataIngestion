...
public class ReportStatusController {
  ...
  @Operation(
      ...
      description =
          "Return all info such as the pipeline which the message is currently at, dlt info,nbs ingestion etc..",
      parameters = {
        @Parameter(
            in = ParameterIn.HEADER,
            name = "clientid",
  ...
  public ResponseEntity<List<MessageStatus>> getMessageStatus(
      @PathVariable("elr-id") String rawMessageId) {
    List<String> messageIdList = Arrays.asList(rawMessageId.split(","));
    List<MessageStatus> statusList =
        messageIdList.stream()

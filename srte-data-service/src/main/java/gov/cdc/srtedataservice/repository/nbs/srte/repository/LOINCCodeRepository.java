...
import gov.cdc.srtedataservice.repository.nbs.srte.model.LOINCCode;
...
public interface LOINCCodeRepository extends JpaRepository<LOINCCode, String> {
  ...
  @Query(
      value = "select * from LOINC_code where time_aspect = 'Pt' and system_cd = '^Patient'",
      nativeQuery = true)
  Optional<List<LOINCCode>> findLoincCodes();

  @Query(
      "SELECT DISTINCT lcc.conditionCd FROM LOINCCode lc INNER JOIN LOINCCodeCondition lcc ON lc.loincCode = lcc.loincCd WHERE lc.loincCode = :loincCd")

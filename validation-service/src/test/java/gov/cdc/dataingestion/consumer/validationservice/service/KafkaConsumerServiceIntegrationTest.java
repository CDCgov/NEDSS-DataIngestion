...
public class KafkaConsumerServiceIntegrationTest {
    ...
    public void handleMessage_consumedMessage_invalidTypeAndMessage() throws InterruptedException {
        ...
        Assertions.assertEquals(MessageType.None.name(), target.getMessageType().name());
    }
    ...
    public void handleMessage_consumedMessage_validHL7v2() throws InterruptedException {
        ...
        boolean messageConsumed = target.getLatch().await(10, TimeUnit.SECONDS);
        Assertions.assertTrue(messageConsumed);
        Assertions.assertTrue(target.isMessageValid());
        Assertions.assertEquals(MessageType.HL7v2.name(), target.getMessageType().name());
    }
    ...
    public void handleMessage_consumedMessage_validCSV() throws InterruptedException {
        ...
        kafkaProducerService.sendMessageFromController(data, topicName, KafkaHeaderValue.MessageType_CSV);
        var target = kafkaConsumerService;
        boolean messageConsumed = target.getLatch().await(10, TimeUnit.SECONDS);
        Assertions.assertTrue(messageConsumed);
        Assertions.assertTrue(target.isMessageValid());
        Assertions.assertEquals(MessageType.CSV.name(), target.getMessageType().name());
    }

...
public class SeedWriter implements ItemWriter<NbsPerson> {
  ...
  public void write(Chunk<? extends NbsPerson> chunk) throws Exception {
    ...
    List<String> personParentUids =
        chunk.getItems().stream().map(NbsPerson::personParentUid).toList();
  ...
  private List<Cluster> fetchClusters(List<String> personParentUids) {
    ...
    List<MpiPerson> clusterEntries =
        namedParameterJdbcTemplate.query(
            QueryConstants.PERSON_RECORDS_BY_PARENT_IDS,
            new MapSqlParameterSource("ids", personParentUids),
            mapper);

    Map<String, List<MpiPerson>> clusterDataMap =
        clusterEntries.stream().collect(Collectors.groupingBy(MpiPerson::parent_id));

    return personParentUids.stream()
        .map(personParentUid -> new Cluster(clusterDataMap.get(personParentUid), personParentUid))
        .toList();
  }
}

...
class MergeServiceTest {
  ...
  @Mock private PersonTableMergeHandler personTableMergeHandler;

  @Mock private AdminCommentMergeHandler adminCommentMergeHandler;
  ...
  void setUp() {
    List<SectionMergeHandler> handlers =
        Arrays.asList(personTableMergeHandler, adminCommentMergeHandler);
    StatementSpec mockStatementSpec = Mockito.mock(StatementSpec.class);

    when(deduplicationClient.sql(MergeService.MARK_PATIENTS_AS_MERGED))
        .thenReturn(mockStatementSpec);
    ...
    when(deduplicationClient.sql(MergeService.SAVE_PATIENT_MERGE_AUDIT))
        .thenReturn(auditStatementSpec);
  ...
  void testPerformMerge_CallsAllHandlersInOrder_AndSavesAudit() throws Exception {
    ...
    InOrder inOrder =
        inOrder(personTableMergeHandler, adminCommentMergeHandler, deduplicationClient);

    inOrder
        .verify(personTableMergeHandler)
        .handleMerge(eq(matchIdStr), eq(request), any(PatientMergeAudit.class));
    inOrder
        .verify(adminCommentMergeHandler)
        .handleMerge(eq(matchIdStr), eq(request), any(PatientMergeAudit.class));

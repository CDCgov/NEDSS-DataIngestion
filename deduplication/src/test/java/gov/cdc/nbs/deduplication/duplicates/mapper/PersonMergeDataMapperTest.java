...
class PersonMergeDataMapperTest {
  ...
  private void assertGeneralPatientInformation(PersonMergeData personMergeData) {
    ...
    assertThat(personMergeData.generalPatientInformation().maritalStatusDescription()).isEqualTo(MARITAL_STATUS_DESCRIPTION);
    assertThat(personMergeData.generalPatientInformation().mothersMaidenName()).isEqualTo(MOTHERS_MAIDEN_NAME);
    assertThat(personMergeData.generalPatientInformation().adultsInHouseholdNumber()).isEqualTo(ADULTS_IN_HOUSEHOLD_NUMBER);
    assertThat(personMergeData.generalPatientInformation().childrenInHouseholdNumber()).isEqualTo(CHILDREN_IN_HOUSEHOLD_NUMBER);
    assertThat(personMergeData.generalPatientInformation().occupationCode()).isEqualTo(OCCUPATION_CODE);
    assertThat(personMergeData.generalPatientInformation().educationLevelDescription()).isEqualTo(EDUCATION_LEVEL_DESCRIPTION);
    assertThat(personMergeData.generalPatientInformation().primaryLanguageDescription()).isEqualTo(PRIMARY_LANGUAGE_DESCRIPTION);

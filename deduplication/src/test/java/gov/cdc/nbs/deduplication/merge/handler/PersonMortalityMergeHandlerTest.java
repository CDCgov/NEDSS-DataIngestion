...
class PersonMortalityMergeHandlerTest {
  ...
  void setUp() {
    ...
    fieldSourceWithDiffIds =
        new PatientMergeRequest.MortalityFieldSource(
            SOURCE_ID, SOURCE_ID, SOURCE_ID, SOURCE_ID, SOURCE_ID, SOURCE_ID);

    fieldSourceWithSameIds =
        new PatientMergeRequest.MortalityFieldSource(
            SURVIVOR_ID, SURVIVOR_ID, SURVIVOR_ID, SURVIVOR_ID, SURVIVOR_ID, SURVIVOR_ID);
  ...
  void handleMerge_ShouldAddAuditUpdateActions() {
    ...
    when(nbsTemplate.queryForList(
            eq(PersonMortalityMergeHandler.SELECT_DEATH_CITY_FOR_AUDIT_BEFORE_UPDATE),
            any(MapSqlParameterSource.class)))
    ...
    when(nbsTemplate.queryForList(
            eq(PersonMortalityMergeHandler.SELECT_DEATH_COUNTRY_FOR_AUDIT_BEFORE_UPDATE),
            any(MapSqlParameterSource.class)))

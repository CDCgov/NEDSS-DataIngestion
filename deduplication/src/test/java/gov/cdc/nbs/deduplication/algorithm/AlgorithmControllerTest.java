...
class AlgorithmControllerTest {
  ...
  @InjectMocks private AlgorithmController controller;
      ...
      new Pass(
          ...
          List.of(
              ...
              new MatchingAttributeEntry(
                  MatchingAttribute.LAST_NAME, MatchingMethod.JAROWINKLER, 0.66)),
          0.52,
          0.92);
  ...
  void should_throw_exception_export() throws IOException {
    ...
    when(mapper.writeValueAsString(any(AlgorithmExport.class)))
  ...
  void should_import() throws IOException {
    ...
    MultipartFile multipartFile =
        new MockMultipartFile("importedFile", mapper.writeValueAsBytes(exportedAlgorithm));
  ...
  void should_throw_exception_import() throws IOException {
    ...
    when(mapper.readValue(multipartFile.getBytes(), AlgorithmExport.class))
        .thenThrow(JsonProcessingException.class);

    AlgorithmException ex =
        assertThrows(AlgorithmException.class, () -> controller.importAlgorithm(multipartFile));

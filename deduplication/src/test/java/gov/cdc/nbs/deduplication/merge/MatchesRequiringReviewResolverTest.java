...
class MatchesRequiringReviewResolverTest {
  ...
  @InjectMocks MatchesRequiringReviewResolver resolver;
  ...
  void should_fetch() {
    ...
    when(template.queryForObject(MatchesRequiringReviewResolver.COUNT_QUERY, Integer.class))
        .thenReturn(14);
    // fetch
    ...
    when(deduplicationTemplate.query(
            ...
            captor.capture(),
            ArgumentMatchers.<RowMapper<MatchRequiringReview>>any()))
        .thenReturn(
            List.of(
  ...
  void should_throw_error_invalid_sort() {
    MergeListException ex =
        assertThrows(MergeListException.class, () -> resolver.toOrder("badSort"));

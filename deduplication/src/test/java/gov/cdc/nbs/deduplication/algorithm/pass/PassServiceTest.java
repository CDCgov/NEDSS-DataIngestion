...
class PassServiceTest {
  ...
  @Mock private DataElementsService dataElementsService;

  @Mock private DibbsService dibbsService;

  @Mock private ObjectMapper mapper;

  @InjectMocks private PassService service;
  ...
  private final Algorithm algorithm =
      new Algorithm(Stream.of(pass).collect(Collectors.toCollection(ArrayList::new)));
  ...
  private void mockCurrentConfig(Algorithm returnedAlgorithm) throws JsonProcessingException {
    ...
    when(mockTemplate.queryForList(PassService.SELECT_CURRENT_CONFIG, String.class))
  ...
  void should_throw_algorithm_exception() throws JsonProcessingException {
    ...
    AlgorithmException ex =
        assertThrows(AlgorithmException.class, () -> service.getCurrentAlgorithm());
    ...
    Algorithm actual =
        service.save(
            new Pass(
                ...
                List.of(
                    new MatchingAttributeEntry(
                        MatchingAttribute.ADDRESS, MatchingMethod.EXACT, 0.77),
                    new MatchingAttributeEntry(
                        MatchingAttribute.BIRTHDATE, MatchingMethod.EXACT, 0.9)),
                0.52,
                0.92));
  ...
  void should_throw_exception_when_mapper_fails() throws JsonProcessingException {
    ...
    mockCurrentConfig(emptyAlgorithm);
    when(dataElementsService.getCurrentDataElements()).thenReturn(TestData.DATA_ELEMENTS);
    when(mapper.writeValueAsString(Mockito.any(Algorithm.class)))
        .thenThrow(JsonProcessingException.class);

    PassModificationException ex =
        assertThrows(PassModificationException.class, () -> service.save(pass));
  ...
  void update_pass_should_throw_exception_no_pass_found() throws JsonProcessingException {
    ...
    PassModificationException ex =
        assertThrows(PassModificationException.class, () -> service.update(2l, null));
  ...
  void delete_pass_two_passes() throws JsonProcessingException {
    ...
    passes.add(
        new Pass(
            ...
            List.of(
                new MatchingAttributeEntry(
                    MatchingAttribute.FIRST_NAME, MatchingMethod.EXACT, 0.45),
                new MatchingAttributeEntry(
                    MatchingAttribute.LAST_NAME, MatchingMethod.JAROWINKLER, 0.32)),
            0.52,
            0.92));
    passes.add(
        new Pass(
  ...
  void delete_pass_should_throw_exception_no_pass_found() throws JsonProcessingException {
    ...
    PassModificationException ex =
        assertThrows(PassModificationException.class, () -> service.delete(2l));
    assertThat(ex.getMessage()).isEqualTo("Failed to find pass with Id: 2");
  }
  ...
  void should_save_dibbs() throws JsonProcessingException {
    ...
    verify(dibbsService, times(1)).save(algorithmMapper.map(algorithm, TestData.DATA_ELEMENTS));
  }

...
class EdxMatchJdbcRepositoryTest {
  ...
  @InjectMocks private EdxMatchJdbcRepository repository;
  ...
  void testMergeEdxPatientMatch_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_EDX_PATIENT_MATCH), any(MapSqlParameterSource.class));
  }
  ...
  void testMergeEdxEntityMatch_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_EDX_ENTITY_MATCH), any(MapSqlParameterSource.class));
  }
}

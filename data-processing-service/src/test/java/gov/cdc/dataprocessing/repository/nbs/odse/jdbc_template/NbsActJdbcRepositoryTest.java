...
class NbsActJdbcRepositoryTest {
  ...
  @InjectMocks private NbsActJdbcRepository repository;
  ...
  void testMergeNbsActEntity_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_NBS_ACT_ENTITY), any(MapSqlParameterSource.class));
  }
  ...
  void testMergeNbsActEntityHist_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_NBS_ACT_ENTITY_HIST), any(MapSqlParameterSource.class));
  }
  ...
  void testDeleteNbsEntityAct_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(DELETE_NBS_ACT_ENTITY_BY_UID), any(MapSqlParameterSource.class));
  }
  ...
  void testGetNbsActEntitiesByActUid_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_NBS_ACT_ENTITIES_BY_ACT_UID),

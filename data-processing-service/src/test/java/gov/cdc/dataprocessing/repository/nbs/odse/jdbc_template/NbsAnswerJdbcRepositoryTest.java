...
class NbsAnswerJdbcRepositoryTest {
  ...
  @InjectMocks private NbsAnswerJdbcRepository repository;
  ...
  void testMergeNbsAnswer_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_NBS_ANSWER), any(MapSqlParameterSource.class));
  }
  ...
  void testMergeNbsAnswerHist_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_NBS_ANSWER_HIST), any(MapSqlParameterSource.class));
  }
  ...
  void testDeleteByNbsAnswerUid_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(DELETE_NBS_ANSWER_BY_UID), any(MapSqlParameterSource.class));
  }
  ...
  void testFindByActUid_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_NBS_ANSWER_BY_ACT_UID),

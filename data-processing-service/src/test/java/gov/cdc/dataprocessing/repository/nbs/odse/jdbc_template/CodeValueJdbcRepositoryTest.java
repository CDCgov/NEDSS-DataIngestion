...
class CodeValueJdbcRepositoryTest {
  ...
  @InjectMocks private CodeValueJdbcRepository repository;
  ...
  void testFindCodeDescriptionsByCodeSetNm_shouldReturnResults() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_CODE_VALUE_GENERAL_BY_CODE_SET_NM),
  ...
  void testFindCodeValuesByCodeSetNm_shouldReturnResults() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_CODE_VALUE_GENERAL_BY_CODE_SET_NM_ORDERED),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(recordCode));
  ...
  void testFindCodeValuesByCodeSetNmAndCode_shouldReturnResults() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_CODE_VALUE_GENERAL_BY_CODE_SET_AND_CODE),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(recordCode));

    List<CodeValueGeneral> result =
        repository.findCodeValuesByCodeSetNmAndCode("testSet", "TEST_CODE");

...
class EntityLocatorJdbcRepositoryTest {
  ...
  @InjectMocks private EntityLocatorJdbcRepository repository;
  ...
  void testCreatePhysicalLocator_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse)
        .update(eq(INSERT_SQL_PHYSICAL_LOCATOR), any(MapSqlParameterSource.class));
  }
  ...
  void testCreatePostalLocator_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse)
        .update(eq(INSERT_SQL_POSTAL_LOCATOR), any(MapSqlParameterSource.class));
  }
  ...
  void testFindEntityLocatorParticipations_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_ENTITY_LOCATOR_PARTICIPATIONS_BY_ENTITY_UID),
  ...
  void testFindByEntityUid_shouldReturnList() {
    EntityLocatorParticipation resultObj = new EntityLocatorParticipation();
    when(jdbcTemplateOdse.query(
            eq(SELECT_ENTITY_LOCATOR_BY_ENTITY_UID),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(resultObj));
  ...
  void testFindByPostalLocatorUids_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_POSTAL_LOCATOR_BY_UIDS),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(resultObj));
  ...
  void testFindByPhysicalLocatorUids_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_PHYSICAL_LOCATOR_BY_UIDS),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(resultObj));
  ...
  void testFindByTeleLocatorUids_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_TELE_LOCATOR_BY_UIDS),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(resultObj));
  ...
  void testUpdateTeleLocator() {
    ...
    when(jdbcTemplateOdse.update(any(String.class), any(MapSqlParameterSource.class)))
        .thenReturn(1);

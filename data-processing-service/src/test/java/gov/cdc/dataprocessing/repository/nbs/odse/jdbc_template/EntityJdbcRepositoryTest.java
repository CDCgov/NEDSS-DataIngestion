...
class EntityJdbcRepositoryTest {
  ...
  @InjectMocks private EntityJdbcRepository repository;
  ...
  void testCreateEntity_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(INSERT_SQL_ENTITY), any(MapSqlParameterSource.class));
  }
  ...
  void testUpdateEntity_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(UPDATE_ENTITY_BY_ID), any(MapSqlParameterSource.class));
  }
}

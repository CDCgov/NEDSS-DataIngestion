...
class ConfirmationMethodJdbcRepositoryTest {
  ...
  @InjectMocks private ConfirmationMethodJdbcRepository repository;
  ...
  void testUpsertConfirmationMethod_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_CONFIRMATION_METHOD), any(MapSqlParameterSource.class));
  }
  ...
  void testFindByPublicHealthCaseUid_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_CONFIRMATION_METHOD_BY_UID),

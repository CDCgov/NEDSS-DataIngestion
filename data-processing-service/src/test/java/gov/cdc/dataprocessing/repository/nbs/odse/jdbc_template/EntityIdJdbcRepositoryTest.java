...
class EntityIdJdbcRepositoryTest {
  ...
  @InjectMocks private EntityIdJdbcRepository repository;
  ...
  void testCreateEntityId_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(INSERT_SQL_ENTITY_ID), any(MapSqlParameterSource.class));
  }
  ...
  void testBatchCreateEntityIds_shouldCallBatchUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .batchUpdate(eq(INSERT_SQL_ENTITY_ID), any(MapSqlParameterSource[].class));
  }
  ...
  void testMergeEntityId_shouldCallUpdate() {
    ...
    verify(jdbcTemplateOdse, times(1))
        .update(eq(MERGE_ENTITY_ID), any(MapSqlParameterSource.class));
  }
  ...
  void testFindEntityIds_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_ENTITY_ID_BY_ENTITY_ID),
  ...
  void testFindEntityIdsActive_shouldReturnList() {
    ...
    when(jdbcTemplateOdse.query(
            eq(SELECT_ENTITY_ID_BY_ENTITY_ID_ACTIVE),
            any(MapSqlParameterSource.class),
            any(BeanPropertyRowMapper.class)))
        .thenReturn(List.of(mock));

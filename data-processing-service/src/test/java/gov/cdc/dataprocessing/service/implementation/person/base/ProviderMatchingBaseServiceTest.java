...
class ProviderMatchingBaseServiceTest {
  @InjectMocks private ProviderMatchingBaseService providerMatchingBaseService;
  ...
  void testProcessingProvider_ValidData() throws DataProcessingException {
    ...
    doReturn(1L)
        .when(spyService)
        .persistingProvider(any(PersonContainer.class), anyString(), anyString());
  ...
  void testProcessingProvider_ExceptionHandling() throws DataProcessingException {
    ...
    assertThrows(
        ...
        () -> spyService.processingProvider(personContainer, "PROVIDER", "BUSINESS_TRIGGER"));
  }
  ...
  void testProcessingProvider_CallOrgHashCodeTrue_Exception() throws DataProcessingException {
    ...
    doThrow(new RuntimeException("Error"))
        .when(spyService)
        .setProvidertoEntityMatch(any(PersonContainer.class));
  ...
  void testPersistingProvider_NewPerson() throws DataProcessingException {
    ...
    // Call the method under test
    Long result =
        providerMatchingBaseService.persistingProvider(
            personContainer, "PROVIDER", "BUSINESS_TRIGGER");
  ...
  void testSetProvidertoEntityMatch_Success() throws DataProcessingException {
    ...
    // Verify interactions and results
    verify(edxPatientMatchRepositoryUtil, times(4))
        .saveEdxEntityMatch(any(EdxEntityMatchDto.class));
    verify(patientRepositoryUtil, times(1)).updateExistingPersonEdxIndByUid(anyLong());
  }

  @Test
  void testSetProvidertoEntityMatch_SaveEdxEntityMatchException_First()
    ...
    // Call the method under test and verify exception is thrown
    assertThrows(
        RuntimeException.class, () -> spyService.setProvidertoEntityMatch(personContainer));
  ...
  @Test
  void testSetProvidertoEntityMatch_SaveEdxEntityMatchException_Second()
      throws DataProcessingException {
    // Mock the PersonContainer and PersonDto
    PersonContainer personContainer = mock(PersonContainer.class);
    PersonDto personDto = mock(PersonDto.class);
    ...
    // Verify interactions
    verify(edxPatientMatchRepositoryUtil, times(2))
        .saveEdxEntityMatch(any(EdxEntityMatchDto.class));
  }
  ...
  void testGetIdentifierForProvider_ExceptionHandling_2() {
    ...
    assertThrows(
        ...
        () -> providerMatchingBaseService.getIdentifierForProvider(personContainer));
  }

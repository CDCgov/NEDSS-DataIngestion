...
class RetrieveSummaryServiceTests {
  ...
  @Mock private NotificationRepositoryUtil notificationRepositoryUtil;
  @InjectMocks private RetrieveSummaryService retrieveSummaryService;
  ...
  void retrieveDocumentSummaryVOForInv_Success() {
    long uid = 10L;
    when(customRepository.retrieveDocumentSummaryVOForInv(10L)).thenReturn(new HashMap<>());
    retrieveSummaryService.retrieveDocumentSummaryVOForInv(uid);
    verify(customRepository, times(1)).retrieveDocumentSummaryVOForInv(any());
  }

  @Test
  void retrieveDocumentSummaryVOForInv_Exception() {
    long uid = 10L;
    when(customRepository.retrieveDocumentSummaryVOForInv(10L))
  ...
  void notificationSummaryOnInvestigation_Success() throws DataProcessingException {
    ...
    when(customRepository.retrieveNotificationSummaryListForInvestigation(eq(10L), any()))
  ...
  void getAssociatedDocumentList_Success() {
    ...
    when(customRepository.getAssociatedDocumentList(
            eq(uid), eq(targetClassCd), eq(sourceClassCd), any()))
  ...
  void getAssociatedDocumentList_Success_2() {
    ...
    when(queryHelper.getDataAccessWhereClause(NBSBOLookup.DOCUMENT, "VIEW", "")).thenReturn(null);
    when(customRepository.getAssociatedDocumentList(
            eq(uid), eq(targetClassCd), eq(sourceClassCd), any()))
        .thenReturn(new HashMap<>());
  ...
  void updateNotification_Success() throws DataProcessingException {
    ...
    when(notificationRepositoryUtil.getNotificationContainer(10L)).thenReturn(notConn);

    when(prepareAssocModelHelper.prepareVO(
            any(),
            eq(NBSBOLookup.NOTIFICATION),
            eq(businessTriggerCd),
            eq("Notification"),
  ...
  void retrieveNotificationSummaryListForInvestigation1_Success() throws DataProcessingException {
    ...
    when(queryHelper.getDataAccessWhereClause(NBSBOLookup.INVESTIGATION, "VIEW", "Notification"))
        .thenReturn("BLAH");

    var map = new HashMap<String, String>();
    map.put("1", "1");
    when(catchingValueService.getCodedValuesCallRepos("PHC_CLASS")).thenReturn("1");

    when(catchingValueService.getCodedValuesCallRepos("PHC_TYPE")).thenReturn("1");
    ...
    when(catchingValueService.getCodeDescTxtForCd("1", "NBS_DOC_PURPOSE")).thenReturn("1");

    var test =
        retrieveSummaryService.retrieveNotificationSummaryListForInvestigation1(publicHealthUID);
  ...
  void notificationSummaryOnInvestigationProcessingNotificationCol_Inves() {
    ...
    var test =
        retrieveSummaryService.notificationSummaryOnInvestigationProcessingNotificationCol(
  ...
  void retrieveTreatmentSummaryVOForInv_ReturnsEmptyMap() {
    ...
    Map<Object, Object> result =
        retrieveSummaryService.retrieveTreatmentSummaryVOForInv(publicHealthUid);
  ...
  @Test
  void retrieveNotificationSummaryListForInvestigation_DataAccessClauseNull()
  ...
  @Test
  void retrieveNotificationSummaryListForInvestigation_SetsRecipientToAdminFlagCDC_WhenNndIndIsYes()
      throws DataProcessingException {
    ...
    NotificationSummaryContainer resultContainer =
        (NotificationSummaryContainer) result.iterator().next();
    assertEquals(NEDSSConstant.ADMINFLAGCDC, resultContainer.getRecipient());
  }

  @Test
  void
      retrieveNotificationSummaryListForInvestigation_SetsRecipientToLocalDesc_WhenNndIndIsNotYesAndRecipientIsNull()
          throws DataProcessingException {
  ...
  @Test
  void retrieveNotificationSummaryListForInvestigation_DataAccessClauseNullOri()
      throws DataProcessingException {
    Long publicHealthUid = 123L;
    ...
    Collection<Object> result =
        retrieveSummaryService.retrieveNotificationSummaryListForInvestigation(publicHealthUid);
  ...
  @Test
  void
      retrieveNotificationSummaryListForInvestigation_SetsRecipientToAdminFlagCDC_WhenNndIndIsYesOri()
          throws DataProcessingException {
    Long publicHealthUid = 789L;
  ...
  @Test
  void
      retrieveNotificationSummaryListForInvestigation_SetsRecipientToLocalDesc_WhenNndIndIsNotYesAndRecipientIsNullOri()
          throws DataProcessingException {
    Long publicHealthUid = 20250617L;
  ...
  @Test
  void
      notificationSummaryOnInvestigation_UsesRetrieveNotificationSummaryListForInvestigation1_WhenCaseClassCdIsNull()
          throws DataProcessingException {
    ...
    Collection<Object> result =
        retrieveSummaryService.notificationSummaryOnInvestigation(phcContainer, proxy);
    ...
    verify(queryHelper)
        .getDataAccessWhereClause(
            any(), any(),
            any()); // ensure retrieveNotificationSummaryListForInvestigation1 was triggered
  }

  @Test
  void notificationSummaryOnInvestigation_AssociatedIndTrue_WhenAutoResendIndTrue_AndPamProxy()
      throws DataProcessingException {
    // Setup PublicHealthCaseContainer with no caseClassCd (to trigger
    ...
    // Assert
    assertTrue(
        pamProxy.isAssociatedNotificationsInd(),
        "PamProxyContainer.associatedNotificationsInd should be true");
  }
}

...
class SrteCodeObsServiceTest {
  @Mock private ProgAreaSnomeCodeStoredProcRepository progAreaSnomeCodeStoredProcRepository;
  @Mock private SnomedConditionRepository snomedConditionRepository;
  @Mock private LOINCCodeRepository loincCodeRepository;
  @Mock private LabResultRepository labResultRepository;
  @Mock private LabTestRepository labTestRepository;
  @Mock private LabTestLoincRepository labTestLoincRepository;
  @Mock private LabResultSnomedRepository labResultSnomedRepository;
  @Mock private SnomedCodeRepository snomedCodeRepository;
  @Mock private ConditionCodeRepository conditionCodeRepository;

  @InjectMocks private SrteCodeObsService srteCodeObsService;
  ...
  void tearDown() {
    Mockito.reset(
        ...
        snomedConditionRepository,
        loincCodeRepository,
        labResultRepository,
        labTestRepository,
        labTestLoincRepository,
        labResultSnomedRepository,
        snomedCodeRepository,
        conditionCodeRepository,
        authUtil);
  }
  ...
  void getSnomed_Success() throws DataProcessingException {
    ...
    when(progAreaSnomeCodeStoredProcRepository.getSnomed(code, type, clia))
  ...
  void getConditionForSnomedCode_Success_1() {
    ...
    lst.add("TEST");
    when(snomedConditionRepository.getConditionForSnomedCode(snomedCd))
  ...
  void getDefaultConditionForLocalResultCodeSuccess_1() {
    ...
    var lst = new ArrayList<String>();
    lst.add("TEST");
    when(labResultRepository.findDefaultConditionCdByLabResultCdAndLaboratoryId(snomedCd, "TEST"))
        .thenReturn(Optional.of(lst));
  ...
  void getDefaultConditionForLabTest_Success_1() {
    ...
    var lst = new ArrayList<String>();
    lst.add("TEST");
    when(labTestRepository.findDefaultConditionForLabTest(snomedCd, "TEST"))
        .thenReturn(Optional.of(lst));
  ...
  void getProgramArea_Success_Null() throws DataProcessingException {
    ...
    var res =
        srteCodeObsService.getProgramArea(
            reportingLabCLIA, observationContainerCollection, electronicInd);
    assertNotNull(res);
  }
  ...
  void getPAFromSNOMEDCodes_ProgExclude() throws DataProcessingException {
    ...
    labResultLst.add(labResult);
    when(labResultRepository.findLabResultProgramAreaExclusion(any(), any()))
        .thenReturn(Optional.of(labResultLst));
  ...
  void getPAFromLocalResultCode_Success_Null() {
    ...
    loincLst.add(loin);
    when(labResultRepository.findLabResultProgramAreaExclusion(any(), any()))
        .thenReturn(Optional.of(loincLst));
  ...
  void findLocalResultDefaultConditionProgramAreaCdFromLabTestWithoutJoin_Null() {
    ...
    var res =
        srteCodeObsService.findLocalResultDefaultConditionProgramAreaCdFromLabTestWithoutJoin(
            codeVector, reportingLabCLIA, nextLookup);

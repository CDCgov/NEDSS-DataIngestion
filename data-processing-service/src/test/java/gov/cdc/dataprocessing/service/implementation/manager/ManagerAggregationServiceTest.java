...
class ManagerAggregationServiceTest {
  ...
  @Mock private IPersonService patientService;
  @Mock private IUidService uidService;
  @Mock private IObservationService observationService;
  @Mock private IObservationMatchingService observationMatchingService;
  ...
  @InjectMocks private ManagerAggregationService managerAggregationService;
  @Mock AuthUtil authUtil;

  @Mock private LabResultProxyContainer labResult;

  @Mock private EdxLabInformationDto edxLabInformationDto;

  @Mock private PersonAggContainer personAggContainer;

  @Mock private OrganizationContainer organizationContainer;

  @Mock private Collection<ObservationContainer> observationContainerCollection;

  @Mock private Collection<PersonContainer> personContainerCollection;

  @Mock private ObservationContainer observationContainer;

  @Mock private ObservationDto observationDto;

  @Mock private ActIdDto actIdDto;

  @Mock private PersonContainer personContainer;

  @Mock private PersonDto personDto;
  @Mock private LabResultProxyContainer labResultProxyContainer;
  ...
  void processingObservationMatching_Test_1() throws DataProcessingException {
    ...
    when(observationMatchingService.checkingMatchingObservation(edxLabInformationDto))
        .thenReturn(obsDto);
    when(observationService.getObservationToLabResultContainer(11L))
        .thenReturn(labResultProxyContainer);

    var test =
        managerAggregationService.processingObservationMatching(
            edxLabInformationDto, labResultProxyContainer, aPersonUid);
  ...
  void testProgAndJurisdictionAggregationAsync_HappyPath() throws DataProcessingException {
    ...
    when(labResult.getTheObservationContainerCollection())
        .thenReturn(observationContainerCollection);
    ...
    managerAggregationService.progAndJurisdictionAggregation(
        labResult, edxLabInformationDto, personAggContainer, organizationContainer);
  ...
  void testProgAndJurisdictionAggregationAsync_Exception() throws DataProcessingException {
    ...
    doThrow(new DataProcessingException("Error"))
        .when(programAreaService)
        .getProgramArea(any(), any(), any());
  ...
  void testObservationAggregation_HappyPath() {
    ...
    Collection<ObservationContainer> observationContainerCollection =
        Arrays.asList(observationContainer);

    // Call the method under test
    managerAggregationService.observationAggregation(
        labResult, edxLabInformationDto, observationContainerCollection);
  ...
  void patientAggregation_Test() throws DataProcessingConsumerException, DataProcessingException {
    ...
    var res =
        managerAggregationService.patientAggregation(
            labResultProxyContainer, edxLabInformationDto, personContainerCollection);
  ...
  void testPatientAggregation_WhenPersonCollectionIsNull_ReturnsEmptyContainer() throws Exception {
    PersonAggContainer result =
        managerAggregationService.patientAggregation(
            labResultProxyContainer, edxLabInformationDto, null);
  ...
  void testPatientAggregation_WhenCdIsProvider_CallsProcessingProvider() throws Exception {
    ...
    when(patientService.processingProvider(
            eq(labResultProxyContainer), eq(edxLabInformationDto), eq(container), eq(false)))
        .thenReturn(container);

    PersonAggContainer result =
        managerAggregationService.patientAggregation(
            labResultProxyContainer, edxLabInformationDto, personList);
  ...
  void testRoleAggregation_ScopingEntityUidNull_CountZero() {
    ...
    verify(labResult)
        .setTheRoleDtoCollection(
            argThat(list -> list.stream().anyMatch(r -> ((RoleDto) r).getRoleSeq() == 1)));
  }
                        ...
                        .anyMatch(
                            r ->
                                ((RoleDto) r).getRoleSeq() == 2
                                    && ((RoleDto) r)
                                        .getSubjectClassCd()
                                        .equals(EdxELRConstant.ELR_MAT_CD))));
  }

  @SuppressWarnings("java:S1117")
  @Test
  void testProgAndJurisdictionAggregationHelper_AssignsProgramAndJurisdiction()
    ...
    // Act
    managerAggregationService.progAndJurisdictionAggregationHelper(
        labResult, edxInfo, personAgg, orgContainer);

    // Assert
    verify(programAreaService)
        .getProgramArea(
            argThat(col -> col.contains(resultContainer)), eq(requestContainer), eq("CLIA123"));

    verify(jurisdictionService)
        .assignJurisdiction(
  ...
  void testServiceAggregation_ExecutesAllAggregationSteps() throws Exception {
    // Arrange
    ManagerAggregationService spyService =
        Mockito.spy(
            new ManagerAggregationService(
                organizationService,
                patientService,
                uidService,
                observationService,
    ...
    doNothing()
        ...
        .observationAggregation(
            eq(labResult), eq(edxLabInformationDto), eq(observationContainerCollection));
    doReturn(personAggContainer)
        .when(spyService)
        .patientAggregation(eq(labResult), eq(edxLabInformationDto), eq(personContainerCollection));
    when(organizationService.processingOrganization(eq(labResult)))
        .thenReturn(organizationContainer);
    ...
    doNothing()
        ...
        .progAndJurisdictionAggregationHelper(
            ...
            eq(personAggContainer),
            eq(organizationContainer));

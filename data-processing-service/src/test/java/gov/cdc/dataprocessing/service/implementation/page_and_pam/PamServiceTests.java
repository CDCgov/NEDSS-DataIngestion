...
class PamServiceTests {
  ...
  @InjectMocks private PamService pamService;
  ...
  void setPamProxyWithAutoAssoc_Success_PamIsNew() throws DataProcessingException {
    ...
    when(patientRepositoryUtil.loadPerson(101L)).thenReturn(perConn);
    ...
    when(patientRepositoryUtil.createPerson(any())).thenReturn(patObj);

    when(prepareAssocModelHelper.prepareVO(
            any(), eq("INVESTIGATION"), eq("INV_CR"), eq("PUBLIC_HEALTH_CASE"), eq("BASE"), eq(1)))
        .thenReturn(phcDt);
  ...
  void insertPamVO_Success() throws DataProcessingException {
    ...
    verify(answerService, times(1)).storeActEntityDTCollectionWithPublicHealthCase(any(), any());
  }
}

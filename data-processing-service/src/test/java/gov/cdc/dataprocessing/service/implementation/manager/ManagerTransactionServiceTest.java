...
class ManagerTransactionServiceTest {
  ...
  @Test
  void testProcessWithTransactionSeparation_WhenProcessingReturnsNull_SkipsHandling()
      throws Exception {
  ...
  @Test
  void testProcessWithTransactionSeparation_WhenProcessingReturnsValid_CallsHandling()
      throws Exception {
  ...
  @Test
  void testProcessWithTransactionSeparation_WhenProcessingThrows_ExceptionPropagated()
      throws Exception {
    when(managerService.processingELR(123, false))
        .thenThrow(new DataProcessingDBException("DB error"));

    assertThrows(
        DataProcessingDBException.class,
        () -> transactionService.processWithTransactionSeparation(123, false));
  ...
  @Test
  void testProcessWithTransactionSeparation_WhenHandlingThrows_ExceptionPropagated()
      throws Exception {
    PublicHealthCaseFlowContainer container = new PublicHealthCaseFlowContainer();
    when(managerService.processingELR(123, false)).thenReturn(container);
    doThrow(new EdxLogException("Handling failed", container))
        .when(managerService)
        .handlingWdsAndLab(container, false);

    assertThrows(
        EdxLogException.class,
        () -> transactionService.processWithTransactionSeparation(123, false));

...
class ProviderMatchingServiceTest {
  ...
  @InjectMocks private ProviderMatchingService providerMatchingService;
  ...
  void getMatchingProvider_local_id_entityMatch() throws DataProcessingException {
    ...
    EDXActivityDetailLogDto edxActivityDetailLogDtoResult =
        providerMatchingService.getMatchingProvider(personContainer);
    assertEquals(
        String.valueOf(edxEntityMatchingDT.getEntityUid()),
        edxActivityDetailLogDtoResult.getRecordId());
  }
  ...
  void getMatchingProvider_name_addr_with_empty_entity() throws DataProcessingException {
    ...
    EdxEntityMatchDto edxEntityMatchingDT =
        new EdxEntityMatchDto(); // entityUid is null (simulate no match)
    when(edxPatientMatchRepositoryUtil.getEdxEntityMatchOnMatchString(any(), any()))
        .thenReturn(edxEntityMatchingDT);
    ...
    doReturn("STREET_ADDR1 CITYDESCTXT STATE_CD ZIP_CD")
        .when(spyService)
        .nameAddressStreetOneProvider(any());
    doReturn(123L).when(spyService).processingProvider(any(), any(), any());
  ...
  void getMatchingProvider_telephone_empty_entity() throws DataProcessingException {
    ...
    ProviderMatchingService spyService = spy(providerMatchingService);
    doReturn("123 Main St")
        .when(spyService)
        .nameAddressStreetOneProvider(any()); // avoid nameAddr1 == null
  ...
  void attemptMatch_shouldPersistWhenMatchFoundAndRoleIsNull() throws DataProcessingException {
    ...
    when(edxPatientMatchRepositoryUtil.getEdxEntityMatchOnMatchString(
            NEDSSConstant.PRV, matchString))

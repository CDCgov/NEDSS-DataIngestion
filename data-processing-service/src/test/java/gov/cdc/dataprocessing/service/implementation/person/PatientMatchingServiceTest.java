...
class PatientMatchingServiceTest {
  ...
  @Mock private ObjectProvider<DeduplicationService> serviceProvider;
  ...
  void shouldPerformModernizedMatching() throws DataProcessingException {
    ...
    when(deduplicationService.match(Mockito.any(PersonMatchRequest.class)))
        .thenReturn(new MatchResponse(1l, MatchType.EXACT));
  ...
  void modernizedMatchingThrowsException() {
    ...
    patientMatchingService =
        new PatientMatchingService(
            ...
            true,
            serviceProvider);
    when(deduplicationService.match(Mockito.any(PersonMatchRequest.class))).thenReturn(null);
    PersonContainer container = new PersonContainer();
    ...
    assertThat(exception.getMessage())
        .isEqualTo("Null response returned from deduplication service");
    verify(deduplicationService, times(1)).match(Mockito.any());
  }

  @Test
  void tryMatchByLocalId_noLocalId() throws DataProcessingException {
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(null, null))
        .thenReturn(null);
  ...
  void tryMatchByLocalId_LocalId() throws DataProcessingException {
    ...
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(null, "LOCALID"))
        .thenReturn(null);
  ...
  @Test
  void tryMatchByLocalId_useIdentifierWhenLocalIdIsNull_butIdentifierIsNull()
  ...
  @Test
  void tryMatchByLocalId_useIdentifierWhenLocalIdIsNull_butIdentifierFirstElementIsEmpty()
      throws DataProcessingException {
    // Create a PersonContainer with no localId but with entity identifiers that result in empty
    // first element
    PersonContainer container = new PersonContainer();
    PersonDto personDto = new PersonDto();
    personDto.setCd("PAT");
  ...
  void tryMatchByIdentifier_noIdentifier() throws DataProcessingException {
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(null, null))
        .thenReturn(null);
    EdxPatientMatchDto matchDto =
        patientMatchingService.tryMatchByIdentifier(new PersonContainer());
    assertThat(matchDto).isNull();
  }
  ...
  void tryMatchByIdentifier_null() throws DataProcessingException {
    ...
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(
            null, "SSN-VALUE^SS^SSA^SOCIAL SECURITY ADMINISTRATION^IDTYPE^LASTNAME^FIRSTNAME"))
        .thenReturn(null);
  ...
  void tryMatchByIdentifier_multiMatch() throws DataProcessingException {
    ...
    container.setTheEntityIdDtoCollection(List.of(id1));
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(
            "PAT", "SSN-VALUE^SS^SSA^SOCIAL SECURITY ADMINISTRATION^IDTYPE^LASTNAME^FIRSTNAME"))
        .thenReturn(edxPatientMatchDto);
  ...
  void tryMatchByDemographics_noDemographics() throws DataProcessingException {
    ...
    EdxPatientMatchDto matchDto =
        patientMatchingService.tryMatchByDemographics(new PersonContainer());
    assertThat(matchDto).isNull();
  }
  ...
  void tryMatchByDemographics_null() throws DataProcessingException {
    ...
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(
            "PAT", "LASTNAME^FIRSTNAME^2024-12-17 12:00:00.0^U"))
        .thenReturn(null);
  ...
  void tryMatchingString_null() throws DataProcessingException {
    ...
    when(edxPatientMatchRepositoryUtil.getEdxPatientMatchOnMatchString(any(), any()))
        .thenReturn(null);
    // call test method
    EdxPatientMatchDto edxPatientMatchDtoResult =
  ...
  void getMatchingPatient_identifier() throws DataProcessingException {
    ...
    when(prepareAssocModelHelper.prepareVO(any(), any(), any(), any(), any(), any()))
        .thenReturn(mpr.getThePersonDto());

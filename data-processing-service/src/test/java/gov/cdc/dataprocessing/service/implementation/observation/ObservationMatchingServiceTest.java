...
class ObservationMatchingServiceTest {
  @Mock private ObservationMatchStoredProcRepository observationMatchStoredProcRepository;
  @Mock private ObservationRepository observationRepository;
  @InjectMocks private ObservationMatchingService observationMatchingService;
  ...
  @Test
  void checkingMatchObservation_Success_Matched_Exception_With_Accession()
    ...
    when(observationMatchStoredProcRepository.getMatchedObservation(edxLabInformationDto))
        .thenReturn(1L);
    ...
    assertEquals(
        "An Observation Lab test match was found for Accession # 123, but the activity time is out of sequence.",
        thrown.getMessage());
  }
  ...
  void processMatchedProxyVO_Success() {
    ...
    observationMatchingService.processMatchedProxyVO(
        labResultProxyVO, matchedlabResultProxyVO, edxLabInformationDT);
  ...
  void checkingMatchingObservation_Test_Null_2() throws DataProcessingException {
    ...
    when(observationMatchStoredProcRepository.getMatchedObservation(any())).thenReturn(10L);
  ...
  void testStatusCombination_NewVsNew_MatchSuccess() throws Exception {
    EdxLabInformationDto dto =
        createDto(EdxELRConstant.ELR_OBS_STATUS_CD_NEW, new Timestamp(System.currentTimeMillis()));
    ObservationDto matched =
        createMatchedObs("LOC1", new Timestamp(System.currentTimeMillis() - 1000));
  ...
  void testStatusCombination_NewVsNew_ActivityTimeOutOfSequence() throws DataProcessingException {
    EdxLabInformationDto dto =
        createDto(EdxELRConstant.ELR_OBS_STATUS_CD_NEW, new Timestamp(System.currentTimeMillis()));
    ObservationDto matched =
        createMatchedObs("LOC5", new Timestamp(System.currentTimeMillis() + 10000));
    ObservationMatchingService spy = spy(observationMatchingService);
    doReturn(matched).when(spy).matchingObservation(dto);
  ...
  void testStatusCombination_Invalid_ThrowsDefaultBranch() throws DataProcessingException {
    ...
    ObservationDto matched =
        createMatchedObs("LOC6", new Timestamp(System.currentTimeMillis() - 1000));
    ObservationMatchingService spy = spy(observationMatchingService);
    doReturn(matched).when(spy).matchingObservation(dto);
  ...
  @Test
  void testCheckingMatchingObservation_WhenMatchedObsIsNull_ReturnsNull()
      throws DataProcessingException {
    // Arrange
    EdxLabInformationDto dto = new EdxLabInformationDto();
  ...
  void testHandleInvalidCombination_SupercededVsCompleted() {
    ...
    DataProcessingException ex =
        assertThrows(
            DataProcessingException.class,
            () ->
                observationMatchingService.handleInvalidCombination(
                    odsStatus, msgStatus, dto, matchedObs, fillerNumber));

    assertTrue(dto.isFinalPostCorrected());
    assertEquals("LOC001", dto.getLocalId());
    assertTrue(
        ex.getMessage()
            .contains("Final report with Accession # ACC123 was sent after a corrected report"));
  }
  ...
  void testHandleInvalidCombination_CompletedVsNew() {
    ...
    assertTrue(
        ex.getMessage()
            .contains("Preliminary report with Accession # ACC456 was sent after a final report"));
  }

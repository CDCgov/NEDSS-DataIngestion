...
class InvestigationServiceTests {
  @Mock private EdxDocumentRepository edxDocumentRepository;
  @Mock private PublicHealthCaseRepositoryUtil publicHealthCaseRepositoryUtil;
  ...
  @Mock private ActRelationshipService actRelationshipService;
  @Mock private INotificationService notificationService;
  @Mock private IObservationSummaryService observationSummaryService;
  @Mock private QueryHelper queryHelper;
  @Mock private Observation_SummaryRepository observationSummaryRepository;
  @Mock private IContactSummaryService contactSummaryService;
  @Mock private CachingValueDpDpService cachingValueDpService;
  @Mock private ILdfService ldfService;
  @Mock private LabTestRepository labTestRepository;
  @InjectMocks private InvestigationService investigationService;
  @Mock private ICacheApiService cacheApiService;
  ...
  void setAssociations_Success() throws DataProcessingException {
    ...
    Collection<LabReportSummaryContainer> reportSumVOCollection =
        gson.fromJson(reportSumVOCollectionStr, listOfPersonsType);
    ...
    when(prepareAssocModelHelper.prepareAssocDTForActRelationship(any()))
        .thenReturn(actRelationshipDT);

    when(observationRepositoryUtil.loadObject(10006210L)).thenReturn(labProxyContainer);

    when(prepareAssocModelHelper.prepareVO(
            any(),
            eq("OBSERVATIONLABREPORT"),
            eq("OBS_LAB_ASC"),
            eq("OBSERVATION"),
            eq("BASE"),
            eq(1)))
        .thenReturn(labProxyContainer.getTheObservationDto());
    ...
    when(publicHealthCaseRepositoryUtil.getNonPersonLivingSubject(13L))
        .thenReturn(new NonPersonLivingSubjectDto());
    ...
    when(publicHealthCaseRepositoryUtil.getClinicalDocument(18L))
        .thenReturn(new ClinicalDocumentDto());
    when(publicHealthCaseRepositoryUtil.getReferral(19L)).thenReturn(new ReferralDto());
    when(publicHealthCaseRepositoryUtil.getPatientEncounter(20L))
        .thenReturn(new PatientEncounterDto());
    ...
    when(queryHelper.getDataAccessWhereClause(NBSBOLookup.OBSERVATIONLABREPORT, "VIEW", "obs"))
        .thenReturn("BLAH");
    when(queryHelper.getDataAccessWhereClause(NBSBOLookup.OBSERVATIONLABREPORT, "VIEW", ""))
        .thenReturn("BLAH");
    ...
    when(observationSummaryService.findAllActiveLabReportUidListForManage(10L, " AND BLAH"))
        .thenReturn(labReportUidSummarVOs);
    ...
    investigationService.setAssociations(
        investigationUid, reportSumVOCollection, null, null, null, true);
  ...
  void setObservationAssociationsImpl_Test_Success() throws DataProcessingException {
    ...
    investigationService.setObservationAssociationsImpl(
        investigationUid, reportSumVOCollection, true);
  ...
  void setObservationAssociationsImpl_Test_Success_NotAssociated() throws DataProcessingException {
    ...
    List<LabReportSummaryContainer> reportSumVOCollection =
        gson.fromJson(reportSumVOCollectionStr, listOfPersonsType);
  ...
  void getPageProxyVO_Success() throws DataProcessingException, ParseException {
    ...
    when(retrieveSummaryService.notificationSummaryOnInvestigation(any(), any()))
        .thenReturn(notiSumCol);
    ...
    when(observationSummaryService.findAllActiveLabReportUidListForManage(10006070L, " AND BLAH"))
  ...
  void processingNonAssociatedReportSummaryContainer_Success() throws DataProcessingException {
    ...
    var test =
        investigationService.processingNonAssociatedReportSummaryContainer(
            reportSumVO, odsDT, rootDT);
  ...
  void updateAutoResendNotifications_Success_InvestigationConn() {
    ...
    when(notificationService.getNotificationById(10L)).thenReturn(notification);
  ...
  @Mock InvestigationContainer invesCon;
    ...
    DataProcessingException exception =
        assertThrows(
            ...
            () -> {
              investigationService.setAssociations(
                  investigationUID,
                  reportSumVOCollection,
                  vaccinationSummaryVOCollection,
                  summaryDTColl,
                  treatmentSumColl,
                  isNNDResendCheckRequired);
            });
  ...
  void testProcessingInvestigationSummary_LiteCase() throws DataProcessingException {
    ...
    assertDoesNotThrow(
        () -> {
          investigationService.processingInvestigationSummary(
              investigationProxyVO, thePublicHealthCaseContainer, lite);
        });
  ...
  void testProcessingInvestigationSummary_NonLiteCase() throws DataProcessingException {
    ...
    investigationProxyVO
        ...
        .setTheActRelationshipDTCollection(actRelationshipDTCollection);

    when(retrieveSummaryService.notificationSummaryOnInvestigation(any(), any()))
        .thenReturn(notificationSummaryVOCollection);
  ...
  void populateDescTxtFromCachedValues_Test() throws DataProcessingException {
    ...
    labLst.add(labTs);
    when(labTestRepository.findLabTestByLabIdAndLabTestCode(any(), eq("TEST")))
        .thenReturn(Optional.of(labLst));

...
class EntityLocatorParticipationServiceTest {
  @Mock private DataModifierReposJdbc dataModifierReposJdbc;
  @Mock private EntityLocatorJdbcRepository entityLocatorJdbcRepository;
  @Mock private UidPoolManager uidPoolManager;

  @Mock private EntityLocatorParticipationRepository entityLocatorParticipationRepository;
  @Mock private TeleLocatorRepository teleLocatorRepository;
  @Mock private PostalLocatorRepository postalLocatorRepository;

  @Mock private PhysicalLocatorRepository physicalLocatorRepository;
  @Mock private IOdseIdGeneratorWCacheService iOdseIdGeneratorWCacheService;

  @Mock private PersonRepository personRepository;
  @InjectMocks private EntityLocatorParticipationService entityLocatorParticipationService;
  ...
  void tearDown() {
    Mockito.reset(
        entityLocatorParticipationRepository,
        teleLocatorRepository,
        postalLocatorRepository,
        physicalLocatorRepository,
        iOdseIdGeneratorWCacheService,
        authUtil,
        personRepository,
        dataModifierReposJdbc);
  }
  ...
  void updateEntityLocatorParticipation_Success() throws DataProcessingException {
    ...
    when(entityLocatorParticipationRepository.findByParentUid(10L))
        .thenReturn(Optional.of(entityPatCol));
    ...
    when(iOdseIdGeneratorWCacheService.getValidLocalUid(LocalIdClass.PERSON, true))
        .thenReturn(localUid);
  ...
  void testDeleteEntityLocatorParticipationWhenPersonResIsEmpty() {
    ...
    EntityLocatorParticipationDto dto =
        createEntityLocatorParticipationDto(NEDSSConstant.POSTAL, true, "BIR");
  ...
  void testDeleteEntityLocatorParticipationWhenPostalRevisionIsEmpty() {
    when(personRepository.findByPersonUid(anyLong()))
        .thenReturn(Optional.of(List.of(new Person())));
  ...
  void testDeleteEntityLocatorParticipationWhenEntityMprEntityResIsEmpty() {
    when(personRepository.findByPersonUid(anyLong()))
        .thenReturn(Optional.of(List.of(new Person())));
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(new PostalLocator())));
    when(entityLocatorParticipationRepository.findByParentUid(anyLong()))
  ...
  void testDeleteEntityLocatorParticipationWhenEntityMprResIsEmpty() {
    ...
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(new PostalLocator())));
    when(entityLocatorParticipationRepository.findByParentUid(anyLong()))
        .thenReturn(Optional.of(List.of(new EntityLocatorParticipation())));
    when(entityLocatorParticipationRepository.findLocatorUidsByEntityUid(anyLong()))
        .thenReturn(Optional.empty());
  ...
  private EntityLocatorParticipationDto createEntityLocatorParticipationDto(
      String classCd, boolean itDelete, String useCd) {
  ...
  void testDeleteEntityLocatorParticipationWhenConditionsAreMet() {
    ...
    when(entityLocatorParticipationRepository.findByParentUid(anyLong()))
        .thenReturn(Optional.of(List.of(new EntityLocatorParticipation())));
    when(entityLocatorParticipationRepository.findLocatorUidsByEntityUid(anyLong()))
        .thenReturn(Optional.of(List.of(1L)));
  ...
  void testDeleteEntityLocatorParticipationWhenExceptionThrown() {
    ...
    assertThrows(
        ...
        () ->
            entityLocatorParticipationService.deleteEntityLocatorParticipation(
                locatorCollectionMock, 1L));
  ...
  void testDeleteEntityLocatorParticipationWithSpecificScenario() {
    ...
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(postalLocator)));
    when(entityLocatorJdbcRepository.findByEntityUid(parentUid)).thenReturn(List.of(entityLocator));
    when(entityLocatorParticipationRepository.findLocatorUidsByEntityUid(parentUid))
        .thenReturn(Optional.of(List.of(1L)));
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(postalLocator)));

    entityLocatorParticipationService.deleteEntityLocatorParticipation(
        locatorCollectionMock, patientUid);
  ...
  void testDeleteEntityLocatorParticipationWhenExceptionThrownInDeleteOperations() {
    ...
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(postalLocator)));
    when(entityLocatorParticipationRepository.findByParentUid(parentUid))
        .thenReturn(Optional.of(List.of(entityLocator)));
    when(entityLocatorParticipationRepository.findLocatorUidsByEntityUid(parentUid))
        .thenReturn(Optional.of(List.of(1L)));
    when(postalLocatorRepository.findByPostalLocatorUids(anyList()))
        .thenReturn(Optional.of(List.of(postalLocator)));
  ...
  void testDeleteEntityLocatorParticipationWhenElseConditionIsMet() {
    ...
    when(entityLocatorParticipationRepository.findByParentUid(parentUid))
        .thenReturn(Optional.of(List.of(entityLocator, entityLocator)));

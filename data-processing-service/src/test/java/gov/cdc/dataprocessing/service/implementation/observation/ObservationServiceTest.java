...
class ObservationServiceTest {
  @Mock private INNDActivityLogService nndActivityLogService;
  @Mock private IMessageLogService messageLogService;
  @Mock private ObservationRepositoryUtil observationRepositoryUtil;
  ...
  @Mock private IActRelationshipService actRelationshipService;
  @Mock private IEdxDocumentService edxDocumentService;
  @Mock private IAnswerService answerService;
  @Mock private IParticipationService participationService;
  @Mock private ObservationRepository observationRepository;
  @Mock private PersonRepository personRepository;
  @Mock private IJurisdictionService jurisdictionService;
  @Mock private OrganizationRepositoryUtil organizationRepositoryUtil;
  @Mock private IObservationCodeService observationCodeService;
  @Mock private ObservationUtil observationUtil;
  @Mock private PersonUtil personUtil;
  @Mock private IProgramAreaService programAreaService;
  @Mock private PrepareAssocModelHelper prepareAssocModelHelper;
  @Mock private IUidService uidService;

  @Mock private IInvestigationService investigationService;
  ...
  void tearDown() {
    Mockito.reset(
        nndActivityLogService,
        messageLogService,
        observationRepositoryUtil,
        notificationService,
        materialService,
        patientRepositoryUtil,
        roleService,
        actRelationshipService,
        edxDocumentService,
        answerService,
        ...
        personRepository,
        jurisdictionService,
        organizationRepositoryUtil,
        observationCodeService,
        observationUtil,
        personUtil,
        programAreaService,
  ...
  void getObservationToLabResultContainer_ObservationUidIsNull_ShouldThrowException() {
    ...
    assertEquals(
        "HL7CommonLabUtil.getLabResultToProxy observationUid is null", thrown.getMessage());
  }
  ...
  void getObservationToLabResultContainer_ReturnData_ELRisTrue() throws DataProcessingException {
    ...
    when(patientRepositoryUtil.loadPerson(partCol.get(0).getSubjectEntityUid()))
        .thenReturn(patContainer);
    ...
    when(patientRepositoryUtil.loadPerson(roleDtColl.get(0).getSubjectEntityUid()))
        .thenReturn(scopedPerson);
    ...
    when(organizationRepositoryUtil.loadObject(
            act108pat.getSubjectEntityUid(), act108pat.getActUid()))
        .thenReturn(orgContainer);
    ...
    when(observationRepositoryUtil.loadObject(act108Act.getSourceActUid()))
        .thenReturn(act108ActObsContainer);

    // retrieveReflexRTsAkaObservationFromActRelationship
    when(observationRepositoryUtil.loadObject(act108ActObsContainerAct.getSourceActUid()))
        .thenReturn(new ObservationContainer());
  ...
  void processingLabResultContainer_Success() throws DataProcessingException {
    ...
    when(personUtil.processLabPersonContainerCollection(
            labResult.getThePersonContainerCollection(), false, labResult))
        .thenReturn(1L);
    ...
    when(jurisdictionService.deriveJurisdictionCd(labResult, orderTest.getTheObservationDto()))
        .thenReturn(null);
  ...
  void setLabInvAssociation_Success() throws DataProcessingException {
    ...
    doNothing()
        .when(investigationService)
        .setAssociations(
            eq(investigationUid), anyCollection(), eq(null), eq(null), eq(null), eq(true));
  ...
  @Test
  void testProcessObservationWithProcessingDecision_NotLabResultCode()
    ...
    DataProcessingException exception =
        assertThrows(
            DataProcessingException.class,
            () ->
                observationService.processObservationWithProcessingDecision(
                    observationUid, processingDecisionCd, processingDecisionTxt));
  ...
  @Test
  void testProcessObservationWithProcessingDecision_NotUnprocessed()
      throws DataProcessingException {
    Long observationUid = 1L;
    String processingDecisionCd = "DECISION_CD";
    String processingDecisionTxt = "DECISION_TXT";
  ...
  void testProcessObservationWithProcessingDecision_Exception() throws DataProcessingException {
    ...
    when(observationRepositoryUtil.loadObject(observationUid))
  ...
  @Test
  void testPerformOrderTestStateTransition_NewWithProcessingDecision()
      throws DataProcessingException {
    ...
    verify(prepareAssocModelHelper).prepareVO(any(), any(), any(), any(), any(), any());
  }
  ...
  void testProcessingOrderTestStateTransition_NewWithProcessingDecision() {
    ...
    String result =
        observationService.processingOrderTestStateTransition(
            labResultProxyVO, orderTest, null, false);
  ...
  void testProcessingOrderTestStateTransition_DirtyAndIsELR() {
    ...
    String result =
        observationService.processingOrderTestStateTransition(
            labResultProxyVO, orderTest, null, true);
  ...
  void testProcessingOrderTestStateTransition_NoConditionsMet() {
    ...
    String result =
        observationService.processingOrderTestStateTransition(
            labResultProxyVO, orderTest, businessTriggerCd, false);
  ...
  void testRetrievePersonAndRoleFromParticipation_Continue() {
    ...
    Map<DataProcessingMapKey, Object> result =
        observationService.retrievePersonAndRoleFromParticipation(partColl);
  ...
  void testProcessingNotELRLab_isELRTrue() throws DataProcessingException {
    ...
    verifyNoInteractions(
        notificationService, actRelationshipService, edxDocumentService, observationCodeService);
  }
  ...
  void testProcessingNotELRLab_isELRFalse_NoExistingNotification() throws DataProcessingException {
    ...
    when(actRelationshipService.loadActRelationshipBySrcIdAndTypeCode(
            observationId, NEDSSConstant.LAB_REPORT))
        .thenReturn(actRelationshipDtos);

    Collection<EDXDocumentDto> edxDocumentDtos = new ArrayList<>();
    when(edxDocumentService.selectEdxDocumentCollectionByActUid(observationId))
        .thenReturn(edxDocumentDtos);

    ArrayList<String> conditionList = new ArrayList<>();
    when(observationCodeService.deriveTheConditionCodeList(lrProxyVO, orderedTest))
        .thenReturn(conditionList);

    observationService.processingNotELRLab(isELR, lrProxyVO, orderedTest, observationId);

    verify(notificationService).checkForExistingNotification(lrProxyVO);
    verify(actRelationshipService)
        .loadActRelationshipBySrcIdAndTypeCode(observationId, NEDSSConstant.LAB_REPORT);
  ...
  void testLoadingObservationToLabResultContainerActHelper_AllCases() {
    ...
    observationService.loadingObservationToLabResultContainerActHelper(
        lrProxyVO, isELR, allAct, orderedTest);

    assertEquals(
        1,
        ((Collection<ObservationContainer>) allAct.get(DataProcessingMapKey.OBSERVATION)).size());
    assertEquals(
        ((Collection<ObservationContainer>) allAct.get(DataProcessingMapKey.OBSERVATION)).size(),
        lrProxyVO.getTheObservationContainerCollection().size());
  }
  ...
  void testUpdateLabResultWithAutoResendNotification_Exception() throws DataProcessingException {
    ...
    NNDActivityLogDto nndActivityLogDto =
        observationService.updateLabResultWithAutoResendNotification(labResultProxyVO);
    ...
    assertEquals(
        "java.lang.RuntimeException: Test Exception", nndActivityLogDto.getErrorMessageTxt());
  }

  @Test
  void testUpdateLabResultWithAutoResendNotification_NoNotification()
      throws DataProcessingException {
    LabResultProxyContainer labResultProxyVO = new LabResultProxyContainer();
    labResultProxyVO.associatedNotificationInd = false;

    NNDActivityLogDto result =
        observationService.updateLabResultWithAutoResendNotification(labResultProxyVO);
  ...
  @Test
  void testUpdateLabResultWithAutoResendNotification_ExceptionWithNullLocalId()
      throws DataProcessingException {
    LabResultProxyContainer labResultProxyVO = new LabResultProxyContainer();
    labResultProxyVO.associatedNotificationInd = true;
  ...
  void testExtractObservationCollection_whenNotLabResultProxy_returnsNull() {
    // Arrange
    BaseContainer proxyVO =
        new BaseContainer() {}; // anonymous subclass, not LabResultProxyContainer

    // Act
    Collection<ObservationContainer> result =
        observationService.extractObservationCollection(proxyVO);
  ...
  @Test
  void testStoreObservationVOCollection_whenExtractedCollectionIsNull_returnsNull()
      throws DataProcessingException {
    // Arrange
  ...
  @Test
  void testStoreObservationVOCollection_whenExtractedCollectionIsEmpty_returnsNull()
      throws DataProcessingException {
    // Arrange
  ...
  void testHandleJurisdictionCode_codeIsAnyJurisdiction_addsError() throws DataProcessingException {
    ...
    assertEquals(
        ProgramAreaJurisdiction.ANY_JURISDICTION,
        errors.get(NEDSSConstant.SETLAB_RETURN_JURISDICTION_ERRORS));
  }

  @Test
  void testHandleJurisdictionCode_codeIsJurisdictionNone_addsError()
      throws DataProcessingException {
    // Arrange
    ObservationDto dto = new ObservationDto();
    ...
    assertTrue(errors.containsKey(NEDSSConstant.SETLAB_RETURN_JURISDICTION_ERRORS));
    assertEquals(
        ProgramAreaJurisdiction.JURISDICTION_NONE,
        errors.get(NEDSSConstant.SETLAB_RETURN_JURISDICTION_ERRORS));
  }

  @Test
  void testHandleJurisdictionCode_codeIsAnyJurisdiction_butNoErrorReturned_doesNotAddToMap()
      throws DataProcessingException {
    // Arrange
    ObservationDto dto = new ObservationDto();
    dto.setJurisdictionCd(ProgramAreaJurisdiction.ANY_JURISDICTION);
  ...
  @Test
  void testHandleProgramAreaCode_whenPaCdIsNotAnyProgramArea_doesNothing()
      throws DataProcessingException {
    // Arrange
    ObservationDto dto = new ObservationDto();
  ...
  @Test
  void testHandleProgramAreaCode_whenDerivedErrorIsNull_doesNothing()
      throws DataProcessingException {
    // Arrange
    ObservationDto dto = new ObservationDto();
  ...
  @Test
  void testHandleProgramAreaCode_whenDerivedErrorIsPresent_addsToErrors()
      throws DataProcessingException {
    // Arrange
    ObservationDto dto = new ObservationDto();
    dto.setProgAreaCd(ProgramAreaJurisdiction.ANY_PROGRAM_AREA);
  ...
  void testRetrieveEntityFromParticipationForContainer_WhenNoPersonOrRoleKeys() {
    ...
    // Act
    Map<DataProcessingMapKey, Object> result =
        spyService.retrieveEntityFromParticipationForContainer(partColl);
  ...
  void testResultedTestObservation_ContinueWhenReflexObsEmpty() throws Exception {
    ...
    doReturn(null).when(spyService).retrieveReflexObservationsFromActRelationship(any());

    var result = spyService.retrieveObservationFromActRelationship(List.of(dto));

    assertEquals(
        1, ((Collection<?>) result.get(DataProcessingMapKey.OBSERVATION)).size()); // only RT
    assertTrue(((Collection<?>) result.get(DataProcessingMapKey.ORGANIZATION)).isEmpty());
  }
  ...
  void testResultedTestObservation_ContinueWhenReflexObsIsEmpty() throws Exception {
    ...
    assertEquals(
        1, ((Collection<?>) result.get(DataProcessingMapKey.OBSERVATION)).size()); // only RT added
  }
  ...
  void testContinue_WhenActRelDtoIsNull() throws Exception {
    ...
    Collection<ObservationContainer> result =
        spyService.retrieveReflexObservationsFromActRelationship(input);
  ...
  void testContinue_WhenReflexObservationIsNull() throws Exception {
    ...
    Collection<ObservationContainer> result =
        spyService.retrieveReflexObservationsFromActRelationship(List.of(dto));
  ...
  void testAct109Condition(
      String typeCd, String srcCls, String tgtCls, String recStatus, boolean shouldMatch)
      throws Exception {
    ActRelationshipDto dto = new ActRelationshipDto();
    dto.setTypeCd(typeCd);
    dto.setSourceClassCd(srcCls);
  ...
  void testRetrieveReflexRTsAkaObservationFromActRelationship_ElseCases(
      String typeCd, String sourceClassCd, String targetClassCd, String recordStatusCd)
      throws Exception {
    ActRelationshipDto dto = new ActRelationshipDto();
    dto.setTypeCd(typeCd);
    ...
    ObservationService spyService = Mockito.spy(observationService);
    doReturn(mock(ObservationContainer.class))
        .when(spyService)
  ...
  void testContinueWhenPartDtoIsNull() {
    ...
    OrganizationContainer result =
        spyService.retrievePerformingLabAkaOrganizationFromParticipation(input);
  ...
  void testElseCasesForPerformingLab(
      String typeCd, String subjectClassCd, String actClassCd, String recordStatusCd) {
    ParticipationDto dto = new ParticipationDto();
    dto.setTypeCd(typeCd);
    ...
    OrganizationContainer result =
        spyService.retrievePerformingLabAkaOrganizationFromParticipation(List.of(dto));
  ...
  void testElseCasesForRetrieveIntervention(
      String sourceClassCd, String targetClassCd, String recordStatusCd) throws Exception {
    ActRelationshipDto dto = new ActRelationshipDto();
    dto.setSourceClassCd(sourceClassCd);
    dto.setTargetClassCd(targetClassCd);
    dto.setRecordStatusCd(recordStatusCd);
  ...
  void testProcessingNotELRLab_ElseCasesForAllConditions() throws Exception {
    ...
    // Force else cases:
    when(actRelationshipService.loadActRelationshipBySrcIdAndTypeCode(
            obsId, NEDSSConstant.LAB_REPORT))
  ...
  void testProcessObservations_WhenMprUidNegativeAndNotFound_Throws() throws Exception {
    ...
    Map<Object, Object> mockObsResults = new HashMap<>();
    mockObsResults.put(
        NEDSSConstant.SETLAB_RETURN_OBS_UID, 999L); // Make sure this matches real constant
    ...
    when(participationService.findPatientMprUidByObservationUid(
            eq(NEDSSConstant.PERSON), eq(NEDSSConstant.PAR110_TYP_CD), eq(999L)))
  ...
  @Test
  void testProcessLabReportObsContainerCollection_WhenElectronicIndNotYes_SetsManualFlag()
      throws Exception {
    // Arrange
    LabResultProxyContainer proxy = new LabResultProxyContainer();
  ...
  @Test
  void testProcessLabReportObsContainerCollection_WhenObservationIsNull_ContinuesLoop()
      throws Exception {
    LabResultProxyContainer proxy = new LabResultProxyContainer();
  ...
  @Test
  void
      testLoadingObservationToLabResultContainerActHelper_WhenObsCollectionIsNull_InitializesNewList() {
    // Arrange
    LabResultProxyContainer proxy = new LabResultProxyContainer();
  ...
  @Test
  void testLoadingObservationToLabResultContainer_WhenAnswerServiceThrowsException_LogsError()
      throws Exception {
    ...
    doThrow(new RuntimeException("Simulated failure"))
        .when(answerService)
        .getNbsAnswerAndAssociation(observationId);

    // Act
    LabResultProxyContainer result =
        spyService.loadingObservationToLabResultContainer(observationId, isELR);

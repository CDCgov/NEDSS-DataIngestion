...
class NotificationRepositoryUtilTest {
  @Mock private NotificationJdbcRepository notificationRepository;
  ...
  @InjectMocks private NotificationRepositoryUtil notificationRepositoryUtil;
  ...
  void getNotificationContainer_Test() {
    ...
    when(actLocatorParticipationRepositoryUtil.getActLocatorParticipationCollection(uid))
        .thenReturn(actLocCol);

    var actReCol = new ArrayList<ActRelationshipDto>();
    var actRe = new ActRelationshipDto();
    actReCol.add(actRe);
    when(actRelationshipRepositoryUtil.getActRelationshipCollectionFromSourceId(uid))
        .thenReturn(actReCol);
  ...
  void setNotification_Test_2() throws DataProcessingException {
    ...
    when(notificationRepository.findById(any()))
        .thenReturn(new Notification(notificationContainer.getTheNotificationDT()));

...
class DataModifierReposJdbcTest {
  ...
  void testUpdatePersonNameStatus() {
    ...
    verify(jdbcTemplate)
        .update(
            eq(
                "UPDATE Person_name SET status_cd = 'I' WHERE person_uid = ? AND person_name_seq = ?"),
            eq(123L),
            eq(1));
  }
  ...
  void testDeleteEntityIdAndSeq() {
    ...
    verify(jdbcTemplate)
        .update(
            eq("DELETE FROM Entity_id WHERE entity_uid = ? AND entity_id_seq = ?"),
            eq(111L),
            eq(2));
  }
  ...
  void testDeleteByPatientUidAndMatchStringNotLike() {
    ...
    verify(jdbcTemplate)
        .update(
            eq(
                "DELETE FROM EDX_patient_match WHERE Patient_uid = ? AND match_string NOT LIKE 'LR^%'"),
            eq(222L));
  }
  ...
  void testDeletePersonRaceByUidAndCode() {
    ...
    verify(jdbcTemplate)
        .update(
            eq("DELETE FROM Person_race WHERE person_uid = ? AND race_cd = ?"), eq(333L), eq("A1"));
  }
  ...
  void testDeletePersonRaceByUid() {
    ...
    verify(jdbcTemplate)
        .update(
            startsWith("DELETE FROM Person_race WHERE person_uid = ? AND race_cd NOT IN ("),
            captor.capture());
  ...
  void testUpdateExistingPersonEdxIndByUid() {
    ...
    verify(jdbcTemplate)
        .update(eq("UPDATE Person SET edx_ind = 'Y' WHERE person_uid = ?"), eq(555L));
    assert result == 1;
  }
  ...
  void testDeletePostalLocatorById() {
    ...
    verify(jdbcTemplate)
        .update(eq("DELETE FROM Postal_locator WHERE postal_locator_uid = ?"), eq(666L));
  }
  ...
  void testDeleteActRelationshipByPk() {
    ...
    verify(jdbcTemplate)
        .update(
            eq(
                "DELETE FROM Act_relationship WHERE target_act_uid = ? AND source_act_uid = ? AND type_cd = ?"),
            eq(777L),
            eq(888L),
            eq("T1"));
  }
  ...
  void testDeleteLocatorById() {
    ...
    verify(jdbcTemplate)
        .update(
            eq("DELETE FROM Entity_locator_participation WHERE entity_uid = ? AND locator_uid = ?"),
            eq(999L),
            eq(1000L));
  }
  ...
  void testDeleteParticipationByPk() {
    ...
    verify(jdbcTemplate)
        .update(
            eq(
                "DELETE FROM Participation WHERE subject_entity_uid = ? AND act_uid = ? AND type_cd = ?"),
            eq(2000L),
            eq(3000L),
            eq("X1"));
  }
  ...
  void testDeleteRoleByPk() {
    ...
    verify(jdbcTemplate)
        .update(
            eq("DELETE FROM Role WHERE subject_entity_uid = ? AND cd = ? AND role_seq = ?"),
            eq(4000L),
            eq("Y2"),
            eq(5000L));
  }

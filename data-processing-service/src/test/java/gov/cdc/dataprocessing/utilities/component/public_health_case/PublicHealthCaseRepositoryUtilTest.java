...
class PublicHealthCaseRepositoryUtilTest {
  @Mock private PublicHealthCaseJdbcRepository publicHealthCaseRepository;
  @Mock private SupportForPhcJdbcRepository supportForPhcJdbcRepository;
  @Mock private IOdseIdGeneratorWCacheService odseIdGeneratorService;
  @Mock private ActJdbcRepository actRepository;
  @Mock private ActIdJdbcRepository actIdRepository;
  @Mock private ConfirmationMethodJdbcRepository confirmationMethodRepository;
  @Mock private ActLocatorParticipationJdbcRepository actLocatorParticipationRepository;
  @Mock private CaseManagementJdbcRepository caseManagementRepository;
  @Mock private ConfirmationMethodRepositoryUtil confirmationMethodRepositoryUtil;
  @Mock private CaseManagementRepositoryUtil caseManagementRepositoryUtil;
  @Mock private ActIdRepositoryUtil actIdRepositoryUtil;
  @Mock private ActLocatorParticipationRepositoryUtil actLocatorParticipationRepositoryUtil;
  @Mock private ActRelationshipRepositoryUtil actRelationshipRepositoryUtil;
  @Mock private ParticipationRepositoryUtil participationRepositoryUtil;
  @Mock private NbsCaseAnswerJdbcRepository nbsCaseAnswerRepository;
  @Mock private NbsActJdbcRepository actEntityRepository;
  @InjectMocks private PublicHealthCaseRepositoryUtil publicHealthCaseRepositoryUtil;
  @Mock AuthUtil authUtil;

  @Mock UidPoolManager uidPoolManager;

  @Mock ServicePropertiesProvider servicePropertiesProvider;
  ...
  void setUp() throws DataProcessingException {
    ...
    when(servicePropertiesProvider.getNbsStateCode()).thenReturn("13");
  ...
  void tearDown() {
    Mockito.reset(
        publicHealthCaseRepository,
        authUtil,
        odseIdGeneratorService,
        actRepository,
        actIdRepository,
        confirmationMethodRepository,
        caseManagementRepository,
        confirmationMethodRepositoryUtil,
        caseManagementRepositoryUtil,
        actIdRepositoryUtil,
        actLocatorParticipationRepositoryUtil,
        actRelationshipRepositoryUtil,
        participationRepositoryUtil,
        nbsCaseAnswerRepository,
        actEntityRepository,
        uidPoolManager,
  ...
  void loadObject_Test() throws DataProcessingException {
    ...
    when(confirmationMethodRepositoryUtil.getConfirmationMethodByPhc(phcUid))
        .thenReturn(new ArrayList<>());
    when(caseManagementRepositoryUtil.getCaseManagementPhc(phcUid))
        .thenReturn(new CaseManagementDto());
    when(actIdRepositoryUtil.getActIdCollection(phcUid)).thenReturn(new ArrayList<>());
    when(actLocatorParticipationRepositoryUtil.getActLocatorParticipationCollection(phcUid))
        .thenReturn(new ArrayList<>());
    when(actRelationshipRepositoryUtil.selectActRelationshipDTCollectionFromActUid(phcUid))
        .thenReturn(new ArrayList<>());
    when(participationRepositoryUtil.getParticipationsByActUid(phcUid))
        .thenReturn(new ArrayList<>());
  ...
  void getNonPersonLivingSubject_Test() {
    var uid = 10L;
    when(supportForPhcJdbcRepository.findNonPersonLivingSubjectById(uid))
        .thenReturn(new NonPersonLivingSubject());
  ...
  void getClinicalDocument_Test() {
    var uid = 10L;
    when(supportForPhcJdbcRepository.findClinicalDocumentById(uid))
        .thenReturn(new ClinicalDocument());
  ...
  void getPatientEncounter_Test() {
    var uid = 10L;
    when(supportForPhcJdbcRepository.findPatientEncounterById(uid))
        .thenReturn(new PatientEncounter());
  ...
  @Test
  void updateCaseManagementWithEPIIDandFRNum_BothNull_ShouldSetBoth()
      throws DataProcessingException {
    // Arrange
    CaseManagementDto caseManagementDto = new CaseManagementDto();
    caseManagementDto.setEpiLinkId(null);
    caseManagementDto.setFieldRecordNumber(null);

    // Mock UID generation
    var localId = new LocalUidModel();
    var classTypeUid = new LocalUidGeneratorDto();
    classTypeUid.setUidPrefixCd("EP");
    classTypeUid.setSeedValueNbr(12345L);
    classTypeUid.setUidSuffixCd("LK");
    localId.setClassTypeUid(classTypeUid);

    when(uidPoolManager.getNextUid(EPILINK, false)).thenReturn(localId);
    when(servicePropertiesProvider.getNbsStateCode()).thenReturn("13");
    ...
    // Assert
    assertNotNull(caseManagementDto.getEpiLinkId());
    assertNotNull(caseManagementDto.getFieldRecordNumber());
    assertEquals(caseManagementDto.getEpiLinkId(), caseManagementDto.getFieldRecordNumber());

    // Verify the generated value format: stateCode + epiLinkId.substring(2, length-2) +
    // twoDigitYear
    String expectedValue = "13" + "12345" + getCurrentTwoDigitYear();
    assertEquals(expectedValue, caseManagementDto.getEpiLinkId());
    assertEquals(expectedValue, caseManagementDto.getFieldRecordNumber());

    verify(uidPoolManager, times(1)).getNextUid(EPILINK, false);
    verify(servicePropertiesProvider, times(1)).getNbsStateCode();
  }

  @Test
  void
      updateCaseManagementWithEPIIDandFRNum_EpiLinkIdNotNullFieldRecordNumberNull_ShouldSetOnlyFieldRecordNumber()
          throws DataProcessingException {
    // Arrange
    CaseManagementDto caseManagementDto = new CaseManagementDto();
    caseManagementDto.setEpiLinkId("EXISTING_EPI_LINK_ID");
    caseManagementDto.setFieldRecordNumber(null);
    ...
    var classTypeUid = new LocalUidGeneratorDto();
    classTypeUid.setUidPrefixCd("EP");
    classTypeUid.setSeedValueNbr(67890L);
    classTypeUid.setUidSuffixCd("LK");
    localId.setClassTypeUid(classTypeUid);
    ...
    // Assert
    assertEquals(
        "EXISTING_EPI_LINK_ID", caseManagementDto.getEpiLinkId()); // Should remain unchanged
    assertNotNull(caseManagementDto.getFieldRecordNumber());

    // Verify the generated field record number format
    String expectedFieldRecordNumber = "13" + "67890" + getCurrentTwoDigitYear();
    assertEquals(expectedFieldRecordNumber, caseManagementDto.getFieldRecordNumber());
  ...
  @Test
  void updateCaseManagementWithEPIIDandFRNum_BothNotNull_ShouldNotChange()
      throws DataProcessingException {
    // Arrange
    CaseManagementDto caseManagementDto = new CaseManagementDto();
    caseManagementDto.setEpiLinkId("EXISTING_EPI_LINK_ID");
    caseManagementDto.setFieldRecordNumber("EXISTING_FIELD_RECORD_NUMBER");

    // Act
    publicHealthCaseRepositoryUtil.updateCaseManagementWithEPIIDandFRNum(caseManagementDto);

    // Assert
    assertEquals("EXISTING_EPI_LINK_ID", caseManagementDto.getEpiLinkId());
    assertEquals("EXISTING_FIELD_RECORD_NUMBER", caseManagementDto.getFieldRecordNumber());

    // Verify no UID generation or state code calls
    verify(uidPoolManager, never()).getNextUid(any(), anyBoolean());
    verify(servicePropertiesProvider, never()).getNbsStateCode();
  }

  @Test
  void updateCaseManagementWithEPIIDandFRNum_EpiLinkIdNullFieldRecordNumberNotNull_ShouldNotChange()
      throws DataProcessingException {
    // Arrange
    CaseManagementDto caseManagementDto = new CaseManagementDto();
    caseManagementDto.setEpiLinkId(null);
    ...
    // Assert
    assertNull(caseManagementDto.getEpiLinkId());
    assertEquals("EXISTING_FIELD_RECORD_NUMBER", caseManagementDto.getFieldRecordNumber());
  ...
  /** Helper method to get current two-digit year */
  private String getCurrentTwoDigitYear() {

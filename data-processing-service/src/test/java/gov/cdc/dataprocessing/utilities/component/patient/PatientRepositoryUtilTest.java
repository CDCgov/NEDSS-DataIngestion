...
class PatientRepositoryUtilTest {
  @Mock private PersonJdbcRepository personRepository;
  @Mock private EntityRepositoryUtil entityRepositoryUtil;
  ...
  @Mock private IEntityLocatorParticipationService entityLocatorParticipationService;
  @InjectMocks private PatientRepositoryUtil patientRepositoryUtil;
  ...
  void createPerson_Test() throws DataProcessingException {
    ...
    verify(entityLocatorParticipationService, times(1))
        .createEntityLocatorParticipation(any(), any());
  }
  ...
  void updateExistingPerson_Test() throws DataProcessingException {
    ...
    when(personRepository.selectByPersonUid(any()))
        .thenReturn(new Person(perCon.getThePersonDto(), "UTC"));
  ...
  void deleteInactivePersonRace_Test_Exp() {
    ...
    doThrow(new RuntimeException("TEST"))
        .when(dataModifierReposJdbc)
        .deletePersonRaceByUid(eq(10L), any());
  ...
  private void mockUidPool(long uidSeed, String prefix, long mid, String suffix)
  ...
  void testCreatePerson_withEachSubEntityCollection() throws Exception {
    ...
    container.setTheEntityLocatorParticipationDtoCollection(
        List.of(new EntityLocatorParticipationDto()));
  ...
  void testUpdateExistingPerson_withCollections() throws Exception {
    ...
    verify(personRepository).updatePerson(any(Person.class));
    verify(entityLocatorParticipationService)
        .updateEntityLocatorParticipation(anyList(), anyLong());
  }
  ...
  void testLoadPerson_personNotFound() {
    ...
    when(personRepository.findPersonEthnicByPersonUid(personUid))
        .thenReturn(Collections.emptyList());
    when(entityIdRepository.findEntityIds(personUid)).thenReturn(Collections.emptyList());
    when(entityLocatorParticipationService.findEntityLocatorById(personUid))
        .thenReturn(Collections.emptyList());
  ...
  void testLoadPerson_someCollectionsPopulated() {
    ...
    when(personRepository.findByPersonUid(personUid)).thenReturn(new Person());
    when(personRepository.findPersonNameByPersonUid(personUid))
        .thenReturn(List.of(new PersonName()));
    when(personRepository.findPersonRaceByPersonUid(personUid))
        .thenReturn(List.of(new PersonRace()));
    when(personRepository.findPersonEthnicByPersonUid(personUid))
        .thenReturn(Collections.emptyList());
    when(entityIdRepository.findEntityIds(personUid)).thenReturn(Collections.emptyList());
    when(entityLocatorParticipationService.findEntityLocatorById(personUid))
  ...
  void testLoadPerson_allCollectionsHaveOneItem() {
    ...
    when(personRepository.findPersonRaceByPersonUid(personUid))
        .thenReturn(List.of(new PersonRace()));
    when(personRepository.findPersonEthnicByPersonUid(personUid))
        .thenReturn(List.of(new PersonEthnicGroup()));
    when(entityIdRepository.findEntityIds(personUid)).thenReturn(List.of(new EntityId()));
    when(entityLocatorParticipationService.findEntityLocatorById(personUid))
        .thenReturn(List.of(new EntityLocatorParticipation()));
  ...
  void testUpdatePersonName_exceptionDuringProcessing() {
    ...
    doThrow(new RuntimeException("DB error"))
        .when(dataModifierReposJdbc)
        .updatePersonNameStatus(anyLong(), anyInt());
  ...
  void testUpdateEntityId_deletePath_withException() {
    ...
    doThrow(new RuntimeException("delete error"))
        .when(dataModifierReposJdbc)
        .deleteEntityIdAndSeq(anyLong(), anyInt());
  ...
  void testUpdatePersonRace_whenDeleteThrowsException_logsError() {
    ...
    doThrow(new RuntimeException("Simulated deletion failure"))
        .when(dataModifierReposJdbc)
        .deletePersonRaceByUidAndCode(1L, "2054-5");

...
class ORCHandlerTest {
  ...
  @InjectMocks private ORCHandler orcHandler;
  ...
  void getORCProcessing_Test() throws DataProcessingException {
    ...
    when(nbsObjectConverter.orgTelePhoneType(any(), any(), any()))
        .thenReturn(new EntityLocatorParticipationDto());

    commonOrder.setOrderEffectiveDateTime(new HL7TSType());

    when(nbsObjectConverter.processHL7TSType(any(), any()))
        ...
        assertThrows(
            ...
            () -> {
              orcHandler.getORCProcessing(
                  commonOrder, labResultProxyContainer, edxLabInformationDto);
            });
  ...
  void getORCProcessing_exp_3() throws DataProcessingException {
    ...
    when(nbsObjectConverter.orgTelePhoneType(any(), any(), any()))
        .thenReturn(new EntityLocatorParticipationDto());
    when(nbsObjectConverter.personAddressType(any(), any(), any()))
        .thenThrow(new RuntimeException("TEST"));

...
public class RtiController {
  ...
  public RtiController(UidPoolManager poolManager, IDpDeadLetterService deadLetterService) {
  ...
  @Operation(
      ...
      description =
          "Clears and reloads all UID pools for all LocalIdClass types, including GA variants.",
      parameters = {
        @Parameter(
  ...
  @Operation(
      ...
      description =
          "Fetches DLT records either by specific NBS Interface UID or all records with non-success status if no UID is provided.",
      parameters = {
        @Parameter(
            in = ParameterIn.HEADER,
            name = "clientid",

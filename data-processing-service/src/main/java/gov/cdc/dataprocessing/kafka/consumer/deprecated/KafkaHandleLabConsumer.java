...
public class KafkaHandleLabConsumer {
  private static final Logger logger =
      LoggerFactory.getLogger(KafkaHandleLabConsumer.class); // NOSONAR

  private final IManagerService managerService;
  private final IAuthUserService authUserService;
  private final LabService labService;

  private static final Queue<PublicHealthCaseFlowContainer> pendingMessages =
  ...
  public KafkaHandleLabConsumer(
      IManagerService managerService, IAuthUserService authUserService, LabService labService) {
  ...
  //
  //                PublicHealthCaseFlowContainer nbs = GSON.fromJson(message,
  ...
  //
  //        if (threadEnabled) {
  //            Semaphore concurrencyLimiter = new Semaphore(poolSize); // Same as Hikari max pool
  // size
  //
  //            while (true) {
  ...
  //                            } catch (Exception e) {
  //                                log.error("Error processing NBS {}: {}", data, e.getMessage(),
  // e);
  //                            }
  //                        }
  //                    } finally {
  ...
  //                            try {
  //
  // labService.handleNndNotification(data.getPublicHealthCaseContainer(),
  // data.getEdxLabInformationDto());
  //                            } catch (Exception e) {
  //                                log.error("Error processing NBS {}: {}", data, e.getMessage(),
  // e);
  //                            }
  ...
  //                    labService.handleNndNotification(nbs.getPublicHealthCaseContainer(),
  // nbs.getEdxLabInformationDto());

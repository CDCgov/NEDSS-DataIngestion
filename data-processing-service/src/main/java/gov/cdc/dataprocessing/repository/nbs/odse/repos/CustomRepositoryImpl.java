...
public class CustomRepositoryImpl implements CustomRepository {
  ...
  //    @Transactional
  public List<StateDefinedFieldDataDto> getLdfCollection(
      Long busObjectUid, String conditionCode, String theQuery) {
  ...
  public Map<Object, Object> getAssociatedDocumentList(
      Long uid, String targetClassCd, String sourceClassCd, String theQuery) {
  ...
  //    @Transactional
  public List<NotificationSummaryContainer> retrieveNotificationSummaryListForInvestigation(
      Long publicHealthUID, String theQuery) {
  ...
  //    @Transactional
  public Map<Object, Object> retrieveTreatmentSummaryVOForInv(
      Long publicHealthUID, String theQuery) {
    Map<Object, Object> map = new HashMap<>();
    Query query = entityManager.createNativeQuery(theQuery);
    query.setParameter(PHC_UID, publicHealthUID);
  ...
  //    @Transactional
  public ArrayList<ResultedTestSummaryContainer> getSusceptibilityResultedTestSummary(
      String typeCode, Long observationUid) {
  ...
  //    @Transactional
  public ArrayList<UidSummaryContainer> getSusceptibilityUidSummary(
      ResultedTestSummaryContainer rvo,
      LabReportSummaryContainer labRepEvent,
      LabReportSummaryContainer labRepSumm,
      String typeCode,
      Long observationUid) {
  ...
  //    @Transactional
  public ArrayList<ResultedTestSummaryContainer> getTestAndSusceptibilities(
      String typeCode,
      Long observationUid,
      LabReportSummaryContainer labRepEvent,
      LabReportSummaryContainer labRepSumm) {
  ...
  //    @Transactional
  public ProviderDataForPrintContainer getOrderingPersonPhone(
      ProviderDataForPrintContainer providerDataForPrintVO, Long organizationUid) {
    String theSelect =
        "select phone_nbr_txt \"phoneNbrTxt\", extension_txt \"extensionTxt\" from TELE_locator with (nolock) where TELE_locator_uid in ("
            + " select locator_uid from Entity_locator_participation with (nolock) where entity_uid= "
            + organizationUid
            + " and cd='O' and class_cd='TELE')  ";
  ...
  //    @Transactional
  public ProviderDataForPrintContainer getOrderingPersonAddress(
      ProviderDataForPrintContainer providerDataForPrintVO, Long organizationUid) {
    String theSelect =
        "select street_addr1 \"streetAddr1\", city_desc_txt \"cityDescTxt\", state_cd \"stateCd\", zip_cd \"zipCd\" from Postal_locator with (nolock) where postal_locator_uid in ("
            + "select locator_uid from Entity_locator_participation with (nolock) where entity_uid in ("
            + organizationUid
  ...
  //    @Transactional
  public ProviderDataForPrintContainer getOrderingFacilityPhone(
      ProviderDataForPrintContainer providerDataForPrintVO, Long organizationUid) {
    String theSelect =
        "select phone_nbr_txt \"phoneNbrTxt\", extension_txt \"extensionTxt\" from TELE_locator with (nolock) where TELE_locator_uid in ("
            + " select locator_uid from Entity_locator_participation with (nolock) where entity_uid= "
            + organizationUid
            + "  and cd='PH' and class_cd='TELE')  ";
  ...
  //    @Transactional
  public ProviderDataForPrintContainer getOrderingFacilityAddress(
      ProviderDataForPrintContainer providerDataForPrintVO, Long organizationUid) {

    String theSelect =
        "select street_addr1 \"streetAddr1\", street_addr2 \"streetAddr2\", city_desc_txt \"cityDescTxt\", "
  ...
  public String getReportingFacilityName(Long organizationUid) {
    ...
    String vals = null;
    String theSelect =
        "SELECT organization_name.nm_txt \"nmTxt\" "
            + "FROM organization_name with (nolock) "
  ...
  public ArrayList<Object> getActIdDetails(Long observationUID) {
    ...
    String theSelect =
        "SELECT Act_id.root_extension_txt \"rootExtTxt\" "
  ...
  public ArrayList<Object> getProviderInfo(Long observationUID, String partTypeCd) {
    ...
    String ORD_PROVIDER =
        "person_name.person_uid \"providerUid\",person_name.last_nm \"lastNm\", person_name.nm_degree \"degree\", "
            + // NOSONAR
  ...
  public ArrayList<Object> getPatientPersonInfo(Long observationUID) {
    ArrayList<Object> vals = new ArrayList<>();
    String theSelect =
        "SELECT person_name.last_nm \"lastNm\", "
  ...
  public Map<Object, Object> getLabParticipations(Long observationUID) {
    ...
    String QUICK_FIND_PATIENT =
        "participation.subject_class_cd \"classCd\", " // NOSONAR
    ...
    if (resultValidCheck(results)) {
      for (var item : results) {
        ...
        if (classCd.equalsIgnoreCase(NEDSSConstant.CLASS_CD_PSN)
            && typeCd.equalsIgnoreCase(NEDSSConstant.PAR101_TYP_CD)) {
          continue;
        }
  ...
  public NbsDocumentContainer getNbsDocument(Long nbsUid) throws DataProcessingException {
    ...
    if (resultValidCheck(results)) {
      for (var item : results) {
        ...
        nbsDocumentVO.setEDXEventProcessDTMap(
            publicHealthCaseStoredProcRepository.getEDXEventProcessMap(
                container.getNbsDocumentUid()));
      }
    }
  ...
  public ArrayList<Object> getInvListForCoInfectionId(Long mprUid, String coInfectionId) {
    ...
    Query query =
        entityManager.createNativeQuery(COINFECTION_INV_LIST_FOR_GIVEN_COINFECTION_ID_SQL);

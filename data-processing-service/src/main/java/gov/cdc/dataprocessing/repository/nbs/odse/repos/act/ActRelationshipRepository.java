...
public interface ActRelationshipRepository extends JpaRepository<ActRelationship, Long> {
  ...
  **/
  @Query(
      "SELECT data FROM ActRelationship data WHERE data.sourceActUid = :uid AND data.typeCd = :type")
  Optional<Collection<ActRelationship>> loadActRelationshipBySrcIdAndTypeCode(
      @Param("uid") Long uid, @Param("type") String type);
}

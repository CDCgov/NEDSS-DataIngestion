...
public interface PersonRepository extends JpaRepository<Person, Long> {
  ...
  /**
   * String PATIENTPARENTUID_BY_UID = " SELECT p.person_parent_uid \"personParentUid\" FROM person p
   * with (nolock) where p.person_uid = ? AND p.record_status_cd = 'ACTIVE' "
   */
  @Query(
      "SELECT pn.personParentUid FROM Person pn WHERE pn.personUid = :parentUid AND pn.recordStatusCd='ACTIVE' ")

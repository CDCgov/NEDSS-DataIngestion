...
public class NbsInterfaceJdbcRepository {
  ...
  public NbsInterfaceJdbcRepository(
      @Qualifier("msgouteNamedParameterJdbcTemplate") NamedParameterJdbcTemplate jdbcTemplateOdse) {
    this.jdbcTemplateOdse = jdbcTemplateOdse;
  }

  public NbsInterfaceModel getNbsInterfaceByUid(Integer uid) {
    MapSqlParameterSource params = new MapSqlParameterSource().addValue("nbsInterfaceUid", uid);

    return jdbcTemplateOdse.queryForObject(
        SELECT_NBS_INTERFACE_BY_UID, params, new BeanPropertyRowMapper<>(NbsInterfaceModel.class));
  }
  ...
  public int updateRecordStatusToRtiProcess(List<Integer> ids) {
    ...
    for (int i = 0; i < ids.size(); i += CHUNK_SIZE) {
      ...
      MapSqlParameterSource params =
          new MapSqlParameterSource().addValue("status", "RTI_PENDING").addValue("ids", chunk);

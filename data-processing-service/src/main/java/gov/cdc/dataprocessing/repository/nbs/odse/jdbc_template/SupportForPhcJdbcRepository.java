...
public class SupportForPhcJdbcRepository {
  ...
  public SupportForPhcJdbcRepository(
  ...
  public Place findPlaceById(Long placeUid) {
    ...
    MapSqlParameterSource params = new MapSqlParameterSource().addValue("placeUid", placeUid);

    List<Place> results =
        jdbcTemplateOdse.query(FIND_BY_ID, params, new BeanPropertyRowMapper<>(Place.class));
    return results.isEmpty() ? null : results.getFirst();
  }

  public PatientEncounter findPatientEncounterById(Long patientEncounterUid) {
    MapSqlParameterSource params =
        new MapSqlParameterSource().addValue("patientEncounterUid", patientEncounterUid);

    List<PatientEncounter> result =
        jdbcTemplateOdse.query(
  ...
  public Referral findReferralById(Long referralUid) {
    MapSqlParameterSource params = new MapSqlParameterSource().addValue("referralUid", referralUid);

    List<Referral> result =
        jdbcTemplateOdse.query(
            ReferralDocQuery.FIND_BY_ID, params, new BeanPropertyRowMapper<>(Referral.class));
  ...
  public ClinicalDocument findClinicalDocumentById(Long clinicalDocumentUid) {
    MapSqlParameterSource params =
        new MapSqlParameterSource().addValue("clinicalDocumentUid", clinicalDocumentUid);

    List<ClinicalDocument> result =
        jdbcTemplateOdse.query(
  ...
  public NonPersonLivingSubject findNonPersonLivingSubjectById(Long nonPersonUid) {
    MapSqlParameterSource params =
        new MapSqlParameterSource().addValue("nonPersonUid", nonPersonUid);

    List<NonPersonLivingSubject> result =
        jdbcTemplateOdse.query(
  ...
  public EntityGroup findEntityGroupById(Long entityGroupUid) {
    MapSqlParameterSource params =
        new MapSqlParameterSource().addValue("entityGroupUid", entityGroupUid);

    List<EntityGroup> results =
        jdbcTemplateOdse.query(FIND_BY_UID, params, new BeanPropertyRowMapper<>(EntityGroup.class));

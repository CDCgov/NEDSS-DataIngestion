...
@Repository
public interface ParticipationHistRepository
    extends JpaRepository<ParticipationHist, ParticipationHistId> {
  @Query(
      "SELECT data.versionCtrlNbr FROM ParticipationHist data WHERE data.subjectEntityUid = ?1 AND data.actUid = ?2 AND data.typeCd = ?3")

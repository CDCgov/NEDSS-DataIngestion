...
public class AdvancedCriteria {
  public Map<String, Object> getAdvancedInvCriteriaMap(Algorithm algorithmDocument) {
    ...
    InvCriteriaType advanceInvCriteriaType =
        algorithmDocument.getElrAdvancedCriteria().getInvCriteria();
    ...
    if (advanceInvCriteriaType != null) {
      for (int i = 0; i < advanceInvCriteriaType.getInvValue().size(); i++) {
        InvValueType criteriaType = advanceInvCriteriaType.getInvValue().get(i);
        CodedType criteriaQuestionType = criteriaType.getInvQuestion();
        CodedType criteriaLogicType = criteriaType.getInvQuestionLogic();

        if (criteriaType.getInvStringValue() == null
            && !criteriaType.getInvCodedValue().isEmpty()) {
          String value;
          String[] array = new String[criteriaType.getInvCodedValue().size()];
          for (int j = 0; j < criteriaType.getInvCodedValue().size(); j++) {
            array[j] = criteriaType.getInvCodedValue().get(j).getCode();
          }
          Arrays.sort(array);
          ...
          advanceInvCriteriaMap.put(criteriaQuestionType.getCode(), edxRuleManageDT);

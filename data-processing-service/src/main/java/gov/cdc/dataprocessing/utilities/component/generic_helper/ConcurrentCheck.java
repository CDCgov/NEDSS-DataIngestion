...
public class ConcurrentCheck {
  ...
  public boolean dataConcurrenceCheck(
      RootDtoInterface theRootDTInterface, String tableName, Integer existingVersion) {
    if (tableName.equalsIgnoreCase("Person")) {
      ...
      if (existingVersion == null
          || existingVersion.equals(theRootDTInterface.getVersionCtrlNbr())) {
        return true;
      } else {
        PersonDto newPersonDT = (PersonDto) theRootDTInterface;

        if (existingVersion.equals(theRootDTInterface.getVersionCtrlNbr() - 1)
            && newPersonDT.isReentrant()) {
          return true;
        }
      }
    }

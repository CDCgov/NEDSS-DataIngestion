...
public class PatientRepositoryUtil {
  ...
  public Person createPerson(PersonContainer personContainer) throws DataProcessingException {
    ...
    entityRepositoryUtil.preparingEntityReposCallForPerson(
        personDto, personUid, NEDSSConstant.PERSON, OPERATION_UPDATE);
    ...
    if (Objects.equals(personDto.getPersonParentUid(), personDto.getPersonUid())) {
      Timestamp newLastChgTime =
          new Timestamp(person.getLastChgTime().getTime() + (10 * 60 * 1000)); // +2 minutes
  ...
  public PersonContainer loadPerson(Long personUid) {
    ...
    personContainer.setTheEntityLocatorParticipationDtoCollection(
  ...
  protected void updatePersonName(PersonContainer personContainer) {
    List<PersonNameDto> personList =
        new ArrayList<>(personContainer.getThePersonNameDtoCollection());
  ...
  protected void deleteInactivePersonRace(
      List<String> retainingRaceCodeList, Long patientUid, Long parentUid) {
    // Executes after update process: delete races not in retained list and not directly tied to
    // parent UID
  ...
  protected void createEntityId(PersonContainer personContainer) {
    ...
    List<EntityId> entityIdEntities =
        entityIdDtos.stream()
            .map(
                dto -> {
                  dto.setEntityUid(personUid);

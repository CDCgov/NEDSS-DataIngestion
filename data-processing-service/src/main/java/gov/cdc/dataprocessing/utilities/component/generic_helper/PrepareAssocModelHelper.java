...
public class PrepareAssocModelHelper {
  ...
  public PrepareAssocModelHelper(
  ...
  /**
   * This method is used to populate the system attributes on the 5 association tables
   * (ActRelationship, Participation, Role, EntityLocatoryParticipation, and
   * ActLocatorParticipation).
   */
  public EntityLocatorParticipationDto prepareAssocDTForEntityLocatorParticipation(
      EntityLocatorParticipationDto assocDTInterface) throws DataProcessingException {
    ...
    } else if (!(recStatusCd.equals(NEDSSConstant.RECORD_STATUS_ACTIVE)
  ...
  public ActRelationshipDto prepareAssocDTForActRelationship(ActRelationshipDto assocDTInterface)
  ...
  public ParticipationDto prepareAssocDTForParticipation(ParticipationDto assocDTInterface)
      throws DataProcessingException {
   ...
   * ActLocatorParticipation).
   */
  public ActivityLocatorParticipationDto prepareActivityLocatorParticipationDT(
      ActivityLocatorParticipationDto assocDTInterface) throws DataProcessingException {
  ...
  public ActRelationshipDto prepareActRelationshipDT(ActRelationshipDto assocDTInterface)
      throws DataProcessingException {
    ActRelationshipDto aDTInterface;
    String recStatusCd = assocDTInterface.getRecordStatusCd();
    String statusCd = assocDTInterface.getStatusCd();
   ...
   */
  public RootDtoInterface prepareVO(
      RootDtoInterface theRootDTInterface,
      ...
      throws DataProcessingException {
    if (!theRootDTInterface.isItNew()
        && !theRootDTInterface.isItDirty()
        && !theRootDTInterface.isItDelete()) {
    ...
    if (theRootDTInterface.isItDirty() && !theRootDTInterface.isItNew()) {
      // CONCURRENCE CHECK
      boolean result =
          concurrentCheck.dataConcurrenceCheck(theRootDTInterface, tableName, existingVersion);
      if (result) {
        logger.debug("result in prepareVOUtil is : {}", true);
        // no concurrent dataAccess has occured, hence can continue!
      } else {
        throw new DataProcessingException(
            "NEDSSConcurrentDataException occurred in PrepareVOUtils.Person");
      }
    }

    if (theRootDTInterface.isItNew()
        && (theRootDTInterface.getSuperclass().equalsIgnoreCase(NEDSSConstant.CLASSTYPE_ACT))) {
      logger.debug("new act");
      theRootDTInterface =
          prepareNewActVO(
              theRootDTInterface, businessObjLookupName, businessTriggerCd, tableName, moduleCd);
    } else if (theRootDTInterface.isItNew()
        && (theRootDTInterface.getSuperclass().equalsIgnoreCase(NEDSSConstant.CLASSTYPE_ENTITY))) {
      logger.debug("new entity");
      theRootDTInterface =
          prepareNewEntityVO(
              theRootDTInterface, businessObjLookupName, businessTriggerCd, tableName, moduleCd);
    } else if (theRootDTInterface.isItDirty()
        && (theRootDTInterface.getSuperclass().equalsIgnoreCase(NEDSSConstant.CLASSTYPE_ACT))) {
  ...
  /**
   * This method prepares the Act value object if it is New(Create) and check null for record Status
   * State and set the System attributes in the rootDTInterface
   */
  @SuppressWarnings("java:S1172")
  protected RootDtoInterface prepareNewActVO(
      RootDtoInterface theRootDTInterface,
      String businessObjLookupName,
      String businessTriggerCd,
      String tableName,
    ...
    PrepareEntity prepareVOUtilsHelper =
        prepareEntityStoredProcRepository.getPrepareEntity(
    ...
    if (recordStatusState == null) {
      throw new DataProcessingException(
          "prepareNewActVO - recordStatusState = "
              + null
              + "- objectStatusState = "
              + objectStatusState);
    }

    if (theRootDTInterface.getProgAreaCd() != null
        && theRootDTInterface.getJurisdictionCd() != null) {
      String progAreaCd = theRootDTInterface.getProgAreaCd();
      String jurisdictionCd = theRootDTInterface.getJurisdictionCd();
      Long aProgramJurisdictionOid =
          progAreaJurisdictionUtil.getPAJHash(progAreaCd, jurisdictionCd);
      ...
      logger.debug(
          "aProgramJurisdictionOid from obj  is : {}",
          theRootDTInterface.getProgramJurisdictionOid());
    }
  ...
  @SuppressWarnings("java:S1172")
  protected RootDtoInterface prepareNewEntityVO(
      RootDtoInterface theRootDTInterface,
      String businessObjLookupName,
      String businessTriggerCd,
      String tableName,
    ...
    if (tableName.equals(NEDSSConstant.PATIENT)
        && (!businessTriggerCd.equals(PAT_NO_MERGER))
  ...
  /**
   * This method prepares the Act value object if it is Dirty(Edit,update or Delete) and check null
   * for record Status State and set the System attribures in the rootDTInterface
   */
  @SuppressWarnings("java:S1172")
  protected RootDtoInterface prepareDirtyActVO(
      RootDtoInterface theRootDTInterface,
      String businessObjLookupName,
      String businessTriggerCd,
      String tableName,
  ...
  /**
   * This method prepares the Entity value object if it is Dirty(Edit,update or Delete) and check
   * null for record Status State and set the System attribures in the rootDTInterface
   */
  @SuppressWarnings("java:S1172")
  protected RootDtoInterface prepareDirtyEntityVO(
      RootDtoInterface theRootDTInterface,
      String businessObjLookupName,
      String businessTriggerCd,
      String tableName,
  ...
  public ActivityLocatorParticipationDto prepareAssocDTForActivityLocatorParticipation(
      ActivityLocatorParticipationDto assocDTInterface) throws DataProcessingException {
    ActivityLocatorParticipationDto aDTInterface;
    String recStatusCd = assocDTInterface.getRecordStatusCd();
    String statusCd = assocDTInterface.getStatusCd();

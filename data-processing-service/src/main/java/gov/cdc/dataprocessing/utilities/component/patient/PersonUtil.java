...
public class PersonUtil {
  ...
  public Long processLabPersonContainerCollection(
      Collection<PersonContainer> personContainerCollection,
      boolean morbidityApplied,
      BaseContainer dataContainer)
  ...
  private Long setPersonForObservationFlow(
      String personType, PersonContainer personVO, boolean isNew, boolean isExternal)
      throws DataProcessingException {
    if (personType.equalsIgnoreCase(NEDSSConstant.PAT)) {
      return patientMatchingService.updateExistingPerson(
          personVO, isNew ? NEDSSConstant.PAT_CR : NEDSSConstant.PAT_EDIT);
    } else if (personType.equalsIgnoreCase(NEDSSConstant.PRV)
        && (!isNew || (isNew && isExternal))) {
      return providerMatchingService.setProvider(
          personVO, isNew ? NEDSSConstant.PRV_CR : NEDSSConstant.PRV_EDIT);
    } else {

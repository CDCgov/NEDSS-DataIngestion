...
public class ValidateDecisionSupport {
  ...
  public void processNBSObjectDT(
    ...
    getMethodName =
        "GET" + getMethodName.substring(getMethodName.indexOf(".") + 1, getMethodName.length());
    ...
    try {
      ...
      for (Method value : methodList) {
        if (value.getName().equalsIgnoreCase(getMethodName)) {
          ...
          if (metaData.getDataType().equalsIgnoreCase(NEDSSConstant.NBS_QUESTION_DATATYPE_TEXT)
              || metaData
                  .getDataType()
                  .equalsIgnoreCase(NEDSSConstant.NBS_QUESTION_DATATYPE_CODED_VALUE)) {
            setMethod = phcClass.getMethod(setMethodName, String.class);

          } else if (metaData
                  .getDataType()
                  .equalsIgnoreCase(NEDSSConstant.NBS_QUESTION_DATATYPE_DATETIME)
              || metaData.getDataType().equalsIgnoreCase(NEDSSConstant.DATETIME_DATATYPE)
              || metaData
                  .getDataType()
                  .equalsIgnoreCase(NEDSSConstant.NBS_QUESTION_DATATYPE_DATE)) {
            getCurrentDateValue(edxRuleManageDT);
            setMethod = phcClass.getMethod(setMethodName, Timestamp.class);
          } else if (metaData
              .getDataType()
              .equalsIgnoreCase(NEDSSConstant.NBS_QUESTION_DATATYPE_NUMERIC)) {
            if (value.getReturnType().equals(Integer.class))
              ...
              setMethod = phcClass.getMethod(setMethodName, BigDecimal.valueOf(0).getClass());
                ...
                .getReturnType()
                .equals(String.class)) // Added because question INV139's datatype is NUMERIC in
              // nbs_ui_metadata table but the datatype is varchar in
              // Public_Health_Case table.
              setMethod = phcClass.getMethod(setMethodName, String.class);
          }
  ...
  public void processNBSCaseAnswerDT(
      EdxRuleManageDto edxRuleManageDT,
      PublicHealthCaseContainer publicHealthCaseContainer,
      ...
      if (metaData.getNbsUiComponentUid().compareTo(1013L) == 0) // NOSONAR
      {
        ...
        if (toValueColl != null) {
          ...
          while (iterator.hasNext()) {
            ...
            edxPHCRDocumentUtil.setStandardNBSCaseAnswerVals(
                publicHealthCaseContainer, nbsAnswerDT);
        ...
        NbsCaseAnswerDto nbsAnswerDT = new NbsCaseAnswerDto();
        if (code != null) nbsAnswerDT.setAnswerTxt(code);
  ...
  public void processConfirmationMethodCodeDT(
      EdxRuleManageDto edxRuleManageDT,
      PublicHealthCaseContainer publicHealthCaseContainer,
      NbsQuestionMetadata metaData) {
    String behavior = edxRuleManageDT.getBehavior();
      ...
      if (metaData.getNbsUiComponentUid().compareTo(1013L) == 0) // NOSONAR
      {
        ...
        if (toValueColl != null) {
          for (Object o : toValueColl) {
            ...
            confirmDT.setPublicHealthCaseUid(
            ...
            Collection<ConfirmationMethodDto> confirmColl =
                publicHealthCaseContainer.getTheConfirmationMethodDTCollection();
            if (confirmColl != null) {
        ...
        if (toValueColl != null) {
          ...
          for (Object o : toValueColl) {
            ...
            while (cofirmIt.hasNext()) {
              ...
              if (confirmDT.getConfirmationMethodCd() == null
                  || confirmDT.getConfirmationMethodCd().trim().isEmpty()) {
                break;
              } else {
  ...
  public static void setMethod(
      Object nbsObject, Method setMethod, EdxRuleManageDto edxRuleManageDT) {
    try {
      ...
      for (Object object : parameterArray) {
        if (object.toString().equalsIgnoreCase("class java.math.BigDecimal")) {
          ...
          else setMethod.invoke(nbsObject, new BigDecimal(edxRuleManageDT.getDefaultStringValue()));
        } else if (object.toString().equalsIgnoreCase("class java.lang.String")) {
          if (edxRuleManageDT.getDefaultStringValue() != null
              && !edxRuleManageDT.getDefaultStringValue().trim().isEmpty())
            setMethod.invoke(nbsObject, edxRuleManageDT.getDefaultStringValue());
          else if (edxRuleManageDT.getDefaultCommentValue() != null
              && !edxRuleManageDT.getDefaultCommentValue().trim().isEmpty())
          ...
          setMethod.invoke(
              nbsObject,
              StringUtils.stringToStrutsTimestamp(edxRuleManageDT.getDefaultStringValue()));
          ...
          else setMethod.invoke(nbsObject, edxRuleManageDT.getDefaultStringValue());
  ...
  public PublicHealthCaseContainer processConfirmationMethodTimeDT(
      EdxRuleManageDto edxRuleManageDT,
      PublicHealthCaseContainer publicHealthCaseContainer,
      NbsQuestionMetadata metaData)
    ...
    if (isOverwrite) {
      Collection<ConfirmationMethodDto> list = new ArrayList<>();
      if (time != null
          && publicHealthCaseContainer.getTheConfirmationMethodDTCollection() != null) {
        for (ConfirmationMethodDto confirmDT :
            publicHealthCaseContainer.getTheConfirmationMethodDTCollection()) {
  ...
  public void processNBSCaseManagementDT(
      EdxRuleManageDto edxRuleManageDT,
      PublicHealthCaseContainer publicHealthCaseContainer,
      NbsQuestionMetadata metaData) {
  ...
  public void processConfirmationMethodCodeDTRequired(
    ...
    if (confirmColl != null) {
      ...
      for (ConfirmationMethodDto confirmDT : confirmColl) {
        if (confirmDT.getConfirmationMethodCd() == null
            && confirmDT.getConfirmationMethodTime() != null) {
  ...
  public void parseInvestigationDefaultValuesType(
      Map<Object, Object> map, InvestigationDefaultValuesType investigationDefaultValuesType) {
    ...
    for (DefaultValueType defaultValueType : defaultValueTypeArray) {
      ...
      if (defaultValueType.getDefaultParticipation() != null) {
        try {
          edxRuleManageDT.setParticipationTypeCode(
              defaultValueType.getDefaultParticipation().getParticipationType().getCode());
          edxRuleManageDT.setParticipationUid(
              Long.valueOf(defaultValueType.getDefaultParticipation().getEntityUid()));
          edxRuleManageDT.setParticipationClassCode(
              defaultValueType.getDefaultParticipation().getEntityClass());
        } catch (Exception e) {
          logger.error(
              "The defaultValueType exception is not valid for code and/or uid and/or classCode. Please check: {}",
              defaultValueType); // NOSONAR
        }
        ...
        edxRuleManageDT.setDefaultNumericValue(
            String.valueOf(defaultValueType.getDefaultNumericValue().getValue1()));
      }
  ...
  public void processActIds(
      EdxRuleManageDto edxRuleManageDT,
      PublicHealthCaseContainer publicHealthCaseContainer,
      NbsQuestionMetadata metaData) {
    String behavior = edxRuleManageDT.getBehavior();
    boolean isOverwrite = behavior.equalsIgnoreCase("1"); // NOSONAR
    Collection<ActIdDto> actIdColl = publicHealthCaseContainer.getTheActIdDTCollection();
    if (actIdColl != null && !actIdColl.isEmpty()) {
      ...
      if (actIdDT.getTypeCd() != null
          && actIdDT.getTypeCd().equalsIgnoreCase(NEDSSConstant.ACT_ID_STATE_TYPE_CD)
          && metaData.getDataCd() != null
          && metaData.getDataCd().equalsIgnoreCase(NEDSSConstant.ACT_ID_STATE_TYPE_CD)) {
        if (isOverwrite) actIdDT.setRootExtensionTxt(edxRuleManageDT.getDefaultStringValue());
        else if (actIdDT.getRootExtensionTxt() == null)
          actIdDT.setRootExtensionTxt(edxRuleManageDT.getDefaultStringValue());
  ...
  protected void getCurrentDateValue(EdxRuleManageDto edxRuleManageDT) {
    if (edxRuleManageDT.getDefaultStringValue() != null
        && edxRuleManageDT.getDefaultStringValue().equals(NEDSSConstant.USE_CURRENT_DATE)) {
      var ts = StringUtils.formatDate(TimeStampUtil.getCurrentTimeStamp(tz));

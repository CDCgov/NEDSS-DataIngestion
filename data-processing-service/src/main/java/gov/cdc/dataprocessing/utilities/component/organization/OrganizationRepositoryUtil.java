...
public class OrganizationRepositoryUtil {
  ...
  public long createOrganization(OrganizationContainer organizationContainer)
    ...
    localUid =
        localIdModel.getClassTypeUid().getUidPrefixCd()
            + localIdModel.getClassTypeUid().getSeedValueNbr()
            + localIdModel.getClassTypeUid().getUidSuffixCd();

    if (organizationContainer.getTheOrganizationDto().getLocalId() == null
        || organizationContainer.getTheOrganizationDto().getLocalId().trim().isEmpty()) {
      organizationContainer.getTheOrganizationDto().setLocalId(localUid);
    }
    /** Starts inserting a new organization */
    // Upper stream require this id to not mutated (must be negative), so falseToNew Method can
    ...
    if (organizationContainer.getTheEntityLocatorParticipationDtoCollection() != null
        && !organizationContainer.getTheEntityLocatorParticipationDtoCollection().isEmpty()) {
      createEntityLocatorParticipation(organizationContainer, OPERATION_CREATE);
    }
    // NOTE: Create Role
    if (organizationContainer.getTheRoleDTCollection() != null
        && !organizationContainer.getTheRoleDTCollection().isEmpty()) {
  ...
  private void insertOrganizationNames(
      OrganizationContainer organizationContainer, String operation) {
    logger.debug(
        "insertOrganizationNames(long organizationUID, Collection<Object>  organizationNames)");
    ...
    Collection<OrganizationNameDto> organizationNames =
        organizationContainer.getTheOrganizationNameDtoCollection();
  ...
  private void createEntityId(OrganizationContainer organizationContainer) {
    ArrayList<EntityIdDto> entityList =
        (ArrayList<EntityIdDto>) organizationContainer.getTheEntityIdDtoCollection();
  ...
  private void createEntityLocatorParticipation(OrganizationContainer ovo, String operation)
      throws DataProcessingException {
    var entityLocatorList =
        (ArrayList<EntityLocatorParticipationDto>)
            ovo.getTheEntityLocatorParticipationDtoCollection();
  ...
  private void processLocator(
      EntityLocatorParticipationDto dto, long locatorUid, String operation) {
  ...
  private void persistEntityLocatorParticipation(
      EntityLocatorParticipation data, String operation) {
    if (OPERATION_CREATE.equalsIgnoreCase(operation)) {
   ...
   */
  public Long setOrganization(OrganizationContainer organizationContainer, String businessTriggerCd)
      throws DataProcessingException {
    Long organizationUID;
  ...
  public Long setOrganizationInternal(
      OrganizationContainer organizationContainer, String businessTriggerCd)
      throws DataProcessingException {
    Long organizationUID;
    logger.debug("\n\n Inside set");
    if (!organizationContainer.isItNew() && !organizationContainer.isItDirty()) {
      return organizationContainer.getTheOrganizationDto().getOrganizationUid();
    } else {
      Integer existVer = null;
      if (organizationContainer.getTheOrganizationDto().getOrganizationUid() > 0) {
        var existObs =
            loadObject(organizationContainer.getTheOrganizationDto().getOrganizationUid(), null);
      ...
      OrganizationDto newOrganizationDto =
          (OrganizationDto)
              prepareAssocModelHelper.prepareVO(
                  organizationContainer.getTheOrganizationDto(),
                  "ORGANIZATION",
                  businessTriggerCd,
                  "ORGANIZATION",
                  NEDSSConstant.BASE,
      ...
      Collection<EntityLocatorParticipationDto> elpDTCol =
          organizationContainer.getTheEntityLocatorParticipationDtoCollection();
      Collection<RoleDto> rDTCol = organizationContainer.getTheRoleDTCollection();
      Collection<ParticipationDto> pDTCol =
          organizationContainer.getTheParticipationDtoCollection();
      if (elpDTCol != null) {
        Collection<EntityLocatorParticipationDto> col =
            entityHelper.iterateELPDTForEntityLocatorParticipation(elpDTCol);
        organizationContainer.setTheEntityLocatorParticipationDtoCollection(col);
      }
      if (rDTCol != null) {
      ...
      if (organizationContainer.isItNew()) {
        ...
        organizationUID = organizationContainer.getTheOrganizationDto().getOrganizationUid();
  ...
  private void prepareOrganizationNameBeforePersistence(
    ...
    if (namesCollection != null) {
      try {
        for (anIterator = namesCollection.iterator(); anIterator.hasNext(); ) {
          OrganizationNameDto organizationNameDto = anIterator.next();
          if (organizationNameDto.getNmUseCd().equals("L")) {
            selectedName = organizationNameDto.getNmTxt();
            organizationContainer.getTheOrganizationDto().setDisplayNm(selectedName);
          }
        }
      } catch (Exception e) {
  ...
  /**
   * Specifically for loading the Reporting Lab or when logic requires the loading of a specific
   * participation record. See logic in method regarding loading the ParticipationDTCollection for
   * this organization. The elr can result in the participation to have a substantial amount of
   * Reporting labs with the same subjectEntityUid, therefore need to select based on teh actUid for
  ...
  public OrganizationContainer loadObject(Long organizationUID, Long actUid) {
    ...
    Collection<EntityLocatorParticipationDto> elpColl =
        selectEntityLocatorParticipations(organizationUID);
  ...
  private Collection<OrganizationNameDto> selectOrganizationNames(long organizationUID) {
    ...
    List<OrganizationName> listOptional =
        organizationNameJdbcRepository.findByOrganizationUid(organizationUID);
  ...
  @SuppressWarnings("java:S3776")
  private Collection<EntityLocatorParticipationDto> selectEntityLocatorParticipations(
      long organizationUID) {
    ...
    if (!entityLocatorParticipations.isEmpty()) {
      ...
      physicalLocators =
          entityLocatorParticipations.stream()
              .filter(x -> x.getClassCd().equalsIgnoreCase(NEDSSConstant.PHYSICAL))
              .toList();
      postalLocators =
          entityLocatorParticipations.stream()
              .filter(x -> x.getClassCd().equalsIgnoreCase(NEDSSConstant.POSTAL))
              .toList();
      teleLocators =
          entityLocatorParticipations.stream()
              .filter(x -> x.getClassCd().equalsIgnoreCase(NEDSSConstant.TELE))
              .toList();

      if (!physicalLocators.isEmpty()) {
        List<PhysicalLocator> existingLocator =
            entityLocatorJdbcRepository.findByPhysicalLocatorUids(
      ...
      if (!postalLocators.isEmpty()) {
        var existingLocator =
            entityLocatorJdbcRepository.findByPostalLocatorUids(
      ...
      if (!teleLocators.isEmpty()) {
        var existingLocator =
            entityLocatorJdbcRepository.findByTeleLocatorUids(
                teleLocators.stream()
                    .map(EntityLocatorParticipation::getLocatorUid)
                    .collect(Collectors.toList()));
  ...
  public OrganizationDto prepareVO(
      OrganizationDto organizationDto, String businessTriggerCd, String tableName, String moduleCd)
      throws DataProcessingException {
    if (!organizationDto.isItNew()
        && !organizationDto.isItDirty()
        && !organizationDto.isItDelete()) {
      throw new DataProcessingException("Error while calling prepareVO method in PrepareVOUtils");
    }
    logger.debug(
        "(Boolean.FALSE).equals(new Boolean(theRootDTInterface.tableName)?: {}:theRootDTInterface.moduleCd: {}:businessTriggerCd: {}",
        tableName,
        moduleCd,
        businessTriggerCd);
    ...
    if (organizationDto.isItNew() || organizationDto.isItDirty()) {
      ...
      PrepareEntity prepareEntity =
          this.getPrepareEntityForOrganization(
              businessTriggerCd, moduleCd, organizationDto.getOrganizationUid(), tableName);
  ...
  public PrepareEntity getPrepareEntityForOrganization(
      String businessTriggerCd, String moduleCd, Long uid, String tableName)
      throws DataProcessingException {
    return prepareEntityStoredProcRepository.getPrepareEntity(
        businessTriggerCd, moduleCd, uid, tableName);
  }
}

...
public class PublicHealthCaseRepositoryUtil {
  ...
  public PublicHealthCaseRepositoryUtil(
      PublicHealthCaseJdbcRepository publicHealthCaseJdbcRepository,
  ...
  public PublicHealthCaseContainer update(PublicHealthCaseContainer phcVO)
    ...
    if (phcVO.getTheConfirmationMethodDTCollection() != null) {
      insertConfirmationMethods(
          phcVO.getThePublicHealthCaseDto().getUid(), phcVO.getTheConfirmationMethodDTCollection());
    }
    /** Inserts CaseManagementDto */
    if (phcVO.getTheCaseManagementDto() != null
        && phcVO.getTheCaseManagementDto().isCaseManagementDTPopulated) {
      insertCaseManagementDT(
          phcVO.getThePublicHealthCaseDto().getUid(), phcVO.getTheCaseManagementDto());
    }
    /** Inserts ActIdDT collection */
    if (phcVO.getTheActIdDTCollection() != null) {
      insertActivityIDs(
          phcVO.getThePublicHealthCaseDto().getUid(), phcVO.getTheActIdDTCollection());
    }

    /** Inserts ActivityLocatorParticipationDT collection */
    if (phcVO.getTheActivityLocatorParticipationDTCollection() != null) {
      insertActivityLocatorParticipations(
          phcVO.getThePublicHealthCaseDto().getUid(),
          phcVO.getTheActivityLocatorParticipationDTCollection());
    }
  ...
  public PublicHealthCaseContainer create(PublicHealthCaseContainer phcVO)
      throws DataProcessingException {
    long phcUid;
  ...
  private void insertActivityLocatorParticipations(
      Long phcUid, Collection<ActivityLocatorParticipationDto> activityIDs) {
    ArrayList<ActivityLocatorParticipationDto> activityLocatorArray =
        (ArrayList<ActivityLocatorParticipationDto>) activityIDs;
  ...
  private void insertCaseManagementDT(Long phcUid, CaseManagementDto caseManagementDto)
      throws DataProcessingException {
  ...
  protected void updateCaseManagementWithEPIIDandFRNum(CaseManagementDto caseManagementDto)
      throws DataProcessingException {
    // generate EPI Link Id (Lot Nbr) and field record number if not present

    if (caseManagementDto.getEpiLinkId() == null
        && caseManagementDto.getFieldRecordNumber() == null) {
      SimpleDateFormat sdf = new SimpleDateFormat("yy"); // Just the year, with 2 digits
      String twoDigitYear = sdf.format(Calendar.getInstance().getTime());
      var epicUid = uidPoolManager.getNextUid(LocalIdClass.EPILINK, false);
      String epiLinkId =
          epicUid.getClassTypeUid().getUidPrefixCd()
              + epicUid.getClassTypeUid().getSeedValueNbr()
              + epicUid.getClassTypeUid().getUidSuffixCd();
      String lotNum =
          servicePropertiesProvider.getNbsStateCode()
    ...
    } else if (caseManagementDto.getEpiLinkId() != null
        && caseManagementDto.getFieldRecordNumber() == null) {
      SimpleDateFormat sdf = new SimpleDateFormat("yy"); // Just the year, with 2 digits
      String twoDigitYear = sdf.format(Calendar.getInstance().getTime());
      var epicUid = uidPoolManager.getNextUid(LocalIdClass.EPILINK, false);
  ...
  private Long insertPublicHealthCase(PublicHealthCaseContainer phcVO)
      throws DataProcessingException {
    var uid = uidPoolManager.getNextUid(LocalIdClass.PUBLIC_HEALTH_CASE, true);
    var phcDT = phcVO.getThePublicHealthCaseDto();
    if (phcDT.getCaseTypeCd().equals(NEDSSConstant.I)
        && (phcDT.getInvestigationStatusCd() == null
            || phcDT.getInvestigationStatusCd().trim().isEmpty()
            || phcDT.getProgAreaCd() == null
            || phcDT.getProgAreaCd().trim().isEmpty()
            || phcDT.getJurisdictionCd() == null
            || phcDT.getJurisdictionCd().isEmpty())) {

      String error =
          "********#Investigation canot be inserted with partial information for these fields : Program Area Cd = "
              + phcDT.getProgAreaCd()
              + " Jurisdiction Code = "
              + phcDT.getJurisdictionCd()
              + " Investigation Status = "
              + phcDT.getInvestigationStatusCd();
  ...
  public PublicHealthCaseContainer getPublicHealthCaseContainer(long publicHealthCaseUid)
      throws DataProcessingException {
  ...
  public BasePamContainer getPamVO(Long publicHealthCaseUID) {
    ...
    if (pamAnswerDTReturnMap.get(NEDSSConstant.NON_REPEATING_QUESTION) != null) {
      nbsAnswerMap =
          (HashMap<Object, Object>) pamAnswerDTReturnMap.get(NEDSSConstant.NON_REPEATING_QUESTION);
    }
    if (pamAnswerDTReturnMap.get(NEDSSConstant.REPEATING_QUESTION) != null) {
      nbsRepeatingAnswerMap =
          (HashMap<Object, Object>) pamAnswerDTReturnMap.get(NEDSSConstant.REPEATING_QUESTION);
    }
    pamVO.setPamAnswerDTMap(nbsAnswerMap);
    pamVO.setPageRepeatingAnswerDTMap(nbsRepeatingAnswerMap);

    Collection<NbsActEntityDto> pamCaseEntityDTCollection =
        getActEntityDTCollection(publicHealthCaseUID);
  ...
  private Map<Object, Object> getPamAnswerDTMaps(Long publicHealthCaseUID) {
    ...
    while (it.hasNext()) {
      ...
      if (pamAnsDT.getNbsQuestionUid() != null
          && nbsQuestionUid != 0
          && pamAnsDT.getNbsQuestionUid().longValue() != nbsQuestionUid.longValue()
          && !coll.isEmpty()) {
      ...
      if (pamAnsDT.getAnswerGroupSeqNbr() != null && pamAnsDT.getAnswerGroupSeqNbr() > -1) {
        if (nbsRepeatingAnswerMap.get(pamAnsDT.getNbsQuestionUid()) == null) {
          ...
          Collection collection =
              (Collection) nbsRepeatingAnswerMap.get(pamAnsDT.getNbsQuestionUid());
          collection.add(pamAnsDT);
          nbsRepeatingAnswerMap.put(pamAnsDT.getNbsQuestionUid(), collection);
        }
      } else if ((pamAnsDT.getNbsQuestionUid() != null

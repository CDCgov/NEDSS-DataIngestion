...
public class EdxPhcrDocumentUtil {
  private static final Logger logger =
      LoggerFactory.getLogger(EdxPhcrDocumentUtil.class); // NOSONAR
  ...
  public Map<Object, Object> loadQuestions(String conditionCode) throws DataProcessingException {
    ...
    if (cacheApiService.getSrteCacheBool(
        ObjectName.INVESTIGATION_FORM_CONDITION_CODE.name(), conditionCode)) {
      invFormCd =
  ...
  protected Map<Object, Object> getQuestionMapForFormCode(String invFormCd) {
    ...
    if (invFormCd.equals(NBSConstantUtil.INV_FORM_RVCT)
        || invFormCd.equals(NBSConstantUtil.INV_FORM_VAR)) {
      ...
      if (OdseCache.dmbMap.containsKey(invFormCd)) {
        ...
        Map<Object, Object> questions =
            (Map<Object, Object>) OdseCache.DMB_QUESTION_MAP.get(invFormCd);
        if (questions != null) {
  ...
  public String requiredFieldCheck(
      Map<Object, Object> requiredQuestionIdentifierMap, Map<Object, Object> nbsCaseAnswerMap) {
    //
        ...
        if (reqdKey != null) {
          if (nbsCaseAnswerMap == null || nbsCaseAnswerMap.get(reqdKey) == null) {
            NbsQuestionMetadata metaData =
                (NbsQuestionMetadata) requiredQuestionIdentifierMap.get(reqdKey);
  ...
  public NbsCaseAnswerDto setStandardNBSCaseAnswerVals(
      PublicHealthCaseContainer publicHealthCaseContainer, NbsCaseAnswerDto nbsCaseAnswerDT) {

    nbsCaseAnswerDT.setActUid(
        publicHealthCaseContainer.getThePublicHealthCaseDto().getPublicHealthCaseUid());
    nbsCaseAnswerDT.setAddTime(publicHealthCaseContainer.getThePublicHealthCaseDto().getAddTime());
    nbsCaseAnswerDT.setLastChgTime(
        publicHealthCaseContainer.getThePublicHealthCaseDto().getLastChgTime());
    nbsCaseAnswerDT.setAddUserId(
        publicHealthCaseContainer.getThePublicHealthCaseDto().getAddUserId());
    nbsCaseAnswerDT.setLastChgUserId(
        publicHealthCaseContainer.getThePublicHealthCaseDto().getLastChgUserId());
    ...
    nbsCaseAnswerDT.setRecordStatusTime(
        publicHealthCaseContainer.getThePublicHealthCaseDto().getRecordStatusTime());

...
public class NotificationRepositoryUtil {
  ...
  public NotificationContainer getNotificationContainer(Long uid) {
    ...
    var actPatCollection =
        actLocatorParticipationRepositoryUtil.getActLocatorParticipationCollection(uid);
    if (!actPatCollection.isEmpty()) {
      notificationContainer.setTheActivityLocatorParticipationDTCollection(actPatCollection);
    }

    var actReCollection =
        actRelationshipRepositoryUtil.getActRelationshipCollectionFromSourceId(uid);
  ...
  public Long setNotification(NotificationContainer notificationContainer)
      throws DataProcessingException {
    Long notificationUid;

    Collection<ActivityLocatorParticipationDto> alpDTCol =
        notificationContainer.getTheActivityLocatorParticipationDTCollection();
    Collection<ActRelationshipDto> arDTCol =
        notificationContainer.getTheActRelationshipDTCollection();
    ...
    if (notificationContainer.isItNew()) {
      ...
      var notification =
          getNotificationContainer(
              notificationContainer.getTheNotificationDT().getNotificationUid());
  ...
  private Long createNotification(NotificationContainer notificationContainer)
      throws DataProcessingException {
    var uidData = uidPoolManager.getNextUid(LocalIdClass.NOTIFICATION, true);
    var uid = uidData.getGaTypeUid().getSeedValueNbr();
    var localId =
        uidData.getClassTypeUid().getUidPrefixCd()
            + uidData.getClassTypeUid().getSeedValueNbr()
            + uidData.getClassTypeUid().getUidSuffixCd();

    actRepositoryUtil.insertActivityId(
        uid, NEDSSConstant.NOTIFICATION_CLASS_CODE, NEDSSConstant.EVENT_MOOD_CODE);
  ...
  private Long updateNotification(NotificationContainer notificationContainer) {
    ...
    actRepositoryUtil.updateActivityId(
        uid, NEDSSConstant.NOTIFICATION_CLASS_CODE, NEDSSConstant.EVENT_MOOD_CODE);
    ...
    actLocatorParticipationRepositoryUtil.insertActLocatorParticipationCollection(
        uid, notificationContainer.getTheActivityLocatorParticipationDTCollection());

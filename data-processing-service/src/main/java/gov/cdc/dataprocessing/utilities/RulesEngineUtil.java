...
public class RulesEngineUtil {
  ...
  public static int[] CalcMMWR(String pDate) {
    ...
    int[] r = {0, 0};
    if (pDate == null || pDate.trim().isEmpty()) return r;
    try {
      //  Define constants.
      int SECOND = 1000;
      ...
      Date varJan1Date =
          new SimpleDateFormat("MM/dd/yyyy").parse("01/01/" + cal.get(Calendar.YEAR));
      ...
      if (varJan1Day < 5) {
        ...
        if (w == 53) {
          Date varNextJan1Date =
              new SimpleDateFormat("MM/dd/yyyy").parse("01/01/" + (cal.get(Calendar.YEAR) + 1));

...
public class PageRepositoryUtil {
  ...
  public Long setPageActProxyVO(PageActProxyContainer pageProxyVO) throws DataProcessingException {
    PublicHealthCaseDto phcDT =
        pageProxyVO.getPublicHealthCaseContainer().getThePublicHealthCaseDto();
    boolean isCoInfectionCondition =
        pageProxyVO.getPublicHealthCaseContainer().isCoinfectionCondition();
    Long mprUid;
    ...
    if (pageProxyVO.getMessageLogDTMap() != null && !pageProxyVO.getMessageLogDTMap().isEmpty()) {
      ...
      for (String key : set) {
        if (key.contains(MessageConstants.DISPOSITION_SPECIFIED_KEY)) {
          // Investigator of Named by contact will get message for Named by contact and contact's
          // investigation id.
          continue;
        }
    ...
    if (pageProxyVO.isUnsavedNote()
        && pageProxyVO.getNbsNoteDTColl() != null
  ...
  public void updatForConInfectionId(
      PageActProxyContainer pageActProxyContainer, Long mprUid, Long currentPhclUid)
      throws DataProcessingException {
    updateForConInfectionId(pageActProxyContainer, null, mprUid, null, currentPhclUid, null, null);
  }

  /**
   * @param pageActProxyContainer: PageActProxyContainer that will update the other investigations
   *     that are part of co-infection group
  ...
  public void updateForConInfectionId(
      ...
      PageActProxyContainer supersededProxyVO,
      Long mprUid,
      Map<Object, Object> coInSupersededEpliLinkIdMap,
      Long currentPhclUid,
      Collection<Object> coinfectionSummaryVOCollection,
      String coinfectionIdToUpdate)
      throws DataProcessingException {
    String coninfectionId =
    ...
    Map<Object, Object> updateValueInOtherTablesMap =
        new HashMap<>(); // Map is to update values in other table then NBS_CASE_Answer
      ...
      for (Object o : coinfectionSummaryVOCollection) {
        ...
        if (coninfectionSummaryVO.getPublicHealthCaseUid().compareTo(currentPhclUid) != 0) {
          if (coinfectionIdToUpdate != null) { // Merge Case investigation scenario
            updateCoInfectionInvest(
                updatedValuesMap,
                mapFromQuestions,
                pageActProxyContainer,
                pageActProxyContainer.getPublicHealthCaseContainer(),
                supersededProxyVO.getPublicHealthCaseContainer(),
                coInSupersededEpliLinkIdMap,
                coninfectionSummaryVO,
                coinfectionIdToUpdate,
                updateValueInOtherTablesMap);
            /** Update for closed/open cases that are part of any co-infection groups */
          } else {
            updateCoInfectionInvest(
                updatedValuesMap,
  ...
  protected ArrayList<Object> getInvListForCoInfectionId(Long mprUid, String coInfectionId)
      throws DataProcessingException {
  ...
  protected void updateCoInfectionInvest(
      Map<Object, Object> mappedCoInfectionQuestions,
      Map<Object, Object> fromMapQuestions,
      PageActProxyContainer pageActProxyContainer,
    ...
    PageActProxyContainer proxyVO =
        investigationService.getPageProxyVO(NEDSSConstant.CASE, publicHealthCaseUid);
    /**
     * Merge Investigation case issue where the superseded investigation should not allowed to
     * update!!! 1. Only cases that are not Merge Investigation are allowed to proceed 2. Only cases
     * that are Merge Investigation that are not superseded are allowed to proceed 3. Even Closed
     * cases that are part of co-infection are NOW allowed to proceed with updated co-infection
     * id(https://nbsteamdev.atlassian.net/browse/ND-9114 Description Losing investigation's
     * Coinfection is not assigned the correct Co-Infection Id when status = Closed)
     */
    // Set the winning investigation's coinfectionId to losing investigation's related co-infection
    // investigations.
    if (coinfectionIdToUpdate != null) {
      String survivingEpiLinkId =
          publicHealthCaseContainer.getTheCaseManagementDto().getEpiLinkId();

      if (coInSupersededEpliLinkIdMap.get(
              proxyVO
                  .getPublicHealthCaseContainer()
                  .getThePublicHealthCaseDto()
        ...
        proxyVO
            .getPublicHealthCaseContainer()
            .getTheCaseManagementDto()
            .setEpiLinkId(survivingEpiLinkId);
      }
      proxyVO
          .getPublicHealthCaseContainer()
          .getThePublicHealthCaseDto()
          .setCoinfectionId(coinfectionIdToUpdate);
      proxyVO.setMergeCase(true);
    }

    if (coinfectionIdToUpdate == null
        || (supersededPublicHealthCaseContainer != null // NOSONAR
            && publicHealthCaseUid.compareTo(
      ...
      logger.debug(
          "updateCoInfectionInvest method call completed for coinfectionIdToUpdate: {}",
          coinfectionIdToUpdate);
    }
  }

  @SuppressWarnings("java:S3776")
  private void updatePageProxyVOInterface(
      PageActProxyContainer proxyActVO, Timestamp lastChgTime, Long lastChgUserId) {
  ...
  @SuppressWarnings("java:S3776")
  protected void processingParticipationPatTypeForPageAct(
      PageActProxyContainer pageActProxyContainer) throws DataProcessingException {
    if (pageActProxyContainer.isItNew() && (!pageActProxyContainer.isItDirty())) {
      // changes according to new Analysis
      String classCd;
      String recordStatusCd;

      for (ParticipationDto participationDto :
          pageActProxyContainer.getTheParticipationDtoCollection()) {

        if (participationDto.getSubjectEntityUid() != null
            && participationDto.getSubjectEntityUid().intValue() > 0) {
          ...
          if (classCd != null && classCd.compareToIgnoreCase(NEDSSConstant.PERSON) == 0) {
            ...
            PersonContainer personVO =
                patientRepositoryUtil.loadPerson(participationDto.getSubjectEntityUid());
            recordStatusCd = personVO.getThePersonDto().getRecordStatusCd();

            if (recordStatusCd != null
                && recordStatusCd
                        .trim()
                        .compareToIgnoreCase(NEDSSConstant.RECORD_STATUS_LOGICAL_DELETE)
                    == 0) {
              throw new DataProcessingException(
  ...
  protected PageActPatient processingPersonContainerForPageAct(
      PageActProxyContainer pageActProxyContainer, PublicHealthCaseDto phcDT)
      throws DataProcessingException {
    PersonContainer personVO;
    ...
    if (pageActProxyContainer.getThePersonContainerCollection() != null) {
      for (anIterator = pageActProxyContainer.getThePersonContainerCollection().iterator();
          anIterator.hasNext(); ) {
        ...
        if (personVO.isItNew()) {
          if (personVO.getThePersonDto().getCd() != null
            ...
            personVO
                .getThePersonDto()
                .setPersonUid(personVO.getThePersonDto().getPersonParentUid());

            patientRevisionUid =
                patientMatchingBaseService.setPatientRevision(
                    personVO, businessTriggerCd, NEDSSConstant.PAT);
          ...
          } else if (personVO.getThePersonDto().getCd() != null
              && personVO.getThePersonDto().getCd().equals(NEDSSConstant.PRV)) {
            // Provider
  ...
  protected PageActPhc processingPhcContainerForPageAct(
      PageActProxyContainer pageActProxyContainer, boolean isCoInfectionCondition)
      throws DataProcessingException {
    ...
    if (pageActProxyContainer.getPublicHealthCaseContainer() != null) {
      ...
      if (publicHealthCaseContainer.getNbsAnswerCollection() != null) {
        logger.debug(
            "********#publicHealthCaseContainer.getNbsAnswerCollection() size from history table: {}",
            publicHealthCaseContainer.getNbsAnswerCollection().size());
      }
      if (publicHealthCaseDto.getPublicHealthCaseUid() != null
          && publicHealthCaseDto.getVersionCtrlNbr() != null) {
        logger.debug(
            "********#Public Health Case Uid: {} Version: {}",
            publicHealthCaseDto.getPublicHealthCaseUid(),
            publicHealthCaseDto.getVersionCtrlNbr());
      }
      ...
      if (pageActProxyContainer.isItNew()) {
        ...
        if (isCoInfectionCondition
            && pageActProxyContainer
                ...
                == null) {
          logger.debug(
              "AssociatedInvestigationUpdateUtil.updatForConInfectionId created an new coinfection id for the case");
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .setCoinfectionId(NEDSSConstant.COINFCTION_GROUP_ID_NEW_CODE);
        }
      ...
      publicHealthCaseDto =
          (PublicHealthCaseDto)
              prepareAssocModelHelper.prepareVO(
                  ...
                  tableName,
                  moduleCd,
                  rootDTInterface.getVersionCtrlNbr());
      publicHealthCaseContainer.setThePublicHealthCaseDto(publicHealthCaseDto);

      falsePublicHealthCaseUid =
          publicHealthCaseContainer.getThePublicHealthCaseDto().getPublicHealthCaseUid();
      ...
      if (falsePublicHealthCaseUid.intValue() < 0) {
        logger.debug(
            "falsePublicHealthCaseUid.intValue() = {}", falsePublicHealthCaseUid.intValue());
        uidService.setFalseToNewForPageAct(
            pageActProxyContainer, falsePublicHealthCaseUid, actualUid);
  ...
  private Long processingPhcActRelationshipForPageAct(PageActProxyContainer pageActProxyContainer)
      throws DataProcessingException {
    Long docUid = null;
    Iterator<ActRelationshipDto> anIteratorActRelationship;
    if (pageActProxyContainer.getPublicHealthCaseContainer().getTheActRelationshipDTCollection()
        != null) {
      for (anIteratorActRelationship =
              pageActProxyContainer
                  .getPublicHealthCaseContainer()
                  .getTheActRelationshipDTCollection()
                  .iterator();
          anIteratorActRelationship.hasNext(); ) {
        ActRelationshipDto actRelationshipDT = anIteratorActRelationship.next();
        if (actRelationshipDT.getTypeCd() != null
            && actRelationshipDT.getTypeCd().equals(NEDSSConstant.DocToPHC)) {
        ...
        logger.debug(
            "Got into The ActRelationship, The ActUid is {}", actRelationshipDT.getTargetActUid());
      }
    }
  ...
  protected void processingParticipationForPageAct(PageActProxyContainer pageActProxyContainer)
      throws DataProcessingException {
  ...
  @SuppressWarnings("java:S3776")
  protected void processingNotificationSummaryForPageAct(
      PageActProxyContainer pageActProxyContainer, PublicHealthCaseDto phcDT)
      throws DataProcessingException {
    if (pageActProxyContainer.getTheNotificationSummaryVOCollection() != null) {
      Collection<Object> notSumVOColl =
          pageActProxyContainer.getTheNotificationSummaryVOCollection();
      for (Object o : notSumVOColl) {
        ...
        if (notSummaryVO.getIsHistory().equals("F")
          ...
          if (notificationRecordStatusCode != null) {
            ...
            if (notificationRecordStatusCode.equalsIgnoreCase(
  ...
  private void processingEventProcessForPageAct(
      PageActProxyContainer pageActProxyContainer, Long phcUid) throws DataProcessingException {
    if (pageActProxyContainer.getPublicHealthCaseContainer().getEdxEventProcessDtoCollection()
        != null) {
      for (EDXEventProcessDto processDT :
          pageActProxyContainer.getPublicHealthCaseContainer().getEdxEventProcessDtoCollection()) {
        if (processDT.getDocEventTypeCd() != null
            && processDT.getDocEventTypeCd().equals(NEDSSConstant.CASE)) {
  ...
  protected void processingNbsDocumentForPageAct(
      PageActProxyContainer pageActProxyContainer, Long docUid) throws DataProcessingException {
    if (docUid != null) {

      // get the
      NbsDocumentContainer nbsDocVO =
          nbsDocumentRepositoryUtil.getNBSDocumentWithoutActRelationship(docUid);
  ...
  protected void handlingCoInfectionAndContactDisposition(
      PageActProxyContainer pageActProxyContainer, Long mprUid, Long actualUid)
      throws DataProcessingException {
    if (!pageActProxyContainer.isRenterant()
        && pageActProxyContainer
                .getPublicHealthCaseContainer()
                .getThePublicHealthCaseDto()
                .getCoinfectionId()
        ...
        && !pageActProxyContainer
            ...
            .getThePublicHealthCaseDto()
            .getCoinfectionId()
            .equalsIgnoreCase(NEDSSConstant.COINFCTION_GROUP_ID_NEW_CODE)
        && mprUid != null
        && !pageActProxyContainer.isMergeCase()
        && !NEDSSConstant.INVESTIGATION_STATUS_CODE_CLOSED.equals(
            pageActProxyContainer
                .getPublicHealthCaseContainer()
                .getThePublicHealthCaseDto()

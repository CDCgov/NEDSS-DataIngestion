...
public class AutoInvestigationService implements IAutoInvestigationService {
  ...
  public AutoInvestigationService(
  ...
  public Object autoCreateInvestigation(
      ObservationContainer observationVO, EdxLabInformationDto edxLabInformationDT)
    ...
    if (edxLabInformationDT.getInvestigationType() != null
        && edxLabInformationDT
            ...
            .equalsIgnoreCase(NEDSSConstant.INV_FORM_RVCT)) {
  ...
  public Object transferValuesTOActProxyVO(
      PageActProxyContainer pageActProxyContainer,
      PamProxyContainer pamActProxyVO,
    ...
    if (entities != null && !entities.isEmpty()) {
      for (Object entity : entities) {
        ...
        createActEntityObject(
            participationDT, pageActProxyContainer, pamActProxyVO, nbsActEntityDTColl, partColl);
      }
    }
    if (coll != null) {
      ...
      for (ParticipationDto partDT : coll) {
        ...
        if (typeCd.equalsIgnoreCase(EdxELRConstant.ELR_AUTHOR_CD)
            && partDT.getSubjectClassCd().equals(EdxELRConstant.ELR_ORG)
            && !isOrgAsReporterOfPHCPartDT) {
          createActEntity = true;
          partDT.setTypeCd("OrgAsReporterOfPHC");
        }
        if (typeCd.equalsIgnoreCase(EdxELRConstant.ELR_ORDER_CD)
            && partDT.getSubjectClassCd().equals(EdxELRConstant.ELR_PERSON_CD)
            && !isPhysicianOfPHCDT) {
          createActEntity = true;
        ...
        if (createActEntity) {
          createActEntityObject(
              partDT, pageActProxyContainer, pamActProxyVO, nbsActEntityDTColl, partColl);
        }
      }
    }
  ...
  private PublicHealthCaseContainer createPublicHealthCaseVO(
      ObservationContainer observationVO, EdxLabInformationDto edxLabInformationDT)
      throws DataProcessingException {
    ...
    phcVO
        .getThePublicHealthCaseDto()
        .setPublicHealthCaseUid((long) (edxLabInformationDT.getNextUid() - 1));
    phcVO
        .getThePublicHealthCaseDto()
        .setJurisdictionCd((observationVO.getTheObservationDto().getJurisdictionCd()));
    phcVO
        .getThePublicHealthCaseDto()
        .setRptFormCmpltTime(observationVO.getTheObservationDto().getRptToStateTime());
    ...
    var res =
        conditionCodeRepository.findProgramAreaConditionCodeByConditionCode(
            edxLabInformationDT.getConditionCode());
    ...
    if (cacheApiService.getSrteCacheBool(
        ObjectName.CHECK_PAI_FOR_STD_OR_HIV.name(), programAreaVO.getStateProgAreaCode())) {
    ...
    phcVO
        .getThePublicHealthCaseDto()
        .setActivityFromTime(
            StringUtils.stringToStrutsTimestamp(
                StringUtils.formatDate(TimeStampUtil.getCurrentTimeStamp(tz))));
    ...
    if (edxLabInformationDT.getConditionCode() != null) {
      ...
      phcVO.setCoinfectionCondition(
          cacheApiService.getSrteCacheBool(
              ObjectName.CO_INFECTION_CONDITION_CODE.name(),
              edxLabInformationDT.getConditionCode()));
    ...
    boolean isSTDProgramArea =
        cacheApiService.getSrteCacheBool(
            ObjectName.CHECK_PAI_FOR_STD_OR_HIV.name(),
            phcVO.getThePublicHealthCaseDto().getProgAreaCd());
    if (isSTDProgramArea) {
  ...
  protected void populateProxyFromPrePopMapping(
      PageActProxyContainer pageActProxyContainer, EdxLabInformationDto edxLabInformationDT)
      throws DataProcessingException {
    ...
    Collection<ObservationContainer> obsCollection =
        edxLabInformationDT.getLabResultProxyContainer().getTheObservationContainerCollection();
    ...
    for (ObservationContainer obs : obsCollection) {
      if (obs.getTheObsValueNumericDtoCollection() != null
        ...
        List<ObsValueNumericDto> obsValueNumList =
            new ArrayList<>(obs.getTheObsValueNumericDtoCollection());
        String value =
            obsValueNumList.getFirst().getNumericUnitCd() == null
        ...
        prePopMap.put(obs.getTheObservationDto().getCd(), value);
          ...
          && fromPrePopMap.containsKey(obs.getTheObservationDto().getCd())) {
        List<ObsValueDateDto> obsValueDateList =
            new ArrayList<>(obs.getTheObsValueDateDtoCollection());
        ...
        List<ObsValueCodedDto> obsValueCodeList =
            new ArrayList<>(obs.getTheObsValueCodedDtoCollection());

        String key =
            obs.getTheObservationDto().getCd() + "$" + obsValueCodeList.getFirst().getCode();
      ...
      } else if (obs.getTheObsValueTxtDtoCollection() != null
          && !obs.getTheObsValueTxtDtoCollection().isEmpty()
          && fromPrePopMap.containsKey(obs.getTheObservationDto().getCd())) {
        for (ObsValueTxtDto obsValueTxtDT : obs.getTheObsValueTxtDtoCollection()) {
          if (obsValueTxtDT.getTxtTypeCd() == null
              || obsValueTxtDT.getTxtTypeCd().trim().isEmpty()
  ...
  @SuppressWarnings({"java:S3776", "java:S6541"})
  private void populateFromPrePopMapping(
      TreeMap<Object, Object> prePopMap, PageActProxyContainer pageActProxyContainer)
      throws DataProcessingException {
    PublicHealthCaseDto phcDT =
        pageActProxyContainer.getPublicHealthCaseContainer().getThePublicHealthCaseDto();
    var res =
        conditionCodeRepository.findProgramAreaConditionCode(
            1, new ArrayList<>(Collections.singletonList(phcDT.getProgAreaCd())));
    ...
    if (programAreaVO == null) // level 2 condition for Hepatitis Diagnosis
    {
      var res2 =
          conditionCodeRepository.findProgramAreaConditionCode(
              2, new ArrayList<>(Collections.singletonList(phcDT.getProgAreaCd())));
      if (res2.isPresent()) {
        var items = res2.get();
    ...
    Map<Object, Object> questionMap =
        (Map<Object, Object>) OdseCache.dmbMap.get(investigationFormCd);

    if (prePopMap == null || prePopMap.isEmpty()) return;
    ...
    if (toPrePopMap != null && !toPrePopMap.isEmpty()) {
      ...
      for (Object obj : toPrePopColl) {
        ...
        String mappingKey =
            toPrePopMappingDT.getFromAnswerCode() == null
        ...
        if (prePopMap.containsKey(mappingKey)) {
          ...
          NbsQuestionMetadata quesMetadata =
              (NbsQuestionMetadata) questionMap.get(toPrePopMappingDT.getToQuestionIdentifier());
  ...
  private void createActEntityObject(
      ParticipationDto partDT,
      PageActProxyContainer pageActProxyContainer,
      PamProxyContainer pamActProxyVO,
      Collection<NbsActEntityDto> nbsActEntityDTColl,
    ...
    if (pageActProxyContainer != null) {
      nbsActEntityDT.setAddTime(
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .getAddTime());
      nbsActEntityDT.setLastChgTime(
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .getLastChgTime());
      nbsActEntityDT.setLastChgUserId(
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .getLastChgUserId());
      nbsActEntityDT.setRecordStatusCd(
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .getRecordStatusCd());
      nbsActEntityDT.setAddUserId(
          pageActProxyContainer
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
              .getAddUserId());
      ...
      nbsActEntityDT.setAddTime(
          pamActProxyVO.getPublicHealthCaseContainer().getThePublicHealthCaseDto().getAddTime());
      nbsActEntityDT.setLastChgTime(
          pamActProxyVO
              .getPublicHealthCaseContainer()
              .getThePublicHealthCaseDto()
      ...
      nbsActEntityDT.setRecordStatusCd(
          pamActProxyVO
              ...
              .getRecordStatusCd());
      nbsActEntityDT.setAddUserId(
          pamActProxyVO.getPublicHealthCaseContainer().getThePublicHealthCaseDto().getAddUserId());
    }

...
public class ProviderMatchingBaseService extends MatchingBaseService {
  ...
  public String nameAddressStreetOneProvider(PersonContainer personContainer) {
    ...
    if (nameAddStr != null) nameAddStr = getNameStringForProvider(personContainer) + nameAddStr;
    return nameAddStr;
  }

  @SuppressWarnings("java:S1172")
  public Long processingProvider(
      PersonContainer personContainer, String businessObjLookupName, String businessTriggerCd)
      throws DataProcessingException {
    boolean callOrgHashCode = false;
    if (personContainer.isItNew()
        && personContainer.getThePersonDto().isItNew()
        && personContainer.getThePersonDto().getElectronicInd().equalsIgnoreCase("Y")
  ...
  @SuppressWarnings("java:S1172")
  protected Long persistingProvider(
      PersonContainer personContainer, String businessObjLookupName, String businessTriggerCd)
      throws DataProcessingException {
    Long personUID;
    String localId;
  ...
  protected void setProvidertoEntityMatch(PersonContainer personContainer)
      throws DataProcessingException {
  ...
  @SuppressWarnings("java:S3776")
  protected List<String> getIdentifierForProvider(PersonContainer personContainer)
      throws DataProcessingException {
    String carrot = "^";
        ...
        for (EntityIdDto entityIdDto : entityIdDtoColl) {
          if ((entityIdDto.getStatusCd().equalsIgnoreCase(NEDSSConstant.STATUS_ACTIVE))) {
            if ((entityIdDto.getRootExtensionTxt() != null)
              ...
              identifier =
                  entityIdDto.getRootExtensionTxt()
                      + carrot
                      + entityIdDto.getTypeCd()
                      + carrot
                      + entityIdDto.getAssigningAuthorityCd()
                      + carrot
                      + entityIdDto.getAssigningAuthorityDescTxt()
                      + carrot
                      + entityIdDto.getAssigningAuthorityIdType();
            } else {
            ...
            if (entityIdDto.getTypeCd() != null
                && !entityIdDto.getTypeCd().equalsIgnoreCase("LR")) {
    ...
    if (personContainer.getThePersonNameDtoCollection() != null
      ...
      for (PersonNameDto personNameDto : personNameDtoColl) {
        if (personNameDto.getNmUseCd() != null
                && personNameDto.getNmUseCd().equals(NEDSSConstant.LEGAL)
                && personNameDto.getLastNm() != null

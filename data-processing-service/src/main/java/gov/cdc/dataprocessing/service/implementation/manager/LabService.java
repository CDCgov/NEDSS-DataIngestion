...
public class LabService {
  ...
  public Long handlePageContainer(PageActProxyContainer pageAct, EdxLabInformationDto edxDto)
  ...
  public Long handlePamContainer(PamProxyContainer pamProxy, EdxLabInformationDto edxDto)
      throws DataProcessingException {
    return pamService.setPamProxyWithAutoAssoc(
        pamProxy, edxDto.getRootObserbationUid(), NEDSSConstant.LABRESULT_CODE);
  }

  public void handleMarkAsReviewed(ObservationDto obsDto, EdxLabInformationDto edxDto)
      throws DataProcessingException {
  ...
  public void handleNndNotification(
      PublicHealthCaseContainer phcContainerModel, EdxLabInformationDto edxDto)
      throws DataProcessingException {
    EDXActivityDetailLogDto detailLog =
        investigationNotificationService.sendNotification(
            phcContainerModel, edxDto.getNndComment());
    detailLog.setRecordType(EdxELRConstant.ELR_RECORD_TP);
    detailLog.setRecordName(EdxELRConstant.ELR_RECORD_NM);

    ArrayList<EDXActivityDetailLogDto> details =
        (ArrayList<EDXActivityDetailLogDto>)
            edxDto.getEdxActivityLogDto().getEDXActivityLogDTWithVocabDetails();
    if (details == null) {

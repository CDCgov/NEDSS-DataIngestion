...
public class ProgramAreaService implements IProgramAreaService {
  ...
  private HashMap<String, String> getProgramAreaHelper(
    ...
    while (obsResults.hasNext()) {
      ...
      if (obsDomainCdSt1 != null
        ...
        if (electronicInd.equals(NEDSSConstant.ELECTRONIC_IND_ELR)) {
          progAreaCd =
              srteCodeObsService.getPAFromSNOMEDCodes(
        ...
        if (!found) {
          progAreaCd =
              srteCodeObsService.getPAFromLocalResultCode(
                  reportingLabCLIA, obsResult.getTheObsValueCodedDtoCollection());
          // If PA returned, check to see if it is the same one as before.
          if (progAreaCd != null) {
            found = true;
  ...
  public String deriveProgramAreaCd(
    ...
    if (!orderTest
      ...
      if (paResults != null && paResults.containsKey(ELRConstant.PROGRAM_AREA_HASHMAP_KEY)) {
        orderTest
            .getTheObservationDto()
            .setProgAreaCd((String) paResults.get(ELRConstant.PROGRAM_AREA_HASHMAP_KEY));
      } else {

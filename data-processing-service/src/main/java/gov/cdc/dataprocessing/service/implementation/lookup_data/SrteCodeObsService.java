...
public class SrteCodeObsService implements ISrteCodeObsService {
  ...
  public SrteCodeObsService(
      ProgAreaSnomeCodeStoredProcRepository progAreaSnomeCodeStoredProcRepository,
  ...
  public Map<String, Object> getSnomed(String code, String type, String clia)
  ...
  public String getDefaultConditionForLocalResultCode(String labResultCd, String laboratoryId) {
    var result =
        labResultRepository.findDefaultConditionCdByLabResultCdAndLaboratoryId(
            labResultCd, laboratoryId);
    if (result.isPresent()) {
      return result.get().getFirst();
    } else {
      return "";
  ...
  public ObservationContainer labLoincSnomedLookup(ObservationContainer obsVO, String labClia) {
     ...
     */
    if (labClia == null
        && obsVO.getTheObservationDto().getCdSystemCd().equals(NEDSSConstant.DEFAULT)) {
  ...
  private void doSnomedCdLookupForObsValueCodedDTs(
      Collection<ObsValueCodedDto> obsValueCodedDtos, String labClia) {
  ...
  public HashMap<Object, Object> getProgramArea(
    ...
    while (obsIt.hasNext()) {
      ...
      // Set exclude flag to false - if any of the components - Lab Result (SNOMED or Local) or Lab
      // Test (LOINC or
      // Local) is excluded, this flag will be set so as not to fail the derivation for this
      // resulted test.
      ...
      if ((obsDomainCdSt1 != null)
        ...
        if (electronicInd.equals(NEDSSConstant.ELECTRONIC_IND_ELR)) {
          progAreaCd =
              getPAFromSNOMEDCodes(reportingLabCLIA, obsVO.getTheObsValueCodedDtoCollection());
        } else {
          progAreaCd =
              getPAFromSNOMEDCodes(NEDSSConstant.DEFAULT, obsVO.getTheObsValueCodedDtoCollection());
        }
  ...
  /**
   * Returns a collection of Snomed codes to be used to resolve the program area code. If more than
   * one type of snomed is resolved, return null.
   *
  ...
  public String getPAFromSNOMEDCodes(
      String reportingLabCLIA, Collection<ObsValueCodedDto> obsValueCodedDtoColl)
      throws DataProcessingException {
    ...
    if (obsValueCodedDtoColl != null) {
      for (ObsValueCodedDto codedDt : obsValueCodedDtoColl) {
        ...
        if (!codeSystemCd.equals(ELRConstant.ELR_SNOMED_CD)) {
          // If local code and it is not excluded from PA Derivation, attempt to retrieve
          // corresponding SNOMED code
          if (!removePADerivationExcludedLabResultCodes(obsCode, reportingLabCLIA)) {
            Map<String, Object> snomedList =
                getSnomed(codedDt.getCode(), ELRConstant.TYPE, reportingLabCLIA);
            if (snomedList.containsKey(SELECT_COUNT)
                && (Integer) snomedList.get(SELECT_COUNT) == 1) {
              snomedVector.addElement(snomedList.get("LOINC"));
            }
          } else {
            // If so, set exclude flag so we won't fail this resulted test if no PA is derived for
            // it
         ...
         */
          ...
          if (!removePADerivationExcludedSnomedCodes(obsCode)) {
            ...
            // Otherwise don't add it and set the exclude flag so we won't fail this resulted test
            // if no PA is derived for it
            programAreaDerivationExcludeFlag = true;
          }
        }
      } // end of while
  ...
  private boolean removePADerivationExcludedSnomedCodes(String snomedCd) {
    ...
    if (result.isPresent()) {
      for (var item : result.get()) {
        if (item.getPaDerivationExcludeCd() != null
            && item.getPaDerivationExcludeCd().equals(NEDSSConstant.YES)) {
          return true;
        }
      }
    }
  ...
  /**
   * Description: Find whether if record with Observation value and CLIA exist in LabResult table or
   * not. If exist then check for ExcludeCode. Return True if value is Yes. Otherwise return False.
   * Program Area use this as indicator to check for SNOMED code when method return false
   */
  private boolean removePADerivationExcludedLabResultCodes(
      String labResultCd, String reportingLabCLIA) {
    var result =
        labResultRepository.findLabResultProgramAreaExclusion(labResultCd, reportingLabCLIA);
    if (result.isPresent()) {
      for (var item : result.get()) {
        if (item.getPaDerivationExcludeCd() != null
            && item.getPaDerivationExcludeCd().equals(NEDSSConstant.YES)) {
  ...
  // AK 7/25/04
  @SuppressWarnings("java:S1149")
  protected String getProgAreaCd(
      Vector<Object> codeVector, String reportingLabCLIA, String nextLookUp, String type) {
    ...
    try {
      for (int k = 0; k < codeVector.size(); k++) {
        progAreaCdList =
            progAreaSnomeCodeStoredProcRepository.getProgAreaCd(
                (String) codeVector.elementAt(k), type, reportingLabCLIA);
   ...
   */
  // AK - 7/25/04
  public String getPAFromLOINCCode(String reportingLabCLIA, ObservationContainer resultTestVO)
      throws DataProcessingException {
    ...
    if (cdSystemCd.equals(ELRConstant.ELR_OBSERVATION_LOINC)) {
      ...
      Map<String, Object> loincList =
          progAreaSnomeCodeStoredProcRepository.getSnomed(obsCode, "LT", reportingLabCLIA);
  ...
  private boolean removePADerivationExcludedLabTestCodes(
      String labTestCd, String reportingLabCLIA) {
  ...
  private boolean removePADerivationExcludedLoincCodes(String loincCd) {
    ...
    if (result.isPresent()) {
      for (var item : result.get()) {
        if (item.getPaDerivationExcludeCode() != null
            && item.getPaDerivationExcludeCode().equals(NEDSSConstant.YES)) {
          return true;
        }
      }
    }
  ...
  public String getPAFromLocalResultCode(
      String reportingLabCLIA, Collection<ObsValueCodedDto> obsValueCodedDtoColl) {
    ...
    for (ObsValueCodedDto obsValueCodedDto : obsValueCodedDtoColl) {
      ...
      progAreaCd =
          findLocalResultDefaultConditionProgramAreaCd(codeVector, reportingLabCLIA, "NEXT");

      if (progAreaCd == null) {
        progAreaCd =
            findLocalResultDefaultConditionProgramAreaCdFromLabResult(
                codeVector, reportingLabCLIA, "NEXT");
      }
  ...
  private String findLocalResultDefaultConditionProgramAreaCd(
      Vector<Object> codeVector, String reportingLabCLIA, String nextLookup) {
    String lastPACode = null;
    try {
      for (int k = 0; k < codeVector.size(); k++) {
        var result =
            conditionCodeRepository.findConditionCodeByLabResultLabIdAndCd(
                codeVector.elementAt(k).toString(), reportingLabCLIA);
  ...
  private String findLocalResultDefaultConditionProgramAreaCdFromLabResult(
      Vector<Object> codeVector, String reportingLabCLIA, String nextLookup) {
    String lastPACode = null;
    try {
      for (int k = 0; k < codeVector.size(); k++) {
        var result =
            labResultRepository.findLocalResultDefaultProgramAreaCd(
                codeVector.elementAt(k).toString(), reportingLabCLIA);
        Collection<String> defaultPACColl = new ArrayList<>();
        if (result.isPresent()) {
          defaultPACColl = result.get();
  ...
  public String getPAFromLocalTestCode(String reportingLabCLIA, ObservationContainer resultTestVO) {
    ...
    progAreaCd =
        findLocalResultDefaultConditionProgramAreaCdFromLabTest(
            codeVector, reportingLabCLIA, "NEXT");

    if (progAreaCd == null) {
      progAreaCd =
          findLocalResultDefaultConditionProgramAreaCdFromLabTestWithoutJoin(
              codeVector, reportingLabCLIA, "NEXT");
    }
    return progAreaCd;
  } // end of method
  ...
  @SuppressWarnings("java:S1172")
  private String findLocalResultDefaultConditionProgramAreaCdFromLabTest(
      Vector<Object> codeVector, String reportingLabCLIA, String nextLookup) {
    String lastPACode = null;
    try {
      for (int k = 0; k < codeVector.size(); k++) {
        var result =
            conditionCodeRepository.findLocalTestDefaultConditionProgramAreaCd(
                codeVector.elementAt(k).toString(), reportingLabCLIA);
        Collection<String> defaultPACColl = new ArrayList<>();
        if (result.isPresent()) {
          defaultPACColl = result.get();
  ...
  protected String findLocalResultDefaultConditionProgramAreaCdFromLabTestWithoutJoin(
    ...
    try {
      for (int k = 0; k < codeVector.size(); k++) {
        var result =
            labTestRepository.findLocalTestDefaultProgramAreaCd(
                codeVector.elementAt(k).toString(), reportingLabCLIA);
        Collection<String> defaultPACColl = new ArrayList<>();
        if (result.isPresent()) {
          defaultPACColl = result.get();
  ...
  private String getLocalTestCode(ObservationDto obsDt) {
    String code = null;
    if (obsDt != null
        && obsDt.getCdSystemCd() != null
        && (obsDt.getCd() != null && !obsDt.getCd().isEmpty() && !obsDt.getCd().equals(" "))) {
      code = obsDt.getCd();
    } // end of if
    return code;

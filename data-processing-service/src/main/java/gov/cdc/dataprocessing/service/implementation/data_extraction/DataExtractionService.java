...
public class DataExtractionService implements IDataExtractionService {
  ...
  public LabResultProxyContainer parsingDataToObject(
      NbsInterfaceModel nbsInterfaceModel, EdxLabInformationDto edxLabInformationDto)
      throws JAXBException, DataProcessingException {
    int rootObsUid = 0;
    ...
    Container container =
        dataExtractionServiceUtility.parsingElrXmlPayload(nbsInterfaceModel.getPayload());
    ...
    if (patientResults == null || patientResults.isEmpty()) {
      ...
      logger.error(
          "No patient segment found. NBS_INTERFACE_UID: {}",
    ...
    if (patientResults.size() > 1) {
      ...
      edxLabInformationDto.setErrorText(EdxELRConstant.ELR_MASTER_LOG_ID_13);
      logger.error(
          "Multiple patient segments found. NBS_INTERFACE_UID: {}",
          nbsInterfaceModel.getNbsInterfaceUid());
    ...
    labResultProxyContainer =
        hl7PatientHandler.getPatientAndNextOfKin(
            patientResult, labResultProxyContainer, edxLabInformationDto);
    ...
    if (orderObservations == null || orderObservations.isEmpty()) {
      ...
      logger.error(
          "No OBR segment found. NBS_INTERFACE_UID: {}", nbsInterfaceModel.getNbsInterfaceUid());
    ...
    for (int i = 0; i < orderObservations.size(); i++) {
              ...
              : (observationRequest.getParent() == null
                  ...
                  || (observationRequest
                      ...
                      && observationRequest
                              ...
                              .getHL7AlternateText()
                          == null)
                  || observationRequest.getParentResult().getParentObservationValueDescriptor()
                      == null
                  || observationRequest
                          .getParentResult()
      ...
      if (isParentInvalid) {
        if (i == 0) {
          ...
          edxLabInformationDto.setErrorText(EdxELRConstant.ELR_MASTER_LOG_ID_13);
          logger.error(
              "Invalid parent section for first OBR. NBS_INTERFACE_UID: {}",
              nbsInterfaceModel.getNbsInterfaceUid());
          ...
          edxLabInformationDto.setErrorText(EdxELRConstant.ELR_MASTER_LOG_ID_13);
          logger.error(
              "Invalid parent section for OBR {}. NBS_INTERFACE_UID: {}",
              i + 1,
              nbsInterfaceModel.getNbsInterfaceUid());
      ...
      observationRequestHandler.getObservationRequest(
          observationRequest, spm, labResultProxyContainer, edxLabInformationDto);

      // Async DB update (fire and forget)
      try {
        Timestamp effectiveFromTime =
            Optional.ofNullable(edxLabInformationDto.getRootObservationContainer())
        ...
        if (effectiveFromTime != null) {
          CompletableFuture.runAsync(
              () -> {
                try {
                  nbsInterfaceStoredProcRepository.updateSpecimenCollDateSP(
                      edxLabInformationDto.getNbsInterfaceUid(), effectiveFromTime);
                } catch (Exception e) {

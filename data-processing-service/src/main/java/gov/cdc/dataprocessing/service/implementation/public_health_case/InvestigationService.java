...
public class InvestigationService implements IInvestigationService {
  ...
  public void setAssociations(
      ...
      Boolean isNNDResendCheckRequired)
   ...
   *
   * @param investigationUID -- The UID for the investigation to which observation is to be
   *     associated or disassociates
  ...
  public void setObservationAssociationsImpl(
      Long investigationUID,
      Collection<LabReportSummaryContainer> reportSumVOCollection,
      boolean invFromEvent)
      throws DataProcessingException {
    PublicHealthCaseDto phcDT =
        publicHealthCaseRepositoryUtil.findPublicHealthCase(investigationUID);
    ...
    if (!reportSumVOCollection.isEmpty()) {
      for (LabReportSummaryContainer reportSumVO : reportSumVOCollection) {
        ...
        // Gets and checks whether any association change; if changed, do something, else go next
        // one
        ...
        // Set from time same as investigation create time if act relationship is created while
        // creating investigation from lab or morbidity report
        if (invFromEvent) {
        ...
        actRelationshipDT =
            prepareAssocModelHelper.prepareAssocDTForActRelationship(actRelationshipDT);
  ...
  protected RootDtoInterface processingNonAssociatedReportSummaryContainer(
      LabReportSummaryContainer reportSumVO, ObservationDto obsDT, RootDtoInterface rootDT)
      throws DataProcessingException {
    if (!reportSumVO.getIsAssociated()) {
      obsDT.setItDirty(true);
      String businessObjLookupName;
      ...
      String tableName = NEDSSConstant.OBSERVATION;
      String moduleCd = NEDSSConstant.BASE;
      Collection<ActRelationshipDto> actRelColl =
          actRelationshipService.loadActRelationshipBySrcIdAndTypeCode(
              reportSumVO.getObservationUid(), "LabReport");
      businessObjLookupName = NEDSSConstant.OBSERVATIONLABREPORT;
  ...
  public PageActProxyContainer getPageProxyVO(String typeCd, Long publicHealthCaseUID)
      throws DataProcessingException {
    ...
    thePublicHealthCaseContainer
        ...
        .setAddUserName(AuthUtil.authUser.getUserId());
    thePublicHealthCaseContainer
        .getThePublicHealthCaseDto()
        .setLastChgUserName(AuthUtil.authUser.getUserId());
    ...
    Iterator<ParticipationDto> participationIterator =
        thePublicHealthCaseContainer.getTheParticipationDTCollection().iterator();
    logger.debug(
    ...
    pageProxyVO.setTheNotificationSummaryVOCollection(
        retrieveSummaryService.notificationSummaryOnInvestigation(
            thePublicHealthCaseContainer, pageProxyVO));
    ...
    if (pageProxyVO.getTheNotificationSummaryVOCollection() != null) {
      for (Object o : pageProxyVO.getTheNotificationSummaryVOCollection()) {
        ...
        for (ActRelationshipDto actRelationDT :
            pageProxyVO.getPublicHealthCaseContainer().getTheActRelationshipDTCollection()) {
    ...
    if (typeCd != null && !typeCd.equals(NEDSSConstant.CASE_LITE)) {
      ...
      String labReportViewClause =
          queryHelper.getDataAccessWhereClause(NBSBOLookup.OBSERVATIONLABREPORT, "VIEW", "obs");
      labReportViewClause = labReportViewClause != null ? AND_UPPERCASE + labReportViewClause : "";

      Collection<UidSummaryContainer> labReportUidSummarVOs =
          observationSummaryService.findAllActiveLabReportUidListForManage(
      ...
      if (labReportUidSummarVOs != null && !labReportUidSummarVOs.isEmpty()) {
        ...
        if (typeCd.equalsIgnoreCase(NEDSSConstant.PRINT_CDC_CASE)) {
          ...
          if (labReportUidSummarVOs != null && !labReportUidSummarVOs.isEmpty()) {
            for (UidSummaryContainer uidSummaryVO : labReportUidSummarVOs) {
              uidSummaryVO.setStatusTime(
                  thePublicHealthCaseContainer.getThePublicHealthCaseDto().getAddTime());
            }
          }
        ...
        labSumVOMap =
            retrieveLabReportSummaryRevisited(labReportUidSummarVOs, isCDCFormPrintCase, uidType);
      ...
      pageProxyVO.setTheInvestigationAuditLogSummaryVOCollection(
          theInvestigationAuditLogSummaryVOCollection);
      ...
      Collection<Object> contactCollection =
          contactSummaryService.getContactListForInvestigation(publicHealthCaseUID);
  ...
  /**
   * This method Associates or disassociates the observation(LAb or MORB) to Investigation from the
   * manage observation page
   *
   * @param investigationUID -- The UID for the investigation to which observation is to be
   *     associated or disassociates
   */
  private void setObservationAssociationsImpl(
      Long investigationUID, Collection<LabReportSummaryContainer> reportSumVOCollection)
      throws DataProcessingException {
    ...
    try {
      ...
      if (!(vo instanceof LabResultProxyContainer)
        ...
        throw new DataProcessingException(
            "vo not instance of VaccinationProxyVO,LabResultProxyVO, or MorbidityProxyVO,PamProxyVO, SummaryReportProxyVO");
      }
  ...
  private void updateNotification(
    ...
    if (checkNotificationPermission1 && (notificationDT.getAutoResendInd().equalsIgnoreCase("T"))) {
  ...
  private InvestigationContainer getInvestigationProxy(Long publicHealthCaseUID)
      throws DataProcessingException {

    return getInvestigationProxyLite(publicHealthCaseUID, false);
  }

  @SuppressWarnings({"java:S3776", "java:S1141", "java:S135", "java:S6541"})
  private InvestigationContainer getInvestigationProxyLite(Long publicHealthCaseUID, boolean lite)
      throws DataProcessingException {
    ...
    ArrayList<ObservationContainer> theObservationVOCollection =
        new ArrayList<>(); // Observation (VO)
    ...
    try {
      ...
      if (patientGroupID != null) {
        var entityGrp =
            publicHealthCaseRepositoryUtil.getEntityGroup(
                thePublicHealthCaseDto.getPatientGroupId());
        if (entityGrp != null) {
      ...
      while (participationIterator.hasNext()) {
        ...
        if (strClassCd != null
            && strClassCd.compareToIgnoreCase(NEDSSConstant.NONPERSONLIVINGSUBJECT) == 0
            && recordStatusCd != null
            && recordStatusCd.equals(NEDSSConstant.RECORD_STATUS_ACTIVE)) {
          var nonpDto = publicHealthCaseRepositoryUtil.getNonPersonLivingSubject(nEntityID);
          if (nonpDto != null) {
            NonPersonLivingSubjectContainer nonPersonLivingSubjectContainer =
                new NonPersonLivingSubjectContainer();
      ...
      for (ActRelationshipDto actRelationshipDto :
          thePublicHealthCaseContainer.getTheActRelationshipDTCollection()) {
        ...
        if (strClassCd != null
            && strClassCd.compareToIgnoreCase(NEDSSConstant.CLINICAL_DOCUMENT_CLASS_CODE) == 0
            && recordStatusCd != null
            && recordStatusCd.equals(NEDSSConstant.RECORD_STATUS_ACTIVE)) {
        ...
        if (strClassCd != null
            && strClassCd.compareToIgnoreCase(NEDSSConstant.PATIENT_ENCOUNTER_CLASS_CODE) == 0
            && recordStatusCd != null
            && recordStatusCd.equals(NEDSSConstant.RECORD_STATUS_ACTIVE)) {
        ...
        if (strClassCd != null
            && strClassCd.compareToIgnoreCase(NEDSSConstant.OBSERVATION_CLASS_CODE) == 0
            && strTypeCd != null
            && strTypeCd.equals(NEDSSConstant.PHC_INV_FORM)) {
          ObservationContainer parentObservationVO =
              observationRepositoryUtil.loadObject(nSourceActID);
          theObservationVOCollection =
              (ArrayList<ObservationContainer>)
                  observationRepositoryUtil.retrieveObservationQuestion(nSourceActID);
      ...
      investigationProxyVO.setTheOrganizationVOCollection(theOrganizationVOCollection);
      ...
      investigationProxyVO.setTheEntityGroupVOCollection(theEntityGroupVOCollection);
      investigationProxyVO.setTheInterventionVOCollection(theInterventionVOCollection);
      investigationProxyVO.setTheClinicalDocumentVOCollection(theClinicalDocumentVOCollection);
      investigationProxyVO.setTheReferralVOCollection(theReferralVOCollection);
      investigationProxyVO.setThePatientEncounterVOCollection(thePatientEncounterVOCollection);
      investigationProxyVO.setTheObservationVOCollection(theObservationVOCollection);

      try {
        if (!lite) {
          theStateDefinedFieldDTCollection =
              new ArrayList<>(
                  ldfService.getLDFCollection(
                      publicHealthCaseUID, investigationProxyVO.getBusinessObjectName()));
        }
      } catch (Exception e) {
        logger.error(
      ...
      if (!lite) {
        String labReportViewClause =
            queryHelper.getDataAccessWhereClause(NBSBOLookup.OBSERVATIONLABREPORT, "VIEW", "obs");
        labReportViewClause =
            labReportViewClause != null ? AND_UPPERCASE + labReportViewClause : "";
      ...
      processingInvestigationSummary(investigationProxyVO, thePublicHealthCaseContainer, lite);

      if (!lite) {
        logger.debug("About to get TreatmentSummaryList for Investigation");
        theTreatmentSummaryVOCollection =
            new ArrayList<>(
                (retrieveSummaryService.retrieveTreatmentSummaryVOForInv(publicHealthCaseUID))
                    .values());
        logger.debug("Number of treatments found: {}", theTreatmentSummaryVOCollection.size());
        investigationProxyVO.setTheTreatmentSummaryVOCollection(theTreatmentSummaryVOCollection);
      }

      if (!lite) {
        theDocumentSummaryVOCollection =
            new ArrayList<>(
                retrieveSummaryService
                    .retrieveDocumentSummaryVOForInv(publicHealthCaseUID)
                    .values());
  ...
  private HashMap<Object, Object> retrieveLabReportSummaryRevisited(
      Collection<UidSummaryContainer> labReportUids, boolean isCDCFormPrintCase, String uidType)
      throws DataProcessingException {
  ...
  private HashMap<Object, Object> getObservationSummaryListForWorkupRevisited(
      Collection<UidSummaryContainer> uidList, boolean isCDCFormPrintCase, String uidType)
      throws DataProcessingException {
    ...
    if (uidList != null) {
      String dataAccessWhereClause =
          queryHelper.getDataAccessWhereClause(NBSBOLookup.OBSERVATIONLABREPORT, "VIEW", "");
      ...
      try {
        ...
        while (itLabId.hasNext()) {
          if (uidType.equalsIgnoreCase(PERSON_PARENT_UID_DB)) {
            ...
            var res =
                observationSummaryRepository.findLabSummaryForWorkupNew(uid, dataAccessWhereClause);
            if (res.isPresent()) {
              labList = res.get();
              count = count + 1;
            }
          ...
          if (labList != null) {
            for (Observation_Lab_Summary_ForWorkUp_New observationLabSummaryForWorkUpNew :
                labList) {
              LabReportSummaryContainer labRepVO =
                  new LabReportSummaryContainer(observationLabSummaryForWorkUpNew);
              labRepVO.setActivityFromTime(fromTime);
              LabReportSummaryContainer labRepSumm = null;
              LabReportSummaryContainer labRepEvent = null;
              Map<Object, Object> uidMap =
                  observationSummaryService.getLabParticipations(labRepVO.getObservationUid());
              if (uidMap != null && uidMap.containsKey(NEDSSConstant.PAR110_TYP_CD)) {
                if (labRepVO.getRecordStatusCd() != null
                    && (labRepVO.getRecordStatusCd().equals("UNPROCESSED"))) {
                  labRepSumm = labRepVO;
                  labRepSumm.setMPRUid((Long) uidMap.get(NEDSSConstant.PAR110_TYP_CD));
                }
                if (labRepVO.getRecordStatusCd() != null
                    && !labRepVO.getRecordStatusCd().equals("LOG_DEL")) {
                  labRepEvent = labRepVO;
                  labRepEvent.setMPRUid((Long) uidMap.get(NEDSSConstant.PAR110_TYP_CD));
                }
              }

              ArrayList<Object> providerDetails =
                  observationSummaryService.getProviderInfo(labRepVO.getObservationUid(), "ORD");
              ArrayList<Object> actIdDetails =
                  observationSummaryService.getActIdDetails(labRepVO.getObservationUid());
              Map<Object, Object> associationsMap =
                  observationSummaryService.getAssociatedInvList(
                      labRepVO.getObservationUid(), "OBS");
              if (labRepEvent != null) {
              ...
              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR111_TYP_CD)
                  && labRepEvent != null) {
                labRepEvent.setReportingFacility(
                    observationSummaryService.getReportingFacilityName(
                        (Long) uidMap.get(NEDSSConstant.PAR111_TYP_CD)));
              }
              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR111_TYP_CD)
                  && labRepSumm != null) {
                labRepSumm.setReportingFacility(
                    observationSummaryService.getReportingFacilityName(
                        (Long) uidMap.get(NEDSSConstant.PAR111_TYP_CD)));
              }

              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR101_TYP_CD)
                  && labRepEvent != null) {
                labRepEvent.setOrderingFacility(
                    observationSummaryService.getReportingFacilityName(
                        (Long) uidMap.get(NEDSSConstant.PAR101_TYP_CD)));
              }
              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR101_TYP_CD)
                  && labRepSumm != null) {
                labRepSumm.setOrderingFacility(
                    observationSummaryService.getReportingFacilityName(
                        (Long) uidMap.get(NEDSSConstant.PAR101_TYP_CD)));
              }

              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR104_TYP_CD)
                  && labRepEvent != null) {
                var code =
                    observationSummaryService.getSpecimanSource(
                        (Long) uidMap.get(NEDSSConstant.PAR104_TYP_CD));
                labRepEvent.setSpecimenSource(
                    catchingValueService.getCodedValue("SPECMN_SRC", code));
              }
              if (uidMap != null
                  && uidMap.containsKey(NEDSSConstant.PAR104_TYP_CD)
                  && labRepSumm != null) {
                var code =
                    observationSummaryService.getSpecimanSource(
                        (Long) uidMap.get(NEDSSConstant.PAR104_TYP_CD));
                labRepSumm.setSpecimenSource(
                    catchingValueService.getCodedValue("SPECMN_SRC", code));
              }

              providerUid =
                  observationSummaryService.getProviderInformation(providerDetails, labRepEvent);

              if (isCDCFormPrintCase
                  && providerUid != null
                  && labAsSourceForInvestigation != null) {
                ...
                if (orderingFacilityUid != null) {
                  ...
                  observationSummaryService.getOrderingFacilityAddress(
                      providerDataForPrintVO, orderingFacilityUid);
                  observationSummaryService.getOrderingFacilityPhone(
                      providerDataForPrintVO, orderingFacilityUid);
                }
                if (providerUid != null) {
                  observationSummaryService.getOrderingPersonAddress(
                      providerDataForPrintVO, providerUid);
                  observationSummaryService.getOrderingPersonPhone(
                      providerDataForPrintVO, providerUid);
                }
              }
              ...
              if (labRepEvent != null) labEventList.add(labRepEvent);
              if (labRepSumm != null) labSummList.add(labRepSumm);

              Long observationUID = labRepVO.getObservationUid();
              observationSummaryService.getTestAndSusceptibilities(
                  "COMP", observationUID, labRepEvent, labRepSumm);
            }
          }
        }
      } catch (Exception ex) {
  ...
  @SuppressWarnings("java:S3776")
  protected void processingInvestigationSummary(
      InvestigationContainer investigationProxyVO,
      PublicHealthCaseContainer thePublicHealthCaseContainer,
      boolean lite)
      throws DataProcessingException {
    if (!lite) {
      investigationProxyVO.setTheNotificationSummaryVOCollection(
          retrieveSummaryService.notificationSummaryOnInvestigation(
              thePublicHealthCaseContainer, investigationProxyVO));
        ...
        for (Object o : investigationProxyVO.getTheNotificationSummaryVOCollection()) {
          ...
          for (ActRelationshipDto actRelationDT :
              investigationProxyVO
                  .getThePublicHealthCaseContainer()
                  .getTheActRelationshipDTCollection()) {
            if ((notifVO.getCdNotif().equalsIgnoreCase(NEDSSConstant.CLASS_CD_SHARE_NOTF)
                    || notifVO
                        .getCdNotif()
                        .equalsIgnoreCase(NEDSSConstant.CLASS_CD_SHARE_NOTF_PHDC))
                && notifVO.getNotificationUid().compareTo(actRelationDT.getSourceActUid()) == 0) {
              actRelationDT.setShareInd(true);
            }
            if ((notifVO.getCdNotif().equalsIgnoreCase(NEDSSConstant.CLASS_CD_EXP_NOTF)
  ...
  protected void populateDescTxtFromCachedValues(Collection<Object> reportSummaryVOCollection)
      throws DataProcessingException {
    ...
    for (Object o : reportSummaryVOCollection) {
      ...
      if (sumVO instanceof LabReportSummaryContainer) {
        ...
        if (labVO.getProgramArea() != null) {
          tempStr =
              cacheApiService.getSrteCacheString(
                  ObjectName.PROGRAM_AREA_CODES.name(), labVO.getProgramArea());
          labVO.setProgramArea(tempStr);
        }
        if (labVO.getJurisdiction() != null) {
          tempStr =
              cacheApiService.getSrteCacheString(
                  ObjectName.JURISDICTION_CODES.name(), labVO.getJurisdiction());
          if (!tempStr.isEmpty()) labVO.setJurisdiction(tempStr);
        }
        if (labVO.getStatus() != null) {
          tempStr = catchingValueService.getCodeDescTxtForCd("ACT_OBJ_ST", labVO.getStatus());
          if (tempStr != null) labVO.setStatus(tempStr);
        }
          ...
          while (resItor.hasNext()) {
            ...
            if (resVO.getCtrlCdUserDefined1() != null) {
              if (resVO.getCtrlCdUserDefined1().equals("N")
                ...
                tempStr =
                    cacheApiService.getSrteCacheString(
                        ObjectName.LAB_RESULT_DESC.name(), resVO.getCodedResultValue());
                resVO.setCodedResultValue(tempStr);
              } else if (resVO.getCtrlCdUserDefined1().equals("Y")
                  && resVO.getOrganismName() != null
                  && resVO.getOrganismCodeSystemCd() != null) {
                if (resVO.getOrganismCodeSystemCd().equals("SNM")) {
                  tempStr =
                      cacheApiService.getSrteCacheString(
                          ObjectName.SNOMED_CODE_BY_DESC.name(), resVO.getCodedResultValue());
                  resVO.setOrganismName(tempStr);
                } else {
                  tempStr =
                      cacheApiService.getSrteCacheString(
                          ObjectName.LAB_RESULT_DESC_WITH_ORGANISM_NAME.name(),
                          resVO.getCodedResultValue());
                  resVO.setOrganismName(tempStr);
                }
              }
            } else {
            ...
            if ((resVO.getCdSystemCd() != null) && (!(resVO.getCdSystemCd().isEmpty()))) {
              if (resVO.getCdSystemCd().equals("LN")) {
                if (resVO.getResultedTestCd() != null && !resVO.getResultedTestCd().isEmpty()) {
                  tempStr =
                      cacheApiService.getSrteCacheString(
                          ObjectName.LOIN_CODE_WITH_COMPONENT_NAME.name(),
                          resVO.getResultedTestCd());
                  if (tempStr != null && !tempStr.isEmpty()) resVO.setResultedTest(tempStr);
                }
              } else if (!resVO.getCdSystemCd().equals("LN")) {
                if (resVO.getResultedTestCd() != null && !resVO.getResultedTestCd().isEmpty()) {
                  var res =
                      labTestRepository.findLabTestByLabIdAndLabTestCode(
                          resVO.getCdSystemCd(), resVO.getResultedTestCd());
                  if (res.isPresent()) {
            ...
            if ((resVO.getResultedTestStatusCd() != null)
                && (!(resVO.getResultedTestStatusCd().isEmpty()))) {
              tempStr =
                  catchingValueService.getCodeDescTxtForCd(
                      "ACT_OBJ_ST", resVO.getResultedTestStatusCd());
              if (tempStr != null && !tempStr.isEmpty()) resVO.setResultedTestStatus(tempStr);
            }
            // End  ER16368
            ...
            if (susColl != null && !susColl.isEmpty()) {
              ...
              while (susItor.hasNext()) {
                ...
                if (susVO.getCodedResultValue() != null && !susVO.getCodedResultValue().isEmpty()) {
                  tempStr =
                      cacheApiService.getSrteCacheString(
                          ObjectName.LAB_RESULT_DESC.name(), susVO.getCodedResultValue());
                  if (tempStr != null && !tempStr.isEmpty()) susVO.setCodedResultValue(tempStr);
                }
                if (susVO.getCdSystemCd() != null && !susVO.getCdSystemCd().isEmpty()) {

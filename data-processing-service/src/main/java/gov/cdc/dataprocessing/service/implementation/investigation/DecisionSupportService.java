...
public class DecisionSupportService implements IDecisionSupportService {
  ...
  final Comparator<PublicHealthCaseDto> ADDTIME_ORDER =
      (e1, e2) -> e2.getAddTime().compareTo(e1.getAddTime()); // NOSONAR
  final Comparator<DsmLabMatchHelper> AlGORITHM_NM_ORDER =
      (e1, e2) -> e1.getAlgorithmNm().compareToIgnoreCase(e2.getAlgorithmNm()); // NOSONAR

  @SuppressWarnings({"java:S3776", "java:S135"})
  // Was: validateProxyVO
  public EdxLabInformationDto validateProxyContainer(
    ...
    ObservationContainer orderedTestObservationVO =
        setupObservationValuesForWds(
            edxLabInformationDT, labResultProxyVO, resultedTestColl, resultedTestCodeColl);
    ...
    for (DsmLabMatchHelper dsmLabMatchHelper : activeElrAlgorithmList) {
      ...
      WdsReport wdsReport =
          dsmLabMatchHelper.isThisLabAMatch(
    ...
    for (ObservationContainer obsVO : labResultProxyVO.getTheObservationContainerCollection()) {
      ...
      if (obsDomainCdSt1 != null && obsDomainCdSt1.equalsIgnoreCase(EdxELRConstant.ELR_RESULT_CD)) {
        ...
        orderedTestObservationVO
            ...
            .setObservationUid(edxLabInformationDT.getRootObserbationUid());
        orderedTestObservationVO.setTheParticipationDtoCollection(
            labResultProxyVO.getTheParticipationDtoCollection());
      }
    }
  ...
  /**
   * Description: this return true, if Action is Review, Investigation, and Investigation with
   * Notification
   */
  ...
  private boolean checkActionNotMarkedAsReviewed(Algorithm algorithmDocument) {
    return (algorithmDocument.getAction() != null
  ...
  private boolean checkActionNotMarkedAsReviewedAndAdvCriteriaApplied(
      Algorithm algorithmDocument, boolean applyAdvInvLogic) {
    return (checkActionNotMarkedAsReviewed(algorithmDocument)) && applyAdvInvLogic;
  }

  @SuppressWarnings({"java:S3776", "java:S135"})
  protected boolean checkActiveWdsAlgorithm(
      EdxLabInformationDto edxLabInformationDT, List<DsmLabMatchHelper> activeElrAlgorithmList) {
    ...
    if (!found) {
      edxLabInformationDT
          .getWdsReports()
          .add(new WdsReport(false, "No active WDS Algorithm found"));
      return false;
    }
  ...
  protected void updateObservationBasedOnAction(
    ...
    if (isActionValid) {
      ...
      EventDateLogicType eventDateLogicType =
          algorithmDocument.getElrAdvancedCriteria().getEventDateLogic();
      ...
      if (applyAdvInvLogic && eventDateLogicType != null) {
        ...
        isdateLogicValidForNewInv =
            specimenCollectionDateCriteria(eventDateLogicType, edxLabInformationDT);
      } else {
        isdateLogicValidForNewInv = true;
      }

      boolean isAdvancedInvCriteriaValid = false;
      boolean reviewActionApplied =
          algorithmDocument.getAction() != null
              && algorithmDocument.getAction().getMarkAsReviewed() != null
              && algorithmDocument
                  .getAction()
      ...
      if (reviewActionApplied && applyAdvInvLogic) {
        ...
        isAdvancedInvCriteriaValid =
            checkAdvancedInvCriteria(algorithmDocument, edxLabInformationDT, questionIdentifierMap);
      } else if (checkActionNotMarkedAsReviewedAndAdvCriteriaApplied(
          algorithmDocument, applyAdvInvLogic)) {
        isAdvancedInvCriteriaValid =
            checkAdvancedInvCriteriaForCreateInvNoti(
                algorithmDocument, edxLabInformationDT, questionIdentifierMap);
      }

      if (reviewActionApplied
          && (!applyAdvInvLogic
        ...
        if (conditionCode != null) {
          ...
          ConditionCode condCode =
              (ConditionCode)
                  cacheApiService.getSrteCacheObject(
                      ObjectName.CONDITION_CODE.name(), conditionCode);
          if (condCode == null) {
      ...
      } else if ((checkActionNotMarkedAsReviewed(algorithmDocument))
          && (!applyAdvInvLogic
        ...
        } else if (algorithmDocument.getAction() != null
            && algorithmDocument.getAction().getCreateInvestigationWithNND() != null) {
          wdsReport.setAction("CREATE_INVESTIGATION_WITH_NOTIFICATION");
          edxLabInformationDT.setAction(
              DecisionSupportConstants.CREATE_INVESTIGATION_WITH_NND_VALUE);
        }

        edxLabInformationDT.setInvestigationType(algorithmDocument.getInvestigationType());
        // AUTO INVESTIGATION
        Object obj =
            autoInvestigationService.autoCreateInvestigation(
                orderedTestObservationVO, edxLabInformationDT);
        ...
        if (applyMap != null && !applyMap.isEmpty() && questionIdentifierMap != null) {
          ...
          for (Object o : set) {
            ...
            NbsQuestionMetadata metaData =
                (NbsQuestionMetadata) questionIdentifierMap.get(questionId);
            if (metaData == null) {
              metaData = new NbsQuestionMetadata();
            } else {
              logger.debug(
                  "DecisionSupportService.updateObservationBasedOnAction: metaData is Null");
            }
            if (metaData.getDataLocation() != null
              ...
              validateDecisionSupport.processNbsObject(
              ...
              validateDecisionSupport.processNBSCaseAnswerDT(
                  edxRuleManageDT, publicHealthCaseContainer, pamVO, metaData);
            } else if (metaData.getDataLocation() != null
                && metaData
                    .getDataLocation()
                    .trim()
                    .toUpperCase()
                    .startsWith("CONFIRMATION_METHOD.CONFIRMATION_METHOD_CD")) {
              validateDecisionSupport.processConfirmationMethodCodeDT(
                ...
                && metaData
                    ...
                    .trim()
                    .toUpperCase()
                    .startsWith("CONFIRMATION_METHOD.CONFIRMATION_METHOD_TIME")) {
              validateDecisionSupport.processConfirmationMethodTimeDT(
                  edxRuleManageDT, publicHealthCaseContainer, metaData);
            } else if (metaData.getDataLocation() != null
                && metaData
                    .getDataLocation()
                    .trim()
                    .toUpperCase()
                    .startsWith("ACT_ID.ROOT_EXTENSION_TXT")) {
              validateDecisionSupport.processActIds(
                  edxRuleManageDT, publicHealthCaseContainer, metaData);
            } else if (metaData.getDataLocation() != null
              ...
              if (edxRuleManageDT.getParticipationTypeCode() == null
                ...
                throw new DataProcessingException(
                    "ValidateDecisionSupport.validateProxyVO Exception thrown for edxRuleManageDT:-"
                        + edxRuleManageDT);
              }
            }
          }
          validateDecisionSupport.processConfirmationMethodCodeDTRequired(
        ...
        if (questionIdentifierMap != null && questionIdentifierMap.get(REQUIRED) != null) {
          Map<Object, Object> nbsAnswerMap = pamVO.getPamAnswerDTMap();
          Map<Object, Object> requireMap =
              (Map<Object, Object>) questionIdentifierMap.get(REQUIRED);
        ...
        ConditionCode condCode =
            (ConditionCode)
                cacheApiService.getSrteCacheObject(ObjectName.CONDITION_CODE.name(), conditionCode);
        if (condCode == null) {
          condCode = new ConditionCode();
        }
        edxLabInformationDT.setConditionName(condCode.getConditionShortNm());
  ...
  private static final ThreadLocal<Unmarshaller> ALGORITHM_UNMARSHALLER =
  ...
  static {
    try {
      ALGORITHM_JAXB_CONTEXT =
          JAXBContext.newInstance("gov.cdc.dataprocessing.model.dsma_algorithm");
  ...
  private Algorithm parseAlgorithmXml(String xmlPayloadContent) throws DataProcessingException {
    try (InputStream inputStream =
        new ByteArrayInputStream(xmlPayloadContent.getBytes(StandardCharsets.UTF_8))) {
  ...
  protected boolean specimenCollectionDateCriteria(
      EventDateLogicType eventDateLogicType, EdxLabInformationDto edxLabInformationDT)
      throws DataProcessingException {
    boolean isdateLogicValidForNewInv;
    ...
    // see if the selection is no for time period, check for all any investigation with the desired
    // condition code
    if (eventDateLogicType.getElrTimeLogic() != null
        ...
        && eventDateLogicType
            ...
            .getElrTimeLogicInd()
            .getCode()
            .equals(NEDSSConstant.NO)) {
      var assocExistPhcWithPid =
          publicHealthCaseStoredProcRepository.associatedPublicHealthCaseForMprForCondCd(
              mprUid, edxLabInformationDT.getConditionCode());
      edxLabInformationDT.setMatchingPublicHealthCaseDtoColl(assocExistPhcWithPid);
    }
    // see if the selection is yes for time period, check for all any investigation with the desired
    // condition code
      ...
      Timestamp specimenCollectionDate =
          edxLabInformationDT
      ...
      if (!comparatorCode.isEmpty() && mprUid > 0) {
        Collection<PublicHealthCaseDto> associatedPhcDTCollection =
            publicHealthCaseStoredProcRepository.associatedPublicHealthCaseForMprForCondCd(
                mprUid, edxLabInformationDT.getConditionCode());
        ...
        if (associatedPhcDTCollection != null && !associatedPhcDTCollection.isEmpty()) {
          for (PublicHealthCaseDto publicHealthCaseDto : associatedPhcDTCollection) {
            ...
            if (publicHealthCaseDto.getAssociatedSpecimenCollDate() != null) {
              dateCompare =
                  publicHealthCaseDto.getAssociatedSpecimenCollDate().getTime()
                      / (1000 * 60 * 60 * 24);
            } else {
            ...
            if (publicHealthCaseDto.getAssociatedSpecimenCollDate() != null) {
              isdateLogicValidWithThisInv =
                  specimenDateTimeCheck(
    ...
    if (edxLabInformationDT.getMatchingPublicHealthCaseDtoColl() != null
      ...
      List<PublicHealthCaseDto> phclist =
          new ArrayList<>(edxLabInformationDT.getMatchingPublicHealthCaseDtoColl());
  ...
  protected boolean specimenDateTimeCheck(
      String comparatorCode, int daysDifference, int value, boolean isdateLogicValidWithThisInv) {
    if (comparatorCode.contains(NEDSSConstant.LESS_THAN_LOGIC) && daysDifference > value) {
      isdateLogicValidWithThisInv = false;
    } else if (comparatorCode.contains(NEDSSConstant.GREATER_THAN_LOGIC)
  ...
  protected boolean checkAdvancedInvCriteria(
      Algorithm algorithmDocument,
      EdxLabInformationDto edxLabInformationDT,
      Map<Object, Object> questionIdentifierMap) {
    boolean isAdvancedInvCriteriaMet = false;

    Map<String, Object> advanceInvCriteriaMap =
        advancedCriteria.getAdvancedInvCriteriaMap(algorithmDocument);
    /*
        ...
        if (advanceInvCriteriaMap != null && !advanceInvCriteriaMap.isEmpty()) {
          ...
          for (String questionId : criteriaSet) {
            ...
            if (object instanceof EdxRuleManageDto edxRuleManageDT) {
              NbsQuestionMetadata criteriaMetaData =
                  (NbsQuestionMetadata) questionIdentifierMap.get(questionId);
              if (criteriaMetaData != null) {
                isAdvancedInvCriteriaMet =
                    wdsObjectChecker.checkNbsObject(edxRuleManageDT, phcDT, criteriaMetaData);
              }
  ...
  private void processAction(EdxLabInformationDto edxRuleAlgorothmManagerDT, Algorithm algorithm) {
    ...
    if (actionType.getCreateInvestigation() != null) {
      ...
      InvestigationDefaultValuesType investigationDefaultValuesType =
          specificActionType.getInvestigationDefaultValues();
      if (investigationDefaultValuesType != null) {
        validateDecisionSupport.parseInvestigationDefaultValuesType(
            applicationMap, investigationDefaultValuesType);
      }
      ...
      CreateInvestigationWithNNDType specificActionType =
          actionType.getCreateInvestigationWithNND();
      InvestigationDefaultValuesType investigationDefaultValuesType =
          specificActionType.getInvestigationDefaultValues();
      edxRuleAlgorothmManagerDT.setAction(
          DecisionSupportConstants.CREATE_INVESTIGATION_WITH_NND_VALUE);
      if (investigationDefaultValuesType != null) {
        validateDecisionSupport.parseInvestigationDefaultValuesType(
            applicationMap, investigationDefaultValuesType);
      ...
      if (specificActionType.getOnFailureToCreateNND() != null) {
        edxRuleAlgorothmManagerDT.setOnFailureToCreateNND(

...
public class PamService implements IPamService {
  ...
  public Long setPamProxyWithAutoAssoc(
  ...
  private Long setPamProxy(PamProxyContainer pamProxyVO) throws DataProcessingException {
    ...
    PublicHealthCaseDto phcDT =
        pamProxyVO.getPublicHealthCaseContainer().getThePublicHealthCaseDto();
    ...
    if (pamProxyVO.isItDirty()) {
      try {
        ...
        String phcLocalId =
            pamProxyVO.getPublicHealthCaseContainer().getThePublicHealthCaseDto().getLocalId();
    ...
    if (pamProxyVO.getThePersonVOCollection() != null) {
      for (anIteratorPerson = pamProxyVO.getThePersonVOCollection().iterator();
          anIteratorPerson.hasNext(); ) {
        personVO = anIteratorPerson.next();
        if (personVO.isItNew()) {
          if (personVO.getThePersonDto().getCd().equals(NEDSSConstant.PAT)) { // Patient
    ...
    if (pamProxyVO.getPublicHealthCaseContainer() != null) {
      ...
      PublicHealthCaseContainer publicHealthCaseContainer =
          pamProxyVO.getPublicHealthCaseContainer();
    ...
    if (pamProxyVO.isUnsavedNote()
        && pamProxyVO.getNbsNoteDTColl() != null
        && !pamProxyVO.getNbsNoteDTColl().isEmpty()) {
    ...
    Long docUid = null;
    if (pamProxyVO.getPublicHealthCaseContainer() != null
        && pamProxyVO.getPublicHealthCaseContainer().getTheActRelationshipDTCollection() != null) {
      ...
      for (anIteratorAct =
              pamProxyVO
                  ...
                  .iterator();
          anIteratorAct.hasNext(); ) {
        ActRelationshipDto actRelationshipDT = anIteratorAct.next();
    ...
    if (pamProxyVO.getTheParticipationDTCollection() != null) {
      for (anIteratorPat = pamProxyVO.getTheParticipationDTCollection().iterator();
          anIteratorPat.hasNext(); ) {
        ParticipationDto participationDT = anIteratorPat.next();

...
public class ManagerService implements IManagerService {
  ...
  public PublicHealthCaseFlowContainer processingELR(Integer data, boolean retryApplied)
    ...
    try {
      ...
      LabResultProxyContainer labResultProxyContainer =
          dataExtractionService.parsingDataToObject(nbsInterfaceModel, edxLabInformationDto);
      edxLabInformationDto.setLabResultProxyContainer(labResultProxyContainer);

      if (nbsInterfaceModel.getObservationUid() != null
          && nbsInterfaceModel.getObservationUid() > 0) {
      ...
      edxLabInformationDto =
          managerAggregationService.processingObservationMatching(
              edxLabInformationDto, labResultProxyContainer, null);
      labResultProxyContainer.setMatchedObservationFound(
          edxLabInformationDto.isMatchedObservationFound());
      ...
      ObservationDto observationDto =
          observationService.processingLabResultContainer(labResultProxyContainer);
      edxLabInformationDto.setLabResultProxyContainer(labResultProxyContainer);
      edxLabInformationDto.setLocalId(observationDto.getLocalId());
      edxLabInformationDto
          .getEdxActivityLogDto()
          .setBusinessObjLocalId(observationDto.getLocalId());
  ...
  protected void finalizeProcessingElr(
    ...
    if (dltLockError) {
      persistingRtiDlt(
          ...
          EMPTY,
          STEP_1,
          DP_FAILURE_STEP_1 + DASH + ERROR_DB_LOCKING);
      if (!retryApplied) {
        composeDltKafkaEvent(String.valueOf(interfaceId), ERROR_DB_LOCKING);
      }
    } else if (nonDltError) {
      persistingRtiDlt(
          exception, (long) interfaceId, EMPTY, STEP_1, DP_FAILURE_STEP_1 + DASH + "Other Error");
      ...
      persistingRtiDlt(
          ...
          EMPTY,
          STEP_1,
          DP_FAILURE_STEP_1 + DASH + ERROR_DB_DATA_INTEGERITY);
      if (!retryApplied) {
  ...
  protected void enrichProgramAreaAndJurisdiction(
      ObservationDto observationDto, EdxLabInformationDto edxLabInformationDto)
      throws DataProcessingException {
    if (observationDto.getProgAreaCd() != null
        && cacheApiService.getSrteCacheBool(
            ObjectName.PROGRAM_AREA_CODES.name(), observationDto.getProgAreaCd())) {
    ...
    if (observationDto.getJurisdictionCd() != null
        && cacheApiService.getSrteCacheBool(
            ObjectName.JURISDICTION_CODES.name(), observationDto.getJurisdictionCd())) {
  ...
  public void handlingWdsAndLab(PublicHealthCaseFlowContainer phcContainer, boolean retryApplied)
    ...
    try {
      ...
      finalizeWdsAndLabProcessing(
          phcContainer, exception, dltLockError, nonDltError, integrityError, retryApplied);
    }
  }
  ...
  protected void finalizeWdsAndLabProcessing(
    ...
    if (dltLockError) {
      persistingRtiDlt(
          exception,
          Long.valueOf(phcContainer.getNbsInterfaceId()),
          payload,
          STEP_2,
          DP_FAILURE_STEP_2 + DASH + ERROR_DB_LOCKING);
      if (!retryApplied) {
      ...
      persistingRtiDlt(
          ...
          EMPTY,
          STEP_2,
          DP_FAILURE_STEP_2 + DASH + ERROR_DB_DATA_INTEGERITY);
      if (!retryApplied) {
      ...
      if (nonDltError) {
        persistingRtiDlt(
            ...
            payload,
            STEP_2,
            DP_FAILURE_STEP_2 + " - Other Non Error");
      }
      edxLogService.updateActivityLogDT(
          phcContainer.getNbsInterfaceModel(), phcContainer.getEdxLabInformationDto());
      edxLogService.addActivityDetailLogs(phcContainer.getEdxLabInformationDto(), EMPTY);
      edxLogService.saveEdxActivityLogs(
          phcContainer.getEdxLabInformationDto().getEdxActivityLogDto());
    }
  }
  ...
  protected void persistingRtiDlt(
      Exception exception, Long nbsInterfaceUid, String payload, String step, String status) {
  ...
  @SuppressWarnings({"java:S6541", "java:S3776"})
  protected PublicHealthCaseFlowContainer initiatingInvestigationAndPublicHealthCase(
      PublicHealthCaseFlowContainer phcContainer) throws DataProcessingException {
    ...
    if (edxDto.getPageActContainer() != null) {
      tracker.setPublicHealthCase(
          edxDto.getPageActContainer().getPublicHealthCaseContainer().getThePublicHealthCaseDto());
    } else if (edxDto.getPamContainer() != null) {
      tracker.setPublicHealthCase(
          edxDto.getPamContainer().getPublicHealthCaseContainer().getThePublicHealthCaseDto());
    }
  ...
  @SuppressWarnings({"java:S6541", "java:S3776"})
  protected void initiatingLabProcessing(PublicHealthCaseFlowContainer phcContainer)
      throws DataProcessingException {
    NbsInterfaceModel interfaceModel = phcContainer.getNbsInterfaceModel();
    EdxLabInformationDto edxDto = phcContainer.getEdxLabInformationDto();
    ...
    if (DecisionSupportConstants.MARK_AS_REVIEWED.equalsIgnoreCase(action)) {
      ...
      if (pageAct != null && obsDto.getJurisdictionCd() != null && obsDto.getProgAreaCd() != null) {
        ...
        pageAct
            ...
            .setPublicHealthCaseUid(phcUid);
      } else if (pamProxy != null
          && obsDto.getJurisdictionCd() != null
          && obsDto.getProgAreaCd() != null) {
  ...
  protected String handleProcessingELRError(
      Exception e, EdxLabInformationDto dto, NbsInterfaceModel model) {
    logException(e);
  ...
  private void processFieldErrors(EdxLabInformationDto dto) {
    if (dto.isReflexResultedTestCdMissing()
        || dto.isResultedTestNameMissing()
        || dto.isOrderTestNameMissing()
        || dto.isReasonforStudyCdMissing()) {
  ...
  private String buildDetailedErrorMessage(
      Exception e, EdxLabInformationDto dto, String accessionNumber) {
    ...
    if (msg.contains(EdxELRConstant.SQL_FIELD_TRUNCATION_ERROR_MSG)
        || msg.contains(EdxELRConstant.ORACLE_FIELD_TRUNCATION_ERROR_MSG)) {
      ...
      return truncateMessage(
          String.format(
              "SQLException while inserting into %s %s %s", tableName, accessionNumber, trace));
    }
  ...
  private void requiredFieldError(String errorTxt, EdxLabInformationDto edxLabInformationDT)
      throws DataProcessingException {
    if (errorTxt != null) {
      edxLabInformationDT.setErrorText(EdxELRConstant.ELR_MASTER_LOG_ID_5);
      if (edxLabInformationDT.getEdxActivityLogDto().getEDXActivityLogDTWithVocabDetails()
          == null) {
        edxLabInformationDT
            .getEdxActivityLogDto()
            .setEDXActivityLogDTWithVocabDetails(new ArrayList<>());
      }

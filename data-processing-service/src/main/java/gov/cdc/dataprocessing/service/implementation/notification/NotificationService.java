...
public class NotificationService implements INotificationService {
  ...
  public boolean checkForExistingNotification(BaseContainer vo) throws DataProcessingException {
    ...
    !(vo instanceof LabResultProxyContainer)) {
      throw new DataProcessingException(
          "NND MessageSenderHelper: VO not of type Vacc,Morb,LabReport");
    }
    long count =
        notificationRepository.getCountOfExistingNotifications(getRootUid(vo), getActClassCd(vo));
    ...
    if (vo instanceof LabResultProxyContainer labResultProxyContainer) {
      ...
      for (ObservationContainer observationContainer : obsColl) {
        ...
        if (obsDomainCdSt1 != null
            ...
            && obsCtrlCdDisplayForm.equalsIgnoreCase(NEDSSConstant.LAB_REPORT)) {
          return observationContainer.getTheObservationDto().getObservationUid();
        }
      }
    }
    return null;
  ...
  public Long setNotificationProxy(NotificationProxyContainer notificationProxyVO)
    ...
    notifDT.setCaseConditionCd(
        notificationProxyVO.getThePublicHealthCaseContainer().getThePublicHealthCaseDto().getCd());
      ...
      notifDT =
          (NotificationDto)
              prepareAssocModelHelper.prepareVO(
                  notifDT, boLookup, triggerCd, tableName, moduleCd, notifDT.getVersionCtrlNbr());

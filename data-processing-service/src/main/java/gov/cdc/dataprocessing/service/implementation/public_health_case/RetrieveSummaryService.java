...
public class RetrieveSummaryService implements IRetrieveSummaryService {
  ...
  /**
   * This method will access the HashMap<Object,Object> of TreatmentSummaryVO for passed
   * investigationUID to papulate the Treatment summary on Investigation page
   *
   * @param publicHealthUID -- UID for investigation to Access Treatment related to it
   * @return HashMap<Object,Object> -- HashMap<Object,Object> of TreatmentSummaryVO for the passed
   *     investigationUID
   */
  ...
  public Collection<Object> notificationSummaryOnInvestigation(
      PublicHealthCaseContainer publicHealthCaseContainer, Object object)
    ...
    if (publicHealthCaseContainer != null
        && publicHealthCaseContainer.getThePublicHealthCaseDto().getCaseClassCd() != null) {
      theNotificationSummaryVOCollection =
    ...
    if (theNotificationSummaryVOCollection != null) {
      ...
      while (anIterator.hasNext()) {
        ...
        if (count == 0) { // check only for the current Notification record
          if (notificationSummaryVO
                  .getRecordStatusCd()
                  .trim()
                  .equals(NEDSSConstant.NOTIFICATION_APPROVED_CODE)
              || notificationSummaryVO
                  .getRecordStatusCd()
                  .trim()
                  .equals(NEDSSConstant.NOTIFICATION_PENDING_CODE)
              || (notificationSummaryVO.getAutoResendInd() != null
                  && notificationSummaryVO.getAutoResendInd().equalsIgnoreCase("T"))) {
        ...
        if (notificationSummaryVO.getRecordStatusCd() != null
            && notificationSummaryVO
                .getRecordStatusCd()
                .trim()
                .equals(NEDSSConstant.PENDING_APPROVAL_STATUS)) {
          notificationSummaryVO.setCd(
              publicHealthCaseContainer.getThePublicHealthCaseDto().getCd());
          notificationSummaryVO.setCdTxt(
              publicHealthCaseContainer.getThePublicHealthCaseDto().getCdDescTxt());
        }
      }
    }

    theNotificationSummaryVOCollection =
        notificationSummaryOnInvestigationProcessingNotificationCol(
            theNotificationSummaryVOCollection, notificationSummaryVO, object);
  ...
  protected Collection<Object> notificationSummaryOnInvestigationProcessingNotificationCol(
    ...
    if (theNotificationSummaryVOCollection != null) {
      for (Object o : theNotificationSummaryVOCollection) {
          ...
          if (notificationSummaryVO.isCaseReport()) {
            ...
            if (notificationSummaryVO.isHistory != null
                ...
                && (notificationSummaryVO.getCdNotif().equals(NEDSSConstant.CLASS_CD_EXP_NOTF)
                    || notificationSummaryVO
                        .getCdNotif()
                        .equals(NEDSSConstant.CLASS_CD_EXP_NOTF_PHDC))
                && !(notificationSummaryVO
                        .getRecordStatusCd()
                        .trim()
                        .equals(NEDSSConstant.NOTIFICATION_REJECTED_CODE)
                    || notificationSummaryVO
                        .getRecordStatusCd()
                        .trim()
                        .equals(NEDSSConstant.NOTIFICATION_MESSAGE_FAILED))) {
        ...
        } else if (object instanceof PageActProxyContainer pageProxy
            && notificationSummaryVO.isCaseReport()) // NOSONAR
        {
          if (notificationSummaryVO
              .getRecordStatusCd()
              .trim()
   ...
   *
   * @return Collection<Object> -- Collection<Object> of NotificationSummaryVO for the passed
   *     publicHealthCaseDT
   */
  @SuppressWarnings({"java:S3776", "java:S1197"})
  protected Collection<Object> retrieveNotificationSummaryListForInvestigation(Long publicHealthUID)
      throws DataProcessingException {
    ...
    if (publicHealthUID != null) {
      ...
      statement[1] =
          SELECT_NOTIFICATION_HIST_FOR_INVESTIGATION_SQL
              + " ORDER BY notHist.version_ctrl_nbr DESC";
      for (String s : statement) {
        List<NotificationSummaryContainer> retval;

        retval =
            customRepository.retrieveNotificationSummaryListForInvestigation(publicHealthUID, s);
        if (retval.isEmpty()) {
          break;
        }
        for (NotificationSummaryContainer newVO : retval) {
          if (newVO.getCaseClassCd() != null && !newVO.getCaseClassCd().trim().isEmpty()) {
            catchingValueService.getCodedValuesCallRepos(CASE_CLASS_CODE_SET_NM);
            newVO.setCaseClassCdTxt(
                catchingValueService.getCodedValuesCallRepos(newVO.getCaseClassCd()));
          }
          if (newVO.getCd() != null && !newVO.getCd().trim().isEmpty()) {
            catchingValueService.getCodedValuesCallRepos(CASE_CLASS_CODE_SET_NM);
            newVO.setCdTxt(catchingValueService.getCodedValuesCallRepos(newVO.getCd()));
          }

          if (newVO.getCdNotif() != null && !newVO.getCdNotif().trim().isEmpty()) {
            newVO.setCdNotif(newVO.getCdNotif());
            newVO.setNotificationSrtDescCd(
                catchingValueService.getCodeDescTxtForCd(newVO.getCdNotif(), "NBS_DOC_PURPOSE"));
          }
          if (newVO.getRecipient() != null && !newVO.getRecipient().trim().isEmpty()) {
            ...
            else newVO.setRecipient(NEDSSConstant.LOCAl_DESC);
          }
  ...
  protected Collection<Object> retrieveNotificationSummaryListForInvestigation1(
      Long publicHealthUID) throws DataProcessingException {
    ArrayList<Object> theNotificationSummaryVOCollection = new ArrayList<>();
    if (publicHealthUID != null) {
      String dataAccessWhereClause =
          queryHelper.getDataAccessWhereClause(NBSBOLookup.INVESTIGATION, "VIEW", "Notification");
      ...
      statement[0] = SELECT_NOTIFICATION_FOR_INVESTIGATION_SQL1 + dataAccessWhereClause;
      statement[1] =
          SELECT_NOTIFICATION_HIST_FOR_INVESTIGATION_SQL1
              + dataAccessWhereClause
              + " ORDER BY notHist.version_ctrl_nbr DESC";
      ...
      for (String s : statement) {
        ...
        for (NotificationSummaryContainer newVO : retval) {
          ...
          if (!newVO.getCdNotif().equals(NEDSSConstant.CLASS_CD_NOTF)) newVO.setCaseReport(true);
   ...
   */
  public Map<Object, Object> getAssociatedDocumentList(
      Long uid, String targetClassCd, String sourceClassCd) {
    Map<Object, Object> assocoiatedDocMap;

    String dataAccessWhereClause =
        queryHelper.getDataAccessWhereClause(NBSBOLookup.DOCUMENT, "VIEW", "");
    if (dataAccessWhereClause == null) {
      dataAccessWhereClause = "";
    } else {
      dataAccessWhereClause = AND_UPPERCASE + dataAccessWhereClause;
    ...
    assocoiatedDocMap =
        customRepository.getAssociatedDocumentList(uid, targetClassCd, sourceClassCd, assocQuery);
  ...
  public void updateNotification(
    ...
    NotificationContainer notificationContainer =
        notificationRepositoryUtil.getNotificationContainer(notificationUid);

...
public class ManagerAggregationService implements IManagerAggregationService {
  ...
  public ManagerAggregationService(
  ...
  public EdxLabInformationDto processingObservationMatching(
    ...
    if (observationDto != null) {
      ...
      LabResultProxyContainer matchedlabResultProxyVO =
          observationService.getObservationToLabResultContainer(observationDto.getObservationUid());
      observationMatchingService.processMatchedProxyVO(
          labResultProxyContainer, matchedlabResultProxyVO, edxLabInformationDto);
  ...
  public void serviceAggregation(
      LabResultProxyContainer labResult, EdxLabInformationDto edxLabInformationDto)
    ...
    Collection<ObservationContainer> observationContainerCollection =
        labResult.getTheObservationContainerCollection();
    Collection<PersonContainer> personContainerCollection =
        labResult.getThePersonContainerCollection();
  ...
  protected void progAndJurisdictionAggregation(
    ...
    if (observationRequest != null
        && observationRequest.getTheObservationDto().getProgAreaCd() == null) {
      programAreaService.getProgramArea(
          observationResults, observationRequest, edxLabInformationDto.getSendingFacilityClia());
    }

    if (observationRequest != null
        && observationRequest.getTheObservationDto().getJurisdictionCd() == null) {
      jurisdictionService.assignJurisdiction(
          personAggContainer.getPersonContainer(),
          personAggContainer.getProviderContainer(),
  ...
  protected void progAndJurisdictionAggregationHelper(
      LabResultProxyContainer labResult,
      EdxLabInformationDto edxLabInformationDto,
      PersonAggContainer personAggContainer,
      OrganizationContainer organizationContainer)
  ...
  protected void roleAggregation(LabResultProxyContainer labResult) {
      ...
      if (!coll.isEmpty()) {
        for (RoleDto roleDT : coll) {
          if (roleDT.isItDelete()) {
            mappedExistingRoleCollection.put(
                roleDT.getSubjectEntityUid() + roleDT.getCd() + roleDT.getScopingEntityUid(),
                roleDT);
          } else {
            mappedNewRoleCollection.put(
                roleDT.getSubjectEntityUid() + roleDT.getCd() + roleDT.getScopingEntityUid(),
                roleDT);
          }
        }
    ...
    if (!mappedNewRoleCollection.isEmpty()) {
      ...
      // will add all roles that are part of the old collection but are not contained in the new
      // collection
      ...
      for (Object o : list) {
        ...
        if (roleDT.getScopingEntityUid() == null) {
          ...
          if (count == 0) {
            ...
            modifiedRoleMap.put(
                roleDT.getSubjectEntityUid() + roleDT.getRoleSeq() + roleDT.getCd(), roleDT);
          }
  ...
  protected void observationAggregation(
      LabResultProxyContainer labResult,
      EdxLabInformationDto edxLabInformationDto,
      Collection<ObservationContainer> observationContainerCollection) {
    if (observationContainerCollection != null && !observationContainerCollection.isEmpty()) {
      for (ObservationContainer obsVO : observationContainerCollection) {
        if (obsVO.getTheObservationDto().getObservationUid()
                == edxLabInformationDto.getRootObserbationUid()
            && edxLabInformationDto.getRootObserbationUid() > 0) {
          long falseUid = -1;
          uidService.setFalseToNewForObservation(
              labResult, falseUid, obsVO.getTheObservationDto().getObservationUid());

...
public class ObservationCodeService implements IObservationCodeService {
  private static final Logger logger =
      LoggerFactory.getLogger(ObservationCodeService.class); // NOSONAR
  ...
  public String getReportingLabCLIA(BaseContainer proxy) {
    ...
    Long reportingLabUid =
        observationUtil.getUid(
            partColl,
            null,
   ...
   * deriveTheConditionCodeList - used by Associate to Investigations when associating an STD lab to
   * a closed investigation. Condition list determines the Processing Decision to show.
   */
  public ArrayList<String> deriveTheConditionCodeList(
      LabResultProxyContainer labResultProxyVO, ObservationContainer orderTest)
    ...
    if (!resultTests.isEmpty()) {
      derivedConditionList =
          getDerivedConditionList(
              reportingLabCLIA, resultTests, orderTest.getTheObservationDto().getElectronicInd());
    }
  ...
  private ArrayList<String> getDerivedConditionList(
      String reportingLabCLIA,
    ...
    for (ObservationContainer observationContainer : observationContainerCollection) {
      ...
      if (obsDomainCdSt1 != null
        ...
        if (electronicInd.equals(NEDSSConstant.ELECTRONIC_IND_ELR)) {
          resultedTestConditionList =
              getConditionsFromSNOMEDCodes(
                  reportingLabCLIA, observationContainer.getTheObsValueCodedDtoCollection());
        } else {
          resultedTestConditionList =
              getConditionsFromSNOMEDCodes(
                  NEDSSConstant.DEFAULT, observationContainer.getTheObsValueCodedDtoCollection());
        }

        // if no conditions found - try LN to retrieve Condition using Resulted Test --> LOINC
        // mapping
        ...
        if (resultedTestConditionList.isEmpty()) {
          String localResultDefaultConditionCd =
              getConditionCodeForLocalResultCode(
                  reportingLabCLIA, observationContainer.getTheObsValueCodedDtoCollection());
        ...
        if (resultedTestConditionList.isEmpty()) {
          String localTestDefaultConditionCd =
              getConditionCodeForLocalTestCode(reportingLabCLIA, observationContainer);
        ...
        if (resultedTestConditionList.isEmpty()) {
          String defaultLabTestConditionCd =
              getDefaultConditionForLabTestCode(obsDTCode, reportingLabCLIA);
    ...
    if (obsValueCodedDtoColl != null) {
      for (ObsValueCodedDto obsValueCodedDto : obsValueCodedDtoColl) {
        ...
        if (!codeSystemCd.equals(ELRConstant.ELR_SNOMED_CD)) {
          Map<String, Object> snomedMap =
              srteCodeObsService.getSnomed(
                  obsValueCodedDto.getCode(), ELRConstant.TYPE, reportingLabCLIA);
  ...
  private String getConditionForLOINCCode(
      String reportingLabCLIA, ObservationContainer resultTestVO) throws DataProcessingException {
  ...
  /**
   * Gets the default condition for a Local Result code. If we find that it maps to more than one
   * condition code, return nothing.
   *
   * @param reportingLabCLIA : String
   * @param obsValueCodedDtoColl: Collection
   * @return conditionCd : String
   */
  private String getConditionCodeForLocalResultCode(
      String reportingLabCLIA, Collection<ObsValueCodedDto> obsValueCodedDtoColl) {
    String conditionCd = "";
    ...
    for (ObsValueCodedDto obsValueCodedDto : obsValueCodedDtoColl) {
      ...
      if (code != null) {
        String defaultCondition =
            srteCodeObsService.getDefaultConditionForLocalResultCode(code, reportingLabCLIA);
   ...
   * @return conditionCd : String
   */
  private String getConditionCodeForLocalTestCode(
      String reportingLabCLIA, ObservationContainer resultTestVO) {
  ...
  private String getDefaultConditionForLabTestCode(String labTestCd, String reportingLabCLIA) {
    String conditionCd =
        srteCodeObsService.getDefaultConditionForLabTest(labTestCd, reportingLabCLIA);
    // see if the DEFAULT is set for the lab test if still not found..
    if ((conditionCd == null || conditionCd.isEmpty())
        && !reportingLabCLIA.equals(NEDSSConstant.DEFAULT)) {
      conditionCd =
          srteCodeObsService.getDefaultConditionForLabTest(labTestCd, NEDSSConstant.DEFAULT);
    }
    return (conditionCd);
  }
}

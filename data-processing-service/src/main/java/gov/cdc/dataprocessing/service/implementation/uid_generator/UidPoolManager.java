...
public class UidPoolManager {
  ...
  public void periodicRefill() {
    for (Map.Entry<String, Queue<LocalUidModel>> entry : uidPools.entrySet()) {
      ...
      LocalIdClass idClass =
          gaApplied ? LocalIdClass.valueOf(key.split("\\|")[1]) : LocalIdClass.valueOf(key);
  ...
  public LocalUidModel getNextUid(LocalIdClass idClass, boolean gaApplied)
  ...
  private void preloadPool(LocalIdClass idClass, boolean gaApplied) throws DataProcessingException {
    ...
    //        var localStartSeed =
    // localUidGeneratorRepository.reserveBatchAndGetStartSeed(idClass.name(), POOL_SIZE);
    var localStartSeed = getSeedFromSource(idClass.name());

    //        var gaStartSeed = gaApplied ?
    // localUidGeneratorRepository.reserveBatchAndGetStartSeed(LocalIdClass.GA.name(), POOL_SIZE) :
    // null;
    var gaStartSeed = gaApplied ? getSeedFromSource(LocalIdClass.GA.name()) : null;

    if (localStartSeed == null) {
      throw new DataProcessingException(
          "Failed to reserve UID batch from DB for " + idClass.name());
    }

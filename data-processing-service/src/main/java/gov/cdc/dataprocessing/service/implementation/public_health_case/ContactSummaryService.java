...
@Service
/**
 * Do not Attempt to refactor this Class Unless you know what need to be done Blindly refactoring
 * this class will causing the significant failure with Case Notification and Mark As Review action
 */
public class ContactSummaryService implements IContactSummaryService {
  ...
  public Collection<Object> getContactListForInvestigation(Long publicHealthCaseUID)
  ...
  private Collection<Object> getPHCContactNamedByPatientSummDTColl(Long publicHealthCaseUID) {
    String dataAccessWhereClause =
        queryHelper.getDataAccessWhereClause(NBSBOLookup.CT_CONTACT, "VIEW", "");
    ...
    String dataAccessWhereClause1 =
        queryHelper.getDataAccessWhereClause(NBSBOLookup.INVESTIGATION, "VIEW", "");
    ...
    if (dataAccessWhereClause1 == null) {
      ...
      dataAccessWhereClause1 =
          dataAccessWhereClause1.replaceAll(PROGRAM_JUS_OID, "contact.program_jurisdiction_oid");
  ...
  @SuppressWarnings("java:S5361")
  private Collection<Object> getPHCPatientNamedAsContactSummDTColl(Long publicHealthCaseUID)
      throws DataProcessingException {
    String dataAccessWhereClause =
        queryHelper.getDataAccessWhereClause(NBSBOLookup.INVESTIGATION, "VIEW", "");
    if (dataAccessWhereClause == null) {
      ...
      dataAccessWhereClause = AND_UPPERCASE + dataAccessWhereClause;
      dataAccessWhereClause =
          dataAccessWhereClause.replaceAll(PROGRAM_JUS_OID, "subject.program_jurisdiction_oid");
    ...
    if (dataAccessWhereClause1 == null) {
      ...
      dataAccessWhereClause1 = AND_UPPERCASE + dataAccessWhereClause1;
      dataAccessWhereClause1 =
          dataAccessWhereClause1.replaceAll(PROGRAM_JUS_OID, CT_PROGRAM_JUS_OID);
  ...
  @SuppressWarnings("java:S5361")
  private Collection<Object> getPHCPatientOtherNamedAsContactSummDTColl(Long publicHealthCaseUID)
      throws DataProcessingException {
    String dataAccessWhereClause =
        queryHelper.getDataAccessWhereClause(NBSBOLookup.INVESTIGATION, "VIEW", "");
    if (dataAccessWhereClause == null) {
  ...
  private Collection<Object> getContactNamedByPatientDTColl(String sql) {
    ...
    cTContactNameByPatientSummDTColl =
        new ArrayList<>(customRepository.getContactByPatientInfo(sql));
    for (CTContactSummaryDto cTContactSumyDT : cTContactNameByPatientSummDTColl) {
      ...
      if (!contactNameColl.isEmpty()) {
        for (PersonName o : contactNameColl) {
          ...
          if (personNameDT.getNmUseCd().equalsIgnoreCase(NEDSSConstant.LEGAL_NAME)) {
            String lastName =
                (personNameDT.getLastNm() == null)
                    ? NO_LAST_NAME_INVESTIGATOR
                    : personNameDT.getLastNm();
            String firstName =
                (personNameDT.getFirstNm() == null)
                    ? NO_FIRST_NAME_INVESTIGATOR
      ...
      // Business rule with convoluted logic, if contact Processing Decision is RSC or SR and the
      // contiact's investigation disposition
      // is A, the disposition of the Contact Record will be Z.
      // If the disposition on the Contact�s existing investigation is C, the disposition of the
      // Contact Record will be E.
      if (cTContactSumyDT.getContactProcessingDecisionCd() != null
          && cTContactSumyDT.getDispositionCd() != null
          && (cTContactSumyDT
                  .getContactProcessingDecisionCd()
                  .equals(CTConstants.RecordSearchClosure)
              || cTContactSumyDT
                  .getContactProcessingDecisionCd()
                  .equals(CTConstants.SecondaryReferral))) {
  ...
  @SuppressWarnings("java:S3776")
  private Collection<Object> getPatientNamedAsContactSummDTColl(String sql, boolean otherInfected)
      throws DataProcessingException {
    ...
    for (CTContactSummaryDto cTContactSumyDT : ctNameByPatientSummDTColl) {
      ...
      cTContactSumyDT.setAssociatedMap(
          retrieveSummaryService.getAssociatedDocumentList(
      ...
      // Setting the disposition to the source patient's disposition for the section 'patient named
      // by contacts'

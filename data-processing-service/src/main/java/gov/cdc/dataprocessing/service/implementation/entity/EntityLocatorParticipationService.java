...
public class EntityLocatorParticipationService implements IEntityLocatorParticipationService {
  ...
  public EntityLocatorParticipationService(
  ...
  protected void deleteEntityLocatorParticipation(
      Collection<EntityLocatorParticipationDto> locatorCollection, Long patientUid) {
    var deletePostal =
        locatorCollection.stream()
            .filter(x -> x.getClassCd().equalsIgnoreCase(NEDSSConstant.POSTAL) && x.isItDelete())
            .toList();
    ...
    if (!deletePostal.isEmpty()) {
      ...
      var postalRevision =
          postalLocatorRepository.findByPostalLocatorUids(
              deletePostal.stream().map(EntityLocatorParticipationDto::getLocatorUid).toList());
      if (parentUid != null && postalRevision.isPresent()) {
        var entityMprEntityRes = entityLocatorJdbcRepository.findByEntityUid(parentUid);
        var entityMprRes =
            entityLocatorParticipationRepository.findLocatorUidsByEntityUid(parentUid);
        if (entityMprRes.isPresent()) {
          ...
          if (postalMpr.isPresent()) {
            ...
            for (var mpr : postalMpr.get()) {
              ...
              var birCheck =
                  entityMprEntityRes.stream()
                      .filter(
                          x ->
                              Objects.equals(x.getLocatorUid(), mpr.getPostalLocatorUid())
                                  && x.getUseCd().equalsIgnoreCase("BIR"))
                      .toList();
              // Comparing String of MPR matched with Incoming Revision Then Do Delete
              if (comparingStrList.contains(comparingString.toString().toUpperCase())
                  && !birCheck.isEmpty()) {
      ...
      for (var deleteRevision : deletePostal) {
        ...
        if (deleteRevision.getUseCd().equalsIgnoreCase("BIR")) {
          ...
          dataModifierReposJdbc.deleteLocatorById(
              deleteRevision.getEntityUid(), deleteRevision.getLocatorUid());
        }
      }
    }

    locatorCollection.removeIf(
        x ->
            x.getClassCd().equalsIgnoreCase(NEDSSConstant.POSTAL)
                && x.isItDelete()
                && x.getUseCd().equalsIgnoreCase("BIR"));
  }

  @SuppressWarnings({"java:S6541", "java:S3776", "java:S6204"})
  public void updateEntityLocatorParticipation(
      Collection<EntityLocatorParticipationDto> locatorCollection, Long patientUid)
      throws DataProcessingException {
    ArrayList<EntityLocatorParticipationDto> personList =
        (ArrayList<EntityLocatorParticipationDto>) locatorCollection;
    var uid = patientUid;
    ...
    if (!entityLocatorParticipations.isEmpty()) {
      ...
      physicalLocators =
          entityLocatorParticipations.stream()
              ...
              .sorted(
                  Comparator.comparing(EntityLocatorParticipation::getRecordStatusTime).reversed())
              .toList();
      postalLocators =
          entityLocatorParticipations.stream()
      ...
      for (EntityLocatorParticipationDto entityLocatorParticipationDto : personList) {
        ...
        if (entityLocatorParticipationDto.getClassCd().equals(NEDSSConstant.PHYSICAL)
            && entityLocatorParticipationDto.getThePhysicalLocatorDto() != null) {
          if (!physicalLocators.isEmpty()) {
            var existingLocator =
                physicalLocatorRepository.findByPhysicalLocatorUids(
            ...
            if (existingLocator.isPresent()) {
              ...
              if (!compareStringList.contains(
                  Arrays.toString(
                          entityLocatorParticipationDto.getThePhysicalLocatorDto().getImageTxt())
                      .toUpperCase())) {
                uid = entityLocatorParticipationDto.getEntityUid();
                entityLocatorParticipationDto
                    .getThePhysicalLocatorDto()
                    .setPhysicalLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
                physicalLocatorRepository.save(
                    new PhysicalLocator(entityLocatorParticipationDto.getThePhysicalLocatorDto()));
              } else {
                newLocator = false;
              }
            } else {
          ...
          if (!postalLocators.isEmpty()) {
            ...
            if (existingLocator.isPresent()) {
              ...
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCityCd());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCityDescTxt());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCntryCd());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCntryDescTxt());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCntyCd());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getCntyDescTxt());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getStateCd());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getStreetAddr1());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getStreetAddr2());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getThePostalLocatorDto().getZipCd());

              if (!compareStringList.contains(existComparingLocator.toString().toUpperCase())) {
                uid = entityLocatorParticipationDto.getEntityUid();
                entityLocatorParticipationDto
                    .getThePostalLocatorDto()
                    .setPostalLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
                entityLocatorParticipationDto
                    .getThePostalLocatorDto()
                    .setRecordStatusCd(NEDSSConstant.ACTIVE);
                postalLocatorRepository.save(
                    new PostalLocator(entityLocatorParticipationDto.getThePostalLocatorDto()));
          ...
          if (!teleLocators.isEmpty()) {
            var existingLocator =
                teleLocatorRepository.findByTeleLocatorUids(
                    teleLocators.stream()
                        .map(EntityLocatorParticipation::getLocatorUid)
                        .collect(Collectors.toList()));
            List<String> compareStringList = new ArrayList<>();
            ...
            if (existingLocator.isPresent()) {
              ...
              StringBuilder existComparingLocator = new StringBuilder();
              existComparingLocator.append(
                  entityLocatorParticipationDto.getTheTeleLocatorDto().getCntryCd());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getTheTeleLocatorDto().getEmailAddress());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getTheTeleLocatorDto().getExtensionTxt());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getTheTeleLocatorDto().getPhoneNbrTxt());
              existComparingLocator.append(
                  entityLocatorParticipationDto.getTheTeleLocatorDto().getUrlAddress());

              if (!compareStringList.contains(existComparingLocator.toString().toUpperCase())) {
                uid = entityLocatorParticipationDto.getEntityUid();
                entityLocatorParticipationDto
                    .getTheTeleLocatorDto()
                    .setTeleLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
                teleLocatorRepository.save(
                    new TeleLocator(entityLocatorParticipationDto.getTheTeleLocatorDto()));
              } else {
                newLocator = false;
              }
            } else {
  ...
  public void createEntityLocatorParticipation(
      Collection<EntityLocatorParticipationDto> locatorCollection, Long uid)
      throws DataProcessingException {
    ArrayList<EntityLocatorParticipationDto> personList =
        (ArrayList<EntityLocatorParticipationDto>) locatorCollection;
    for (EntityLocatorParticipationDto entityLocatorParticipationDto : personList) {
      ...
      if (entityLocatorParticipationDto.getClassCd().equals(NEDSSConstant.PHYSICAL)
        ...
        entityLocatorParticipationDto
            .getThePhysicalLocatorDto()
            .setPhysicalLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
        entityLocatorJdbcRepository.createPhysicalLocator(
            new PhysicalLocator(entityLocatorParticipationDto.getThePhysicalLocatorDto()));
        inserted = true;
      } else if (entityLocatorParticipationDto.getClassCd().equals(NEDSSConstant.POSTAL)
          && entityLocatorParticipationDto.getThePostalLocatorDto() != null) {
        entityLocatorParticipationDto
            .getThePostalLocatorDto()
            .setPostalLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
        entityLocatorJdbcRepository.createPostalLocator(
            new PostalLocator(entityLocatorParticipationDto.getThePostalLocatorDto()));
        inserted = true;
      } else if (entityLocatorParticipationDto.getClassCd().equals(NEDSSConstant.TELE)
          && entityLocatorParticipationDto.getTheTeleLocatorDto() != null
        ...
        entityLocatorParticipationDto
            .getTheTeleLocatorDto()
            .setTeleLocatorUid(localUid.getGaTypeUid().getSeedValueNbr());
        entityLocatorJdbcRepository.createTeleLocator(
            new TeleLocator(entityLocatorParticipationDto.getTheTeleLocatorDto()));
        inserted = true;
      }
      ...
      if (inserted) {
        ...
        entityLocatorJdbcRepository.createEntityLocatorParticipation(
            new EntityLocatorParticipation(entityLocatorParticipationDto, tz));
      }
    }
  }

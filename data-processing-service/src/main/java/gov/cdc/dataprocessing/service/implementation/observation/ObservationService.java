...
public class ObservationService implements IObservationService {
  ...
  public LabResultProxyContainer getObservationToLabResultContainer(Long observationUid)
  ...
  public ObservationDto processingLabResultContainer(
  ...
  public boolean processObservation(Long observationUid) throws DataProcessingException {
    return processObservationWithProcessingDecision(observationUid, null, null);
  }

  public void setLabInvAssociation(Long labUid, Long investigationUid)
      throws DataProcessingException {
  ...
  /** Loading Existing Either Observation or Intervention was: getActVO */
  protected BaseContainer getAbstractObjectForObservationOrIntervention(String actType, Long anUid)
      throws DataProcessingException {
  ...
  protected Map<DataProcessingMapKey, Object> retrieveEntityFromParticipationForContainer(
      Collection<ParticipationDto> partColl) {
    Map<DataProcessingMapKey, Object> entityHolder = new HashMap<>();

    // Retrieve associated persons
    Map<DataProcessingMapKey, Object> assocMapper =
        retrievePersonAndRoleFromParticipation(partColl);
    ...
    entityHolder.put(
        DataProcessingMapKey.ORGANIZATION, retrieveOrganizationFromParticipation(partColl));
  ...
  protected Collection<Object> retrieveOrganizationFromParticipation(
      Collection<ParticipationDto> partColl) {
    ...
    for (ParticipationDto partDT : partColl) {
      ...
      if (subjectClassCd != null
        ...
        var orgContainer =
            organizationRepositoryUtil.loadObject(organizationUid, partDT.getActUid());
  ...
  protected Collection<Object> retrieveMaterialFromParticipation(
      Collection<ParticipationDto> partColl) {
  ...
  /**
   * Mapping Person and Role into Object Array Values from Participation Was:
   * retrievePersonVOsForProxyVO
   */
  @SuppressWarnings({"java:S3776", "java:S1640"})
  protected Map<DataProcessingMapKey, Object> retrievePersonAndRoleFromParticipation(
      Collection<ParticipationDto> partColl) {
    ...
    for (ParticipationDto partDT : partColl) {
      ...
      if (subjectClassCd != null
        ...
        if (typeCd != null && typeCd.equalsIgnoreCase(NEDSSConstant.PAR110_TYP_CD)) {
          ...
          Collection<PersonContainer> scopedPersons =
              retrieveScopedPersons(vo.getThePersonDto().getPersonUid());
          if (scopedPersons != null && !scopedPersons.isEmpty()) {
            for (var person : scopedPersons) {
              if (person.getTheRoleDtoCollection() != null
                  && !person.getTheRoleDtoCollection().isEmpty()) {
  ...
  @SuppressWarnings("java:S1640")
  Map<DataProcessingMapKey, Object> retrieveActForLabResultContainer(
      Collection<ActRelationshipDto> actRelColl) throws DataProcessingException {
    Map<DataProcessingMapKey, Object> mapper = new HashMap<>();

    // Retrieve associated interventions
    mapper.put(
        DataProcessingMapKey.INTERVENTION, retrieveInterventionFromActRelationship(actRelColl));
  ...
  /** Retrieving Observation and the assoc Organization was: retrieveObservationVOsForProxyVO */
  @SuppressWarnings({"java:S3776", "java:S1640", "java:S135"})
  protected Map<DataProcessingMapKey, Object> retrieveObservationFromActRelationship(
      Collection<ActRelationshipDto> actRelColl) throws DataProcessingException {
    Map<DataProcessingMapKey, Object> mapper = new HashMap<>();
    ...
    for (ActRelationshipDto actRelDT : actRelColl) {
      ...
      if (sourceClassCd != null
        ...
        if (typeCd != null && typeCd.equals(NEDSSConstant.ACT_TYPE_PROCESSING_DECISION)) {
          ObservationContainer processingDecObservationContainer =
              (ObservationContainer)
                  getAbstractObjectForObservationOrIntervention(
                      NEDSSConstant.OBSERVATION_CLASS_CODE, observationUid);
        ...
        if (typeCd != null && typeCd.equalsIgnoreCase("APND")) {
          ObservationContainer ordTestCommentVO =
              (ObservationContainer)
                  getAbstractObjectForObservationOrIntervention(
                      NEDSSConstant.OBSERVATION_CLASS_CODE, observationUid);
          theObservationContainerCollection.add(ordTestCommentVO);
          Collection<ActRelationshipDto> arColl =
              ordTestCommentVO.getTheActRelationshipDtoCollection(); // NOSONAR
          if (arColl != null) {
            for (ActRelationshipDto ordTestDT : arColl) {
              if (ordTestDT.getTypeCd().equals("COMP")) {
                // add the resulted test to the collection
                ObservationContainer resTestVO =
                    (ObservationContainer)
                        getAbstractObjectForObservationOrIntervention(
                            NEDSSConstant.OBSERVATION_CLASS_CODE, ordTestDT.getSourceActUid());

                // BB - civil0012298 - Retrieve User Name to be displayed instead of ID
                resTestVO
                    .getTheObservationDto()
                    .setAddUserName(AuthUtil.authUser.getUserId()); // NOSONAR
        ...
        // If a Resulted Test observation
          ...
          ObservationContainer rtObservationContainer =
              (ObservationContainer)
                  getAbstractObjectForObservationOrIntervention(
                      NEDSSConstant.OBSERVATION_CLASS_CODE, observationUid);
          ...
          OrganizationContainer rtPerformingLab =
              retrievePerformingLabAkaOrganizationFromParticipation(
                  rtObservationContainer.getTheParticipationDtoCollection());
          ...
          Collection<ObservationContainer> reflexObsColl =
              retrieveReflexObservationsFromActRelationship(
                  rtObservationContainer.getTheActRelationshipDtoCollection());
  ...
  protected Collection<ObservationContainer> retrieveReflexObservationsFromActRelationship(
      Collection<ActRelationshipDto> actRelColl) throws DataProcessingException {
    ...
    for (ActRelationshipDto actRelDT : actRelColl) {
      ...
      if (typeCd != null
          ...
          && recordStatusCd.equalsIgnoreCase(NEDSSConstant.ACTIVE)) {
        Long observationUid = actRelDT.getSourceActUid();
        ObservationContainer reflexObs =
            (ObservationContainer)
                getAbstractObjectForObservationOrIntervention(
                    NEDSSConstant.OBSERVATION_CLASS_CODE, observationUid);
        ...
        Collection<ObservationContainer> reflexRTs =
            retrieveReflexRTsAkaObservationFromActRelationship(
                reflexObs.getTheActRelationshipDtoCollection());
  ...
  /** Retrieves the Reflex Result Test was: retrieveReflexRTs */
  @SuppressWarnings("java:S135")
  protected Collection<ObservationContainer> retrieveReflexRTsAkaObservationFromActRelationship(
      Collection<ActRelationshipDto> actRelColl) throws DataProcessingException {
  ...
  @SuppressWarnings("java:S135")
  protected OrganizationContainer retrievePerformingLabAkaOrganizationFromParticipation(
      Collection<ParticipationDto> partColl) {
  ...
  protected Collection<Object> retrieveInterventionFromActRelationship(
      Collection<ActRelationshipDto> actRelColl) throws DataProcessingException {
    ...
    for (ActRelationshipDto actRelDT : actRelColl) {
      ...
      if (sourceClassCd != null
        ...
        var intervention =
            getAbstractObjectForObservationOrIntervention(
                NEDSSConstant.INTERVENTION_CLASS_CODE, interventionUid);
  ...
  protected void processingNotELRLab(
      boolean isELR,
      LabResultProxyContainer lrProxyVO,
    ...
    if (!isELR) {
      ...
      Collection<ActRelationshipDto> col =
          actRelationshipService.loadActRelationshipBySrcIdAndTypeCode(
              observationId, NEDSSConstant.LAB_REPORT);
      if (col != null && !col.isEmpty()) {
        lrProxyVO.setAssociatedInvInd(true);
      }

      // get EDX Document data
      Collection<EDXDocumentDto> documentList =
          edxDocumentService.selectEdxDocumentCollectionByActUid(observationId);
      ...
      ArrayList<String> conditionList =
          observationCodeService.deriveTheConditionCodeList(lrProxyVO, orderedTest);
  ...
  protected void loadingObservationToLabResultContainerActHelper(
      LabResultProxyContainer lrProxyVO,
      boolean isELR,
    ...
    if (!allAct.isEmpty()) {
      ...
      lrProxyVO.setTheInterventionVOCollection(
          (Collection<Object>) allAct.get(DataProcessingMapKey.INTERVENTION));

      // Set observation collection
      Collection<ObservationContainer> obsColl =
          (Collection<ObservationContainer>) allAct.get(DataProcessingMapKey.OBSERVATION);
      if (obsColl == null) {
      ...
      Collection<OrganizationContainer> labColl =
          (Collection<OrganizationContainer>) allAct.get(DataProcessingMapKey.ORGANIZATION);
  ...
  /**
   * LabResultProxyVO getLabResultProxyVO(Long observationId, boolean isELR, NBSSecurityObj
   * nbsSecurityObj)
   */
  protected LabResultProxyContainer loadingObservationToLabResultContainer(
      Long observationId, boolean isELR) throws DataProcessingException {
    ...
    ObservationContainer orderedTest =
        (ObservationContainer)
            getAbstractObjectForObservationOrIntervention(
                NEDSSConstant.OBSERVATION_CLASS_CODE, observationId);
    Collection<ParticipationDto> partColl =
        orderedTest.getTheParticipationDtoCollection(); // NOSONAR

    if (partColl != null && !partColl.isEmpty()) {
      Map<DataProcessingMapKey, Object> allEntity =
          retrieveEntityFromParticipationForContainer(partColl);
      if (!allEntity.isEmpty()) {
        lrProxyVO.setThePersonContainerCollection(
            (Collection<PersonContainer>) allEntity.get(DataProcessingMapKey.PERSON));
        lrProxyVO.setTheOrganizationContainerCollection(
            (Collection<OrganizationContainer>) allEntity.get(DataProcessingMapKey.ORGANIZATION));
        lrProxyVO.setTheMaterialContainerCollection(
            (Collection<MaterialContainer>) allEntity.get(DataProcessingMapKey.MATERIAL));
        Collection<RoleDto> roleObjs =
            (Collection<RoleDto>) allEntity.get(DataProcessingMapKey.ROLE);
    ...
    processingNotELRLab(isELR, lrProxyVO, orderedTest, observationId);
    try {
  ...
  protected Map<Object, Object> setLabResultProxy(LabResultProxyContainer labResultProxyVO)
      throws DataProcessingException {

    Map<Object, Object> returnVal =
        setLabResultProxyWithoutNotificationAutoResend(labResultProxyVO);
    ...
    if (labResultProxyVO.getMessageLogDCollection() != null
      ...
      try {
        ...
        logger.error(
            "Unable to store the Error message for = {}",
  ...
  protected NNDActivityLogDto updateLabResultWithAutoResendNotification(
      LabResultProxyContainer labResultProxyVO) throws DataProcessingException {
    ...
    try {
      ...
      Collection<ObservationContainer> observationCollection =
          labResultProxyVO.getTheObservationContainerCollection();
      ObservationContainer obsVO =
          findObservationByCode(observationCollection, NEDSSConstant.LAB_REPORT);
      ...
      logger.error(
          "Exception occurred while calling nndMessageSenderHelper.updateAutoResendNotificationsAsync");
  ...
  protected Map<Object, Object> setLabResultProxyWithoutNotificationAutoResend(
      LabResultProxyContainer labResultProxyVO) throws DataProcessingException {
    ...
    Map<Object, Object> obsResults =
        processObservations(labResultProxyVO, returnVal, patientMprUid);
    ...
    processEdxDocuments(
        labResultProxyVO, (Long) obsResults.get(NEDSSConstant.SETLAB_RETURN_OBS_UID));
    persistPageAnswers(
        labResultProxyVO, (Long) obsResults.get(NEDSSConstant.SETLAB_RETURN_OBS_UID));
    return returnVal;
  }

  protected Long processPatient(LabResultProxyContainer proxy, Map<Object, Object> returnVal)
      throws DataProcessingException {
    Long mprUid =
        personUtil.processLabPersonContainerCollection(
            proxy.getThePersonContainerCollection(), false, proxy);
    if (mprUid != null) {
  ...
  protected Map<Object, Object> processObservations(
      LabResultProxyContainer proxy, Map<Object, Object> returnVal, Long mprUid)
      throws DataProcessingException {
    ...
    if (mprUid != null && mprUid < 0) {
      mprUid =
          participationService.findPatientMprUidByObservationUid(
              NEDSSConstant.PERSON,
              NEDSSConstant.PAR110_TYP_CD,
              (Long) obsResults.get(NEDSSConstant.SETLAB_RETURN_OBS_UID));
      if (mprUid == null) {
        throw new DataProcessingException(
            "Expected this observation to be associated with a patient, observation uid = "
                + obsResults.get(NEDSSConstant.SETLAB_RETURN_OBS_UID));
      }
      returnVal.put(NEDSSConstant.SETLAB_RETURN_MPR_UID, mprUid);
    }
  ...
  protected void processOrganizations(LabResultProxyContainer proxy)
      throws DataProcessingException {
    ...
    for (OrganizationContainer orgContainer : proxy.getTheOrganizationContainerCollection()) {
      ...
      Integer version =
          (loaded != null && loaded.getTheOrganizationDto() != null)
              ? loaded.getTheOrganizationDto().getVersionCtrlNbr()
              : null;
  ...
  protected void processParticipations(LabResultProxyContainer proxy)
      throws DataProcessingException {
  ...
  protected void processActRelationships(LabResultProxyContainer proxy)
      throws DataProcessingException {
  ...
  protected void processEdxDocuments(LabResultProxyContainer proxy, Long observationUid)
      throws DataProcessingException {
    ...
    if (docs == null
        || docs.isEmpty()
        || root.getElectronicInd() == null
        || !root.getElectronicInd().equals(NEDSSConstant.YES)) return;
  ...
  protected void persistPageAnswers(LabResultProxyContainer proxy, Long observationUid)
      throws DataProcessingException {
    ObservationDto root = observationUtil.getRootObservationDto(proxy);
  ...
  @SuppressWarnings("java:S135")
  protected ObservationContainer findObservationByCode(
      Collection<ObservationContainer> coll, String strCode) {
    if (coll == null) {
      return null;
    }
  ...
  /** Processing observation collection Original Name: processObservationVOCollection */
  protected Map<Object, Object> processObservationContainerCollection(
      BaseContainer proxyVO, boolean elrProcessing) throws DataProcessingException {
    if (proxyVO instanceof LabResultProxyContainer labResultProxyContainer) {
      ...
      throw new DataProcessingException(
          "Expected a valid observation proxy vo, it is: " + proxyVO.getClass().getName());
    }
  }

  /** Original Name: processLabReportObsVOCollection */
  @SuppressWarnings({"java:S1481", "java:S1854", "java:S3626"})
  protected Map<Object, Object> processLabReportObsContainerCollection(
      LabResultProxyContainer labResultProxyVO, boolean elrProcessing)
      throws DataProcessingException {
    Collection<ObservationContainer> obsContainerCollection =
        labResultProxyVO.getTheObservationContainerCollection();
    ...
    String electronicInd =
        observationUtil.getRootObservationDto(labResultProxyVO).getElectronicInd();
  ...
  protected Map<Object, Object> processLabReportOrderTest(
      LabResultProxyContainer labResultProxyVO, boolean isELR) throws DataProcessingException {
  ...
  protected void handleProgramAreaCode(
      LabResultProxyContainer proxy, ObservationContainer orderTest, Map<Object, Object> errors)
      throws DataProcessingException {
  ...
  protected void handleJurisdictionCode(
      LabResultProxyContainer proxy, ObservationDto dto, Map<Object, Object> errors)
      throws DataProcessingException {
  ...
  protected void normalizeJurisdictionCode(ObservationDto dto) {
    ...
    if (code != null
        && (code.trim().isEmpty()
            || code.equalsIgnoreCase("ANY")
            || code.equalsIgnoreCase("NONE"))) {
  ...
  protected String processingOrderTestStateTransition(
      LabResultProxyContainer labResultProxyVO,
      ObservationContainer orderTest,
      String businessTriggerCd,
      boolean isELR) {
    if (labResultProxyVO.isItNew()
        && orderTest.getTheObservationDto().getProcessingDecisionCd() != null
        && !orderTest.getTheObservationDto().getProcessingDecisionCd().trim().isEmpty()) {
  ...
  protected void performOrderTestStateTransition(
      LabResultProxyContainer labResultProxyVO, ObservationContainer orderTest, boolean isELR)
      throws DataProcessingException {
    String businessTriggerCd = null;
    ObservationDto newObservationDto;

    businessTriggerCd =
        processingOrderTestStateTransition(labResultProxyVO, orderTest, businessTriggerCd, isELR);
    Integer existObsVer = null;
    if (orderTest.getTheObservationDto().getUid() > 0) {
      var existObs =
          observationRepositoryUtil.loadObject(orderTest.getTheObservationDto().getUid());
    ...
    newObservationDto =
        (ObservationDto)
            prepareAssocModelHelper.prepareVO(
                orderTest.getTheObservationDto(),
  ...
  protected Long storeObservationVOCollection(BaseContainer proxyVO)
      throws DataProcessingException {
  ...
  protected boolean isRootObservation(
      ObservationContainer observationContainer, boolean isLabResultProxyVO) {
    ObservationDto dto = observationContainer.getTheObservationDto();

    String obsDomain = dto.getObsDomainCdSt1();
    if (isLabResultProxyVO
        && NEDSSConstant.ORDERED_TEST_OBS_DOMAIN_CD.equalsIgnoreCase(obsDomain)) {
      return true;
    }
  ...
  protected void handleFalseUidReplacement(
      BaseContainer proxyVO, ObservationContainer observationContainer, Long realUid) {
  ...
  protected boolean processObservationWithProcessingDecision(
      Long observationUid, String processingDecisionCd, String processingDecisionTxt)
      throws DataProcessingException {
    ...
    if (observationDT.getRecordStatusCd().equalsIgnoreCase(NEDSSConstant.OBS_UNPROCESSED)) {
      ...
      RootDtoInterface rootDTInterface =
          prepareAssocModelHelper.prepareVO(
              observationDT,
  ...
  protected void setObservationAssociations(
      Long investigationUid, Collection<LabReportSummaryContainer> observationSummaryVOColl)
      throws DataProcessingException {
    investigationService.setAssociations(
        investigationUid, observationSummaryVOColl, null, null, null, true);
  }
}

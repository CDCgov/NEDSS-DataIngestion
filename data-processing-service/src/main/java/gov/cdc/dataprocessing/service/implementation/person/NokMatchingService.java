...
public class NokMatchingService extends NokMatchingBaseService implements INokMatchingService {
  ...
  public EdxPatientMatchDto getMatchingNextOfKin(PersonContainer personContainer)
        ...
        if (nameAddStrSt1 != null) {
          ...
          edxPatientMatchFoundDT =
              getEdxPatientMatchRepositoryUtil()
                  .getEdxPatientMatchOnMatchString(edxPatientFoundDT.getTypeCd(), nameAddStrSt1);
      ...
      if (nameTelePhoneStrList != null && !nameTelePhoneStrList.isEmpty()) {
        for (String s : nameTelePhoneStrList) {
          ...
          if (nameTelePhone != null) {
            ...
            edxPatientMatchFoundDT =
                getEdxPatientMatchRepositoryUtil()
                    .getEdxPatientMatchOnMatchString(edxPatientFoundDT.getTypeCd(), nameTelePhone);
            if (edxPatientMatchFoundDT.getPatientUid() == null
                || edxPatientMatchFoundDT.getPatientUid() <= 0) {
              matchFound = false;
            } else {
    ...
    if (!matchFound) {
      ...
      if (personContainer.getThePersonDto().getCd() != null
          && personContainer.getThePersonDto().getCd().equals(NEDSSConstant.PAT)) { // Patient

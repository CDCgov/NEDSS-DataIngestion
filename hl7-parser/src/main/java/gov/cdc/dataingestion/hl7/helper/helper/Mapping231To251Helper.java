...
public class Mapping231To251Helper {
    ...
    try {
      if (inMsh231 != null) {
        ...
        if (inMsh231.getDateTimeOfMessage() != null && !inMsh231.getDateTimeOfMessage().isEmpty()) {
          ...
          ts.setDegreeOfPrecision(
              inMsh231.getDateTimeOfMessage().getDegreeOfPrecision().getValue());
        } else {
  ...
  public static PatientIdentification mapPid(PID inPid231, PatientIdentification outPid251) {
    ...
    if (outPid251.getPatientId() != null) {
      outPid251
          ...
          .add(mapCxWithNullToCx(inPid231.getPatientID(), outPid251.getPatientId(), "PT"));
    }
    ...
    for (int b = 0; b < outPid251.getAlternativePatientId().size(); b++) {
      if (outPid251.getAlternativePatientId().get(b) != null) {
        outPid251
            ...
            .add(
                mapCxWithNullToCx(
                    inPid231.getAlternatePatientIDPID(b),
                    outPid251.getAlternativePatientId().get(b),
                    "APT"));
      }
    }
    ...
    for (int c = 0; c < outPid251.getPatientAlias().size(); c++) {
      if (outPid251.getPatientAlias().get(c) != null) {
        outPid251
            .getPatientName()
            .add(mapXpnToXpn(outPid251.getPatientAlias().get(c), new Xpn(), "A"));
      }
    }
    // Race - hapi
    ...
    outPid251
        .getPatientIdentifierList()
        .add(mapSocialSecurityToPatientIdentifier(outPid251.getSsnNumberPatient(), new Cx()));
    ...
    outPid251
        .getPatientIdentifierList()
        .add(mapDlnToCx(outPid251.getDriverLicenseNumberPatient(), new Cx()));
    return outPid251;
  }

  private static PatientIdentification getPatientIdentifierList(
      PID inPid231, PatientIdentification outPid251) {
      ...
      if (inPid231.getPatientIdentifierList(a) != null) {
        ...
        if (patientIdentifier.getIdentifierTypeCode() == null
          ...
          newPatientIdentifier =
              mapCxWithNullToCx(
                  inPid231.getPatientIdentifierList(a),
                  outPid251.getPatientIdentifierList().get(a),
                  "U");
        } else {
          newPatientIdentifier =
              mapCxWithNullToCx(
                  inPid231.getPatientIdentifierList(a),
                  outPid251.getPatientIdentifierList().get(a),
                  patientIdentifier.getIdentifierTypeCode().getValue());
        }
  ...
  private static CommonOrder mapCommonOrderListHelper1st(ORC inOrc231, CommonOrder outOrc251) {
    ...
    if (inOrc231.getEntererSLocation() != null) {
      outOrc251.setEntererLocation(
          mapPl231(inOrc231.getEntererSLocation(), outOrc251.getEntererLocation()));
    }
  ...
  private static CommonOrder mapCommonOrderListHelper1stNested(
      ORC inOrc231, CommonOrder outOrc251) {
    ...
    if (inOrc231.getOrderEffectiveDateTime() != null) {
      outOrc251.setOrderEffectiveDateTime(
          mapTs231(inOrc231.getOrderEffectiveDateTime(), outOrc251.getOrderEffectiveDateTime()));
    }

    if (inOrc231.getOrderControlCodeReason() != null) {
      outOrc251.setOrderControlCodeReason(
          mapCe231(inOrc231.getOrderControlCodeReason(), outOrc251.getOrderControlCodeReason()));
    }

    if (inOrc231.getEnteringOrganization() != null) {
      outOrc251.setEnteringOrganization(
          mapCe231(inOrc231.getEnteringOrganization(), outOrc251.getEnteringOrganization()));
    }

    if (inOrc231.getEnteringDevice() != null) {
      outOrc251.setEnteringDevice(
          mapCe231(inOrc231.getEnteringDevice(), outOrc251.getEnteringDevice()));
    }
  ...
  private static CommonOrder mapCommonOrderListHelper2st(ORC inOrc231, CommonOrder outOrc251) {
    ...
    for (int h = 0; h < inOrc231.getOrderingFacilityPhoneNumber().length; h++) {
      if (inOrc231.getOrderingFacilityPhoneNumber(h) != null) {
        orderingFacilityPhoneList.add(
            mapXtn231(inOrc231.getOrderingFacilityPhoneNumber(h), new Xtn()));
      }
    }
    ...
    for (int j = 0; j < inOrc231.getOrderingProviderAddress().length; j++) {
      if (inOrc231.getOrderingProviderAddress(j) != null) {
        orderingProviderAddressList.add(
            mapXad231(inOrc231.getOrderingProviderAddress(j), new Xad()));
      }
    }
  ...
  private static CommonOrder orderControlCheck(ORC inOrc231, CommonOrder outOrc251) {
    if ((inOrc231.getOrderControl() == null)
        || (inOrc231.getOrderControl() != null && inOrc231.getOrderControl().getValue() == null)
        || (inOrc231.getOrderControl() != null
            && inOrc231.getOrderControl().getValue() != null
            && inOrc231.getOrderControl().getValue().isEmpty())) {
  ...
  public static CommonOrder mapCommonOrder(ORC inOrc231, CommonOrder outOrc251) {
    ...
    if (inOrc231.getPlacerOrderNumber() != null) {
      outOrc251.setPlacerGroupNumber(
          mapEi231(inOrc231.getPlacerOrderNumber(), outOrc251.getPlacerGroupNumber()));
    }

    if (inOrc231.getFillerOrderNumber() != null) {
      outOrc251.setFillerOrderNumber(
          mapEi231(inOrc231.getFillerOrderNumber(), outOrc251.getFillerOrderNumber()));
    }

    if (inOrc231.getPlacerGroupNumber() != null) {
      outOrc251.setPlacerGroupNumber(
          mapEi231(inOrc231.getPlacerGroupNumber(), outOrc251.getPlacerGroupNumber()));
    }
    ...
    if (inOrc231.getDateTimeOfTransaction() != null) {
      outOrc251.setDateTimeOfTransaction(
          mapTs231(inOrc231.getDateTimeOfTransaction(), outOrc251.getDateTimeOfTransaction()));
    }

    outOrc251 = mapCommonOrderListHelper1st(inOrc231, outOrc251);

    if (inOrc231.getAdvancedBeneficiaryNoticeCode() != null) {
      outOrc251.setAdvancedBeneficiaryNoticeCode(
          mapCe231(
              inOrc231.getAdvancedBeneficiaryNoticeCode(),
              outOrc251.getAdvancedBeneficiaryNoticeCode()));
    }
  ...
  public static Eip mapEip231(EIP in, Eip out) {
    out.setPlacerAssignedIdentifier(
        mapEi231(in.getParentSPlacerOrderNumber(), out.getPlacerAssignedIdentifier()));
    out.setFillerAssignedIdentifier(
        mapEi231(in.getParentSFillerOrderNumber(), out.getFillerAssignedIdentifier()));
    return out;
  }
  ...
  public static Ce mapCe231(CE in, Ce out) {
    ...
    out.setNameOfAlternateCodingSystem(in.getNameOfAlternateCodingSystem().getValue());
    return out;
  }
  ...
  private static Cx mapCxWithNullToCxAuthority(CX inOri, Cx cxOut) {
    if (inOri.getAssigningAuthority().getUniversalID() == null
      ...
      cxOut
          .getAssignAuthority()
          .setUniversalId(inOri.getAssigningAuthority().getUniversalID().getValue());
    }
    ...
    if (inOri.getAssigningAuthority().getUniversalIDType() == null
      ...
      cxOut
          .getAssignAuthority()
          .setUniversalIdType(inOri.getAssigningAuthority().getUniversalIDType().getValue());
    }
    return cxOut;
  }
  ...
  private static Cx mapCxWithNullToCxFacility(CX inOri, Cx cxOut) {
    if (inOri.getAssigningFacility() != null
      ...
      cxOut
          .getAssignFacility()
          .setNameSpaceId(inOri.getAssigningFacility().getNamespaceID().getValue());
      if (inOri.getAssigningFacility().getUniversalID() == null
        ...
        cxOut
            .getAssignFacility()
            .setUniversalId(inOri.getAssigningFacility().getUniversalID().getValue());
      }
      ...
      if (inOri.getAssigningFacility().getUniversalIDType() == null
        ...
        cxOut
            .getAssignFacility()
            .setUniversalIdType(inOri.getAssigningFacility().getUniversalIDType().getValue());
      }
    }
    return cxOut;
  }
  ...
  public static Cx mapCxWithNullToCx(CX inOri, Cx cxOut, String defaultValue) {
    ...
    if (inOri.getAssigningAuthority() != null
      ...
      cxOut
          .getAssignAuthority()
          .setNameSpaceId(inOri.getAssigningAuthority().getNamespaceID().getValue());
    ...
    // DI native
    cxOut.setAssignFacility(
        mapAssignedFacility(cxOut.getAssignFacility(), cxOut.getAssignAuthority()));
  ...
  public static Hd mapAssignedFacility(Hd out, Hd inAssignAuthority) {
    if (out.getUniversalId() == null || out.getUniversalId().isEmpty()) {
      out.setUniversalId(inAssignAuthority.getUniversalId());
    }

    // DI native
    if (out.getUniversalIdType() == null || out.getUniversalIdType().isEmpty()) {
      out.setUniversalIdType(inAssignAuthority.getUniversalIdType());
    }

    if (out.getNameSpaceId() == null || out.getNameSpaceId().isEmpty()) {
  ...
  public static ObservationResult mapObservationResultToObservationResult(
      ObservationResult obxIn, ObservationResult obxOut) {
    ...
    if (obxIn.getPerformingOrganizationAddress() == null
      ...
      obxOut
          .getPerformingOrganizationAddress()
          .getStreetAddress()
          .setStreetMailingAddress("Not present in v2.3.1 message");
    } else {
      obxOut.setPerformingOrganizationAddress(
          mapXad(
              obxIn.getPerformingOrganizationAddress(), obxOut.getPerformingOrganizationAddress()));
    }

    if (obxIn.getDateTimeOfTheAnalysis() == null
        || obxIn.getDateTimeOfTheAnalysis() != null
            && obxIn.getDateTimeOfTheAnalysis().getTime() == null) {
  ...
  public static ObservationRequest observationRequestToObservationRequest(
      OBR in231, ObservationRequest in, ObservationRequest out, Ts timestamp)
      throws DiHL7Exception {
    if (in.getResultRptStatusChngDateTime() == null
        || (in.getResultRptStatusChngDateTime().getTime() != null
            && in.getResultRptStatusChngDateTime().getTime().isEmpty())) {
  ...
  public static Prl mapPrl231(PRL in, Prl out) throws DiHL7Exception {
    if (in.getOBX3ObservationIdentifierOfParentResult() != null) {
      out.setParentObservationIdentifier(
          mapCe231(
              in.getOBX3ObservationIdentifierOfParentResult(),
              out.getParentObservationIdentifier()));
    }
    ...
    if (compositeResult != null
        && compositeResult.getExtraComponents() != null
        && compositeResult.getExtraComponents().numComponents() > 0) {
      for (int i = 0; i < compositeResult.getExtraComponents().numComponents(); i++) {
        try {
          descriptor =
              descriptor
                  + "&"
                  + compositeResult.getExtraComponents().getComponent(i).getData().encode();
        } catch (Exception e) {
  ...
  public static Specimen observationRequestToSpecimen(ObservationRequest in, Specimen out) {
    ...
    if (in.getSpecimenSource().getSpecimenSourceNameOrCode() == null
        || (in.getSpecimenSource().getSpecimenSourceNameOrCode() != null
            && in.getSpecimenSource().getSpecimenSourceNameOrCode().getIdentifier() == null)) {

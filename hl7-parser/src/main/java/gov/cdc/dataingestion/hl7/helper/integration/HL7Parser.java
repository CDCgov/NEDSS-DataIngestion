...
public class HL7Parser implements IHL7Parser {
  ...
  public String hl7ORUValidation(String message) throws DiHL7Exception {
    ...
    /**
     * Rule can be expanded if guideline is specified in the future - for now this is just the
     * template - Only enforcing Phone Number Rule
     */
  ...
  public HL7ParsedMessage convert231To251(String message, HL7ParsedMessage preParsedMessage)
    ...
    try {
      ...
      if (parsedMessage.getOriginalVersion().equalsIgnoreCase(SUPPORTED_HL7_VERSION_231)) {
        ...
        for (int a = 0; a < oru.getPatientResult().size(); a++) {
          ...
          oru.getPatientResult().get(a).getPatient().setPatientIdentification(mapPid(pid231, pid));
          // endregion
          ...
          for (int c = 0; c < oru.getPatientResult().get(a).getOrderObservation().size(); c++) {
            ...
            oru.getPatientResult()
                ...
                .get(c)
                .getSpecimen()
                .add(new gov.cdc.dataingestion.hl7.helper.model.hl7.message_group.Specimen(spc));
            // endregion
          }
          ...
          var orderORC251 =
              oru.getPatientResult().get(a).getOrderObservation().get(0).getCommonOrder();
          orderORC251 = mapCommonOrder(orderORC231, orderORC251);
          oru.getPatientResult()
              .get(a)
              .getOrderObservation()
              .get(0)
              .setCommonOrder(mapOBR2and3ToORC2and3(orderOBR231, orderORC251));
          // endregion
        ...
        throw new DiHL7Exception(
            "Unsupported message version. Please only specify HL7v2.3.1. Provided version is:\t"
                + parsedMessage.getOriginalVersion());
      }
    } catch (Exception e) {
  ...
  /** This should only take in 231 then return hapi 231 ORU_R01 object */
  public ca.uhn.hl7v2.model.v231.message.ORU_R01 hl7v231StringParser(String message)
      throws DiHL7Exception {
    try {
              ...
              case ORU_01:
                ORU_R01 msg =
                    (ca.uhn.hl7v2.model.v251.message.ORU_R01)
                        parser.parse(genericParsedMessage.getMessage());
                OruR1 oru = new OruR1(msg);
                parsedMessage.setParsedMessage(oru);

                if (genericParsedMessage
                    .getOriginalVersion()
                    .equalsIgnoreCase(SUPPORTED_HL7_VERSION_231)) {
                  parsedMessage = convert231To251(genericParsedMessage.getMessage(), parsedMessage);
                }
                break;
              default:
                throw new DiHL7Exception(
                    "Unsupported Event Trigger\t\t" + genericParsedMessage.getEventTrigger());
            }
            break;
          default:
            throw new DiHL7Exception(
                "Unsupported Message Type\t\t" + genericParsedMessage.getType());
        }

        return parsedMessage;
      } else {
        throw new DiHL7Exception(
            "Unsupported HL7 Version, please only specify either 2.3.1 or 2.5.1. Provided version is: \t\t"
                + genericParsedMessage.getOriginalVersion());
      }
  ...
  private HL7ParsedMessage<OruR1> hl7StringParseHelperWithTerser(String message)
      throws DiHL7Exception {
    try {
  ...
  // Context for terser
  private Message getMessageFromValidationAndParserContext(String message, HapiContext context)
      throws HL7Exception {
